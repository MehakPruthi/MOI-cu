...
# batch in Tom McCormack's GDP estimates
gdp <- read_csv("c:/projects/CVS_GPS/DNN/gdp.csv")

# batch in population by CSD
pop <- read_csv("c:/projects/CVS_GPS/DNN/csd_pop.txt") %>%
  subset(., select = c(csd, Pop))
...
# Calculate Expected ACcuracy by Random Chance
big_data1 <- read.csv("c:/projects/CVS_GPS/DNN/big_data1.csv")
...
# Xtabs for crosswalk of SCTG2 and CFAF
sctg2_names <- read.csv("c:/projects/cvs_gps/dnn/sctg2_names.csv")

cfaf_sctg2 <- big_data1 %>%
  group_by(ID) %>%
  summarise(sctg2 = first(sctg2), cfaf = first(cfaf_group), dist = mean(tour_dist)) %>%
  merge(., sctg2_names, by="sctg2", all.x=TRUE) %>%
  subset(., cfaf != 0)

shrt_dist <- subset(cfaf_sctg2, dist < 45)
  
print(kable(addmargins(xtabs(~ cfaf + name, cfaf_sctg2))) %>%
  kable_styling("striped",  font_size=12) %>%
  row_spec(0, angle = -90, color = "black") %>%
  column_spec(1, color = "black", bold = T) %>%
  row_spec(c(1:16), color = "black"))

c1 <- subset(cfaf_sctg2, name != "empty")
ggplot(c1, aes(x=name)) + 
  geom_bar(stat = "count") +
...
dnn_df_partial3 <- big_data1 %>%
  .[,c(1:2)] %>%
  group_by(ID) %>%
  summarise(first_csd = first(csduid), last_csd = last(csduid) )
...
dnn_df <- merge(dnn_df_partial1, dnn_df_partial2, by="ID") %>%
  ...
  merge(., gdp, by.x = "first_csd", by.y = "csd", all.x = TRUE) %>%
  merge(., gdp, by.x = "last_csd", by.y = "csd", all.x = TRUE) %>%
  merge(., pop, by.x = "first_csd", by.y = "csd", all.x = TRUE) %>%
  merge(., pop, by.x = "last_csd", by.y = "csd", all.x = TRUE) %>%
  select(c(ID, first_csd, last_csd, gdp.x, gdp.y, Pop.x, Pop.y), everything())
dnn_df[is.na(dnn_df)] <- 0
...
dnn_df$sum <- rowSums(dnn_df[, 11:50])
...
#############################
# new tours that Bryce generated. Need to bring in the week flag and the CFAF definition
...
dnn_df <- read_csv("c:/projects/CVS_GPS/DNN/dnn_df_variablebuffer_1.csv") %>%
  merge(., cfaf_tour, by = "ID", all.x=TRUE) %>% 
  merge(., wk_flag, by = "ID", all.x= TRUE) %>%
  drop_na() %>%
  select(c(ID, cfaf, week_flag), everything())


# How many unique combinations of CSDs
unique_csd <- dnn_df %>%
  group_by(first_csd, last_csd) %>%
  summarise(count = n())
...
testSet$first_csd <- as.factor(testSet$first_csd)
testSet$last_csd <- as.factor(testSet$last_csd)
...
trainSet$first_csd <- as.factor(trainSet$first_csd)
trainSet$last_csd <- as.factor(trainSet$last_csd)


##################################################################
cfaf_lablels <- c("Agriculture", "Base Metals", "Food Products", "Forest Products", "Fuels", "Miscellaneous", "Minerals", 
                  "Other Manufacturing", "Plastic Chemicals", "Transportation", "Waste")

ggplot(rbind(trainSet, testSet), aes(x=class)) + 
  geom_bar(stat = "count") +
  theme(axis.text.x=element_text(angle=90,hjust=1, size = 12)) +
  ggtitle("Final commodities after including the synthesized ones") + 
  scale_x_discrete(labels= cfaf_lablels)
