...
MODEL_YEAR <- 2018 # TODO: Replace this hard-coded value with user-input value from visualization
...
U_AVG <- 0.8 # Replace with user input from Shiny
...
user_input_cost_per_sqft <- 1000 # TODO: Replace this hard-coded value with user-input value from visualization
...
courthouse_asset_updated <- courthours_od %>% 
  ...
  ungroup() %>% 
  mutate(standard.sqft.per.courtroom = courtroom_size(courtrooms.needed),
         area.shortfall = standard.sqft.per.courtroom - existing.rentable.sqft) %>% 
  select(bid, courtrooms, courtrooms.needed, court.hours.distributed, utilization, standard.sqft.per.courtroom)
```
...
future_needs_util <- future_cases_bl_cd_util %>%
  ...
  mutate(op.utilization = ifelse(util.court.hours > inflect.hours & util.court.hours <= max.hours,
                                 inflect.hours.dy + ((max.hours.dy - inflect.hours.dy) * (util.court.hours - inflect.hours)                                                       / (max.hours - inflect.hours)), op.utilization)) %>% 
  mutate(op.utilization = ifelse(util.court.hours > max.hours, max.hours.dy, op.utilization)) %>% 
...
# Read in list of new courtrooms by courthoue
...
new_courthouse_list_complete <- new_courthouse_list %>% 
  mutate(cost_2018 = round(user_input_courtrooms_total * user_input_cost_per_sqft + user_input_flat_courthouse_cost, 0))
