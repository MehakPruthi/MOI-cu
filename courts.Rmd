...
### Projecting for CD level demand - 2 approaches used for WSP to choose one
#### Approach 1: CD is assumed as 1 large courthouse
...
# Calculating utilization for CD level - 3 approaches used for WSP to choose one
...
cd_utilization_appr2 <- future_needs_cd_casetypes %>% 
  group_by(cduid) %>% 
  mutate(total = sum(courthours.needed)) %>% 
  mutate(weight= courthours.needed/total) %>% 
  mutate(adjst_courtrooms_needed = courtrooms.needed*weight) %>% 
  select(-total, -weight) %>% 
