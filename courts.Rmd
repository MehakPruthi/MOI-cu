...
The age ranges for business lines are as follows:
...
courthouse_asset_with_projected_cd <- courthouse_asset %>%
  ...
  mutate(rentable.square.feet.needed = courtrooms.needed * NEW_COURTROOM_SIZE) %>% 
  ungroup()
...
# courthouse_asset_user <- courthouse_asset %>% 
#   rbind(courthouse_asset_new)
...
courthours_od <- treso_demand_origin %>%
  ...
  left_join(courthouse_available, by = c("cduid", "cdname")) %>% 
  ...
  # Give a utility ranking for each POR, casetypes
...
appearance_trip_list <- courthours_od %>% 
  ...
  group_by(treso.id.por, treso.id.pos, casetypes) %>% 
  summarise(court.hours = sum(court.hours.to.closest, court.hours.to.random)) %>% 
  left_join(APPEARANCE_FACTOR, by = "casetypes") %>% 
  mutate(appearances = court.hours / factor) %>% 
...
courthouse_asset_updated <- courthours_od %>% 
  ...
  mutate(court.hours.distributed = court.hours.to.closest + court.hours.to.random) %>% 
  left_join(utilization_rate, by = "casetypes") %>% 
  mutate(op.utilization = 0) %>% 
