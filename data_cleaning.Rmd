...
##Import Health Data from CSVs
  ...
  - Join HBAM with Asset data to establish master ID list
...
HBAMhistorical <- read_csv('input/health/cap.MOH_HBAM_HIS.csv') %>% 
  ...
  mutate(agegroup = replace(agegroup, agegroup == '0-17', 1),
         ...
         agegroup = replace(agegroup, agegroup == '<1', 'ped1'),
         agegroup = replace(agegroup, agegroup == '1-7', 'ped2'),
         agegroup = replace(agegroup, agegroup == '8-17', 'ped3'),
...
HBAMhistorical_csd_spread <- HBAMhistorical_master %>%
  ...
  summarise(cases_tot = sum(cases_ag1, cases_ag2, cases_ag3, cases_ag4, cases_ag5, cases_ag6, cases_ped1, cases_ped2, cases_ped3, na.rm = TRUE),
            cases_ag1 = sum(cases_ag1, cases_ped1, cases_ped2, cases_ped3, na.rm = TRUE),
...
HBAMhistorical_csd <- HBAMhistorical_master %>% 
  select(siteid,
         ...
         csd) %>% 
  ungroup() %>% 
  mutate(agegroup = replace(agegroup, agegroup == 'ped1', 1),
         agegroup = replace(agegroup, agegroup == 'ped2', 1),
         agegroup = replace(agegroup, agegroup == 'ped3', 1)) %>% 
...
rm(HBAMhistorical_csd_spread, csd_pop, csd_pop_long)
```
...
HBAMhistorical_cd_spread <- HBAMhistorical_csd_spread %>% 
  mutate(cd = substring(csd, 0,4)) %>% 
  group_by(cd, caretype) %>% 
  summarise(cases_ag1 = sum(cases_ag1),
...
HBAMhistorical_cd_long <- gather(HBAMhistorical_cd_spread, agegroup, totalcases, cases_ag1:cases_ag6) %>% 
  mutate(agegroup = replace(agegroup, agegroup == 'cases_ag1', 1),
         agegroup = replace(agegroup, agegroup == 'cases_ag2', 2),
         agegroup = replace(agegroup, agegroup == 'cases_ag3', 3),
         agegroup = replace(agegroup, agegroup == 'cases_ag4', 4),
         agegroup = replace(agegroup, agegroup == 'cases_ag5', 5),
         agegroup = replace(agegroup, agegroup == 'cases_ag6', 6)) 
...
cd_caserates <- left_join(HBAMhistorical_cd_long, cd_pop_long, by = c('cd', 'agegroup')) %>% 
