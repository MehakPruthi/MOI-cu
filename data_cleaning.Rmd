...
```{r, include= FALSE}
#Bed Census Data
dailybedhistory <- read.csv('input/health/cap.MOH_DailyBedCensus_Historical.csv')
...
ALCmostappropriate <- read.csv('input.health/ALC.MostAppropriateDischarge from CCO_2019-06-28.csv')

#CIHI Data
CIHIhospitalbeds <- read.csv('input.health/cap.CIHI_Hospital_Beds.csv')

#Health Asset Inventory
hospitallocations <- read.csv('input.health/cap.MOH_Hospital_Locations.csv')
...
HBAMhistorical %>% 
  group_by(agegroup,
           year) %>% 
  summarise(totalsagegroup = sum(totalcases))

#HBAM Projected Data
HBAMproj <- HBAMprojected %>% 
  select(year = Year, 
         siteid = Hospital_Site_ID, 
         hospitalname = Hospital_Site_Name,
         caretype = Care_Type,
