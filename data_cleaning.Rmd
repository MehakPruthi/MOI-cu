...
dsb_mapping <- school_treso_master <- readRDS('cache/edu/school_treso_master.rds') %>%
  ...
  select(year, dsb.index, panel, cduid) %>% 
  group_by(dsb.index, panel, cduid) %>% 
...
dsb_mapping %>% 
  select(cduid, panel, board_type_name, dsb.index) %>% 
...
schoolCapHistorical <- schoolCap %>%
  ...
  saveRDS("cache/edu/school_historical_capacity.rds")
...
hospitallocationsprep <- hospitallocations %>%
  ...
  ungroup() %>% 
  # Lat/long incorrect in raw input data for Providence Care Hospital, and Chatham-Kent in Wallaceburg
  mutate(lat = replace(lat, id == 33, 44.217317),
         long = replace(long, id == 33, -76.527267),
         lat = replace(lat, id == 173, 42.5575838),
         long = replace(long, id == 173, -82.4876522))
```
...
HBAMhistoricalprep <- HBAMhistorical %>% 
  ...
  # filter(year == 2016) %>% 
  # select(-year) %>% 
  filter(orig_csd < 4600000, orig_csd > 3501000) %>% 
  filter(agegroup != '<1', agegroup != '1-7', agegroup != '8-17') %>% #remove overlapping agegroups - assumes they are duplicates - to confirm with MOH
...
HBAMhistorical_master <- HBAMhistoricalprep %>% 
  mutate(caretype = replace(caretype, caretype == "ER", "AM"),
         caretype = replace(caretype, caretype == "DS", "AM")) %>%   # changing RE and DS (2 care types) to AM will create duplicates that need to be grouped and added together
  group_by(year, siteid, id, caretype, agegroup, orig_csd) %>%  
  summarise(cases = sum(cases))%>% # add ER and DS together into a combined AM total 
  left_join(distinct(hospitallocationsapp, id, name, pcode, lat, long), by = c('id'))
  #joining with facilityid creates duplicate records
...
HBAMprojectedprep <- HBAMprojected %>% 
  ...
  mutate(agegroup = replace(agegroup, agegroup == '0-17', 1),
         ...
         caretype = replace(caretype, caretype == 'DS', 'AM')) %>%  # changing the ER and DS (2 care types) to AM will create duplicates that need to be grouped and added together
  group_by(year, siteid, caretype, agegroup) %>%  
  summarise(cases = sum(cases),lengthofstay = sum(lengthofstay)) %>% # add ER and DS together into a combined AM total 
  left_join(select(hospitallocationsapp, datasetID1.siteid, id), by = c('siteid' = 'datasetID1.siteid')) %>% 
  left_join(select(hospitallocationsapp, datasetID2.siteid, id), by = c('siteid' = 'datasetID2.siteid')) %>% 
  left_join(select(hospitallocationsapp, datasetID3.siteid, id), by = c('siteid' = 'datasetID3.siteid')) %>% 
  left_join(select(hospitallocationsapp, datasetID4.siteid, id), by = c('siteid' = 'datasetID4.siteid')) %>% 
...
num_beds <- dailybedcensusprepagg %>%
  ...
  distinct(year, type, siteid, censusdate, staffedbeds, totstaffbeds) %>% 
  group_by(year) %>%
