...
#Bed Census Data
dailybedcensus <- read.csv('input/health/cap.MOH_DailyBedCensus_Summary.csv') %>% 
...
hospitaldischarges <- read.csv('input/health/cap.MOH_Discharges.csv')

#HBAM Data
HBAMhistorical <- read.csv('input/health/cap.MOH_HBAM_HIS.csv') %>% 
  select(year = Year, 
         ...
         csd = CSD)
HBAMprojected <- read.csv('input/health/cap.MOH_HBAM_Proj.csv') %>% 
    select(year = Year, 
         siteid = Hospital_Site_ID, 
         hospitalname = Hospital_Site_Name,
         caretype = Care_Type,
...
#LHIN Data
populationLHIN <- read.csv('input/health/dbo.Population_LHIN.csv')

#ALC Data
##Reference Files
ALCage <- read.csv('input/health/cap.ALCAge.csv')
ALCcases <- read.csv('input/health/cap.ALCCases.csv')
ALCdesignations <- read.csv('input/health/cap.ALCDesignations.csv')
ALCdischargedest <- read.csv('input/health/cap.ALCDischargeDestination.csv')
ALCinpatientbedtype <- read.csv('input/health/cap.ALCInpatientBedType.csv')
#ALC Datasets
ALCannualvolume <- read.csv('input/health/cap.ALCAnnualVolumeAndTOtalDays.csv') %>%
  select(year = Year,
         siteid = SiteNumber,
         casetype = CaseType,
...
ALCmostappropriate <- read.csv('input/health/ALCMostAppropriateDischage.csv') %>% 
...
#Health Asset Inventory
hospitallocations <- read.csv('input/health/dbo.MOH_Hospital_Locations.csv') %>% 
  select(lhin = LHIN,
...
#CCO Linkage Table
idcrosswalk <- read.csv('input/health/SiteID_Linkage_Table.csv') %>% 
