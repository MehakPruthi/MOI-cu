...
# TRESO Data Structuring 
```{r treso data structuring, echo=FALSE}
# Clean hhld_df
...
treso_tb <- socio_economic_tb %>%
  ...
  saveRDS("cache/treso/treso_tb.rds")
...
schoolAssetsPortablesSlim <- schoolAssetsPortables %>%
  ...
  mutate(num.units.calculated.part1 = trunc(area.m2 / PORTABLE_UNIT_SIZE)) %>% 
  mutate(num.units.calculated.part2 = ifelse((area.m2 / PORTABLE_UNIT_SIZE) - num.units.calculated.part1 >= 0.75,
         1, 0)) %>% 
  mutate(num.units.calculated = num.units.calculated.part1 + num.units.calculated.part2) %>% 
  select(-num.units.calculated.part1, -num.units.calculated.part2)

```

## Add FCI, renewal to asset data
```{r add fci, echo=FALSE}
...
## Create School Master List (past & present)

```{r create school master list}
...
schoolForecastSimpleJoin <- schoolForecastSimple %>% 
  left_join(select(school_recent_data, sfis, board_type_name, board.name, school.lat, school.long, otg), 
            by = c('sfis')) %>% 
...
# Save to cache
saveRDS(schoolSFIS, "cache/edu/school_sfis_2017")
...
for (catchment_dist in percentileList){
  schoolCatchment %>%
    ...
    saveRDS(file = paste0('cache/edu/student_travel_', as.character(catchment_dist), '.rds'))
}
...
student_travel <- readRDS(file = 'cache/edu/student_travel_0.9.rds') %>%
  as_tibble() %>%
  group_by(sfis, school.name, school.lat, school.long) %>%
...
saveRDS(eqao_standardized, file = 'cache/edu/eqao_standardized_2017.rds')
...
hospitallocations <- read_csv('input/moh/dbo.MOH_Hospital_Locations.csv') %>%
  ...
  # Omit from location data any hospitals not found in HBAM data
  #inner_join(distinct(HBAMhistorical, siteid, caretype), by = c('datasetID1.siteid' = 'siteid', 'caretype')) %>%
  #inner_join(distinct(HBAMprojected, siteid, caretype), by = c('datasetID1.siteid' = 'siteid', 'caretype')) %>%
...
ALC_idcrosswalk <- read_csv('input/moh/SiteID_Linkage_Table.csv') %>% 
  ...
  select(lhin, name, alcname, caretype = type, pcode, id, ALCsite_id, csduid, csdname)
...
num_beds_province <- dailybedcensus %>%
