...
annualizationfactor <- dailybedcensus %>% 
  group_by(siteid,
           type,
           year) %>% 
  left_join(censusentrycount, by = c('siteid' = 'siteid', 'year' = 'year')) %>% 
...
HBAMhistorical_csd_spread <- HBAMhistorical_master %>%
  ...
  summarise(cases_tot = sum(cases_ag1, cases_ag2, cases_ag3, cases_ag4, cases_ag5, cases_ag6, cases_ped1, cases_ped2, cases_ped3, na.rm = TRUE),
            cases_ag1 = sum(cases_ag1, na.rm = TRUE),
            ...
            cases_ped1 = sum(cases_ped1, na.rm = TRUE),
            cases_ped2 = sum(cases_ped2, na.rm = TRUE),
            cases_ped3 = sum(cases_ped3, na.rm = TRUE))
...
HBAMhistorical_cd_spread <- HBAMhistorical_csd_spread %>% 
  ...
  summarise(cases_ag1 = sum(cases_ag1, cases_ped1, cases_ped2, cases_ped3),
            cases_ag2 = sum(cases_ag2),
            cases_ag3 = sum(cases_ag3),
            cases_ag4 = sum(cases_ag4),
            cases_ag5 = sum(cases_ag5),
...
cd_caserate_ct_agegroup <- left_join(HBAMhistorical_cd_long, cd_pop_long, by = c('cd', 'agegroup')) %>% 
...
saveRDS(cd_caserate_ct_agegroup, file = 'cache/hospitals/cd_caserates.rds')
