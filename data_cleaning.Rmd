...
HBAMhistorical_csd_spread <- HBAMhistorical %>%
  ...
  summarise(cases_tot = sum(cases_ag1, cases_ag2, cases_ag3, cases_ag4, cases_ag5, cases_ag6, na.rm = TRUE),
...
###Census Division (CD) Caserate Calculations
  ...
  - Note: age groups ped1:ped3 redundant within age group "1", exclude when totalling
```{r}
...
HBAMhistorical_cd_long <- gather(HBAMhistorical_cd_spread, agegroup, totalcases, cases_ag1:cases_ped3) %>% 
    mutate(agegroup = replace(agegroup, agegroup == 'cases_ag1', 1),
         ...
         agegroup = replace(agegroup, agegroup == 'cases_ped1', 'ped1'),
         agegroup = replace(agegroup, agegroup == 'cases_ped2', 'ped2'),
         agegroup = replace(agegroup, agegroup == 'cases_ped3', 'ped3'))
...
cd_pop_long <- gather(cd_pop, agegroup, population, pop_ag1:pop_ped3) %>% 
