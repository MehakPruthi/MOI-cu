...
## Import Health Data from SQL
```{r, include=FALSE, eval=FALSE}
healthRodbc <-odbcConnect("SQLDSN")
...
ALCmostappropriate <- read_csv('P:/Y362/2019/191-00826-00 MOI Capacity and Demand Model/3. Modelling/moi_cu_project/input/health/ALC.MostAppropriateDischage from CCO_2019-06-28.csv')
...
# Bed Census Data
dailybedcensus <- read_csv('input/health/cap.MOH_DailyBedCensus_Summary.csv') %>% 
...
hospitaldischarges <- read_csv('input/health/cap.MOH_Discharges.csv') %>%
...
# HBAM Historical
HBAMhistorical <- read_csv('input/health/cap.MOH_HBAM_HIS.csv') %>% 
  select(year = Year, 
...
saveRDS(HBAMhistorical, file = 'cache/hospitals/HBAMhistorical.rds')

## Identifying Duplicated Site IDs
duplicated_siteid <- HBAMhistorical %>% 
  filter(year == 2016) %>%
  group_by(siteid, caretype) %>%
  distinct(siteid) %>% 
  ungroup(caretype) %>% 
  add_count(siteid) %>% 
  filter(n > 1) %>% 
...
# Health Asset Inventory
hospitallocations <- read_csv('input/health/dbo.MOH_Hospital_Locations.csv') %>% 
  select(lhin = LHIN,
         lhinlocation = Location,
         name = Name,
         caretype = Type,
         siteid = 'Master Number',
         ...
         lat = latitude,
         long = longitude) %>% 
  left_join(select(HBAMhistorical, siteid, caretype), by = c('siteid', 'caretype')) %>% 
  distinct()

saveRDS(hospitallocations, file = 'cache/hospitals/hospitallocations.rds')

# Historical HBAM Master Dataset
HBAMhistorical_master <- HBAMhistorical %>% 
  mutate(caretype = replace(caretype, caretype == "ER", "AM"),
         caretype = replace(caretype, caretype == "DS", "AM")) %>% 
  group_by(siteid, caretype, agegroup, csd) %>% 
  summarise(totalcases = sum(totalcases)) %>% 
  left_join(hospitallocations, by = c('siteid', 'caretype')) %>% 
  filter(!is.na(lat))

saveRDS(HBAMhistorical_master, file = 'cache/hospitals/hbamhistorical_master.rds')

# HBAM Projected
HBAMprojected <- read_csv('input/health/cap.MOH_HBAM_Proj.csv') %>% 
    select(year = Year, 
         siteid = Hospital_Site_ID, 
         hospitalname = Hospital_Site_Name,
         caretype = Care_Type,
...
#LHIN Data
populationLHIN <- read_csv('input/health/dbo.Population_LHIN.csv')

#ALC Data
##Reference Files
ALCage <- read_csv('input/health/cap.ALCAge.csv')
ALCcases <- read_csv('input/health/cap.ALCCases.csv')
ALCdesignations <- read_csv('input/health/cap.ALCDesignations.csv')
ALCdischargedest <- read_csv('input/health/cap.ALCDischargeDestination.csv')
ALCinpatientbedtype <- read_csv('input/health/cap.ALCInpatientBedType.csv')

#ALC Datasets
ALCannualvolume <- read_csv('input/health/cap.ALCAnnualVolumeAndTOtalDays.csv') %>%
  select(year = Year,
         siteid = SiteNumber,
         casetype = CaseType,
...
ALCmostappropriate <- read_csv('input/health/ALCMostAppropriateDischage.csv') %>% 
...
ALCmostappropriate %>% #Check number of months in Year entries (5 mo. in 2019)
  group_by(year) %>% 
  distinct(month) %>% 
  count()

ALCmostappropriate <- ALCmostappropriate %>% 
  group_by(year, siteid, agegroup, madd) %>%
  summarise(freq = sum(freq),
            totalALCdays = sum(totalALCdays))

#CIHI Data
# CIHIhospitalbeds <- read.csv('input.health/dbo.CIHI_Hospital_Beds.csv')



#CCO Linkage Table
ALC_idcrosswalk <- read_csv('input/health/SiteID_Linkage_Table.csv') %>% 
...
ALC_idlinkage <- left_join(hospitallocations, idcrosswalk, by = c('siteid', 'facilityid'), suffix = c('.moh', '.cco'))

#Identify Mismatched Site IDs
ALC_mismatchedids <- idlinkage %>% 
  filter(is.na(type.cco))
...
ALC_idlinkage %>% 
  count(type.cco %in% type.moh)

#Filter out Mismatched IDs
hospital_ALC_master <- idlinkage %>% 
  filter(!is.na(type.cco)) %>% 
  group_by(siteid, facilityid, type.moh, name.moh) %>% 
  distinct()
```
...
```{r}
#TRESO-CSD Crosswalk
treso_csd_tb <- read_csv('input/treso/treso_zone_system.csv') %>% 
  rename('treso_zone' = 'treso_id')

treso_health_tb <- socio_economic_tb %>%
  group_by(treso_zone) %>%
  summarise(
    n_pop = n(),
    n_hhlds = length(unique(hhid)),
    ...
    n_pt = length(treso_zone[work_status == "part_time"]),
    n_unemp = length(treso_zone[work_status == "unemployed"]),
    n_ag1_pop = length(treso_zone[age <= 17 & age >= 0]),
    n_ag2_pop = length(treso_zone[age <= 44 & age >= 18]),
    n_ag3_pop = length(treso_zone[age <= 64 & age >= 45]),
    n_ag4_pop = length(treso_zone[age <= 74 & age >= 65]),
    n_ag5_pop = length(treso_zone[age <= 84 & age >= 75]),
    n_ag6_pop = length(treso_zone[age >= 85]),
    # n_ped1_pop = length(treso_zone[age <= 1]),
    # n_ped2_pop = length(treso_zone[age <= 7 & age >= 1]),
    # n_ped3_pop = length(treso_zone[age <= 17 & age >= 8]),
  ...
  # left_join(hhld_structure, by = "treso_zone") %>%
  left_join(treso_csd_tb, by = 'treso_zone')
  # saveRDS("cache/treso_tb.rds")

saveRDS(treso_health_tb, file = 'cache/hospitals/treso_health_tb.rds')

# Clean global variables
rm(hhld_tb, hhld_df, pop_df, socio_economic_tb)

```

##Daily Bed Census Data Cleaning and Aggregation
...
dailyentrycount <- dailybedcensus %>% 
  group_by(siteid, year) %>% 
  count(siteid) %>% 
  rename('entrycount' = 'n')

#Annual Bed-Days Observed
beddaysobserved <- dailybedcensus %>% 
  group_by(siteid,
           type,
           year) %>% 
...
annualizationfactor <- dailybedcensus %>% 
  group_by(siteid,
           type,
           year) %>% 
...
#Physcial Capacity
Hospitalphyscap <- dailybedcensus %>% 
...
##HBAM Data and Caserate Caculations
...
```{r}
#Adjust HBAM data to Match TRESO population spread
# HBAMhistorical_csd_spread <- HBAMhistorical_master %>%
#   select(siteid,
#          caretype,
#          agegroup,
#          totalcases,
#          csd) %>%
#   spread(agegroup, totalcases) %>%
#   rename('cases_ag1' =  "1",
#          'cases_ag2' = "2",
#          'cases_ag3' = "3",
#          'cases_ag4' = "4",
#          'cases_ag5' = "5",
#          'cases_ag6' = "6") %>%
#   group_by(csd, caretype) %>%
#   summarise(cases_ag1 = sum(cases_ag1, na.rm = TRUE),
#             cases_ag2 = sum(cases_ag2, na.rm = TRUE),
#             cases_ag3 = sum(cases_ag3, na.rm = TRUE),
#             cases_ag4 = sum(cases_ag4, na.rm = TRUE),
#             cases_ag5 = sum(cases_ag5, na.rm = TRUE),
#             cases_ag6 = sum(cases_ag6, na.rm = TRUE))
...
HBAMhistorical_csd <- HBAMhistorical_master %>% 
  select(siteid,
         ...
         csd) %>% 
  ungroup() %>% 
  group_by(csd, caretype, agegroup) %>% 
  summarise(totalcases = sum(totalcases))

#Aggregate TRESO pop data to the CSD-level
csd_pop <- treso_health_tb %>% 
  ungroup(treso_zone) %>%
  rename('csd' = 'csduid') %>% 
  group_by(csd) %>% 
  summarise(pop_ag1 = sum(n_ag1_pop),
            pop_ag2 = sum(n_ag2_pop),
            pop_ag3 = sum(n_ag3_pop),
            pop_ag4 = sum(n_ag4_pop),
            pop_ag5 = sum(n_ag5_pop),
            pop_ag6 = sum(n_ag6_pop))

  #Convert TRESO-CSD Population data to long format
csd_pop_long <- gather(csd_pop, agegroup, population, pop_ag1:pop_ag6) %>% 
...
csd_caserates <- left_join(HBAMhistorical_csd, csd_pop_long, by = c('csd', 'agegroup')) %>% 
  mutate(caserate = totalcases/population) %>% 
  filter(!is.na(caserate))

#Calculte Mismatched CSDs
mismatched_csd <- left_join(HBAMhistorical_csd, csd_pop_long, by = c('csd', 'agegroup')) %>% 
  filter(is.na(population)) %>% 
  summarise(sum(totalcases)) #total of 51,755 mismatched cases from 89 CSDs

saveRDS(csd_caserates, file = 'cache/hospitals/csd_caserates.rds')
rm(csd_pop, csd_pop_long)
```

###Census Division (CD) Caserate Calculations
...
HBAMhistorical_cd <- HBAMhistorical_master %>% 
  ...
  select(siteid,
         ...
         cd) %>% 
  ungroup() %>% 
  group_by(cd, caretype, agegroup) %>% 
  summarise(totalcases = sum(totalcases))

cd_pop <- csd_pop %>% 
  mutate(cd = substring(csd, 0,4)) %>% 
  group_by(cd) %>% 
  summarise(pop_ag1 = sum(pop_ag1),
            pop_ag2 = sum(pop_ag2),
            pop_ag3 = sum(pop_ag3),
            pop_ag4 = sum(pop_ag4),
            pop_ag5 = sum(pop_ag5),
...
cd_pop_long <- gather(cd_pop, agegroup, population, pop_ag1:pop_ag6) %>% 
  mutate(agegroup = replace(agegroup, agegroup == 'pop_ag1', 1),
         agegroup = replace(agegroup, agegroup == 'pop_ag2', 2),
         agegroup = replace(agegroup, agegroup == 'pop_ag3', 3),
         agegroup = replace(agegroup, agegroup == 'pop_ag4', 4),
...
cd_caserates <- left_join(HBAMhistorical_cd, cd_pop_long, by = c('cd', 'agegroup')) %>% 
  ungroup() %>% 
  mutate(caserate = totalcases/population,
         cd = as.numeric(cd))

mismatched_cd <- left_join(HBAMhistorical_cd_long, cd_pop_long, by = c('cd', 'agegroup')) %>% 
  filter(is.na(population)) %>% 
  summarise(sum(totalcases))

saveRDS(cd_caserates, file = 'cache/hospitals/cd_caserates.rds')
saveRDS(HBAMhistorical_cd_long, file = 'cache/hospitals/hbam_cd.rds')

```

###Provincial Caserate Calculations
...
three_digit_ALC <- ALCannualvolume %>%
  filter(siteid < 999,
         siteid > 99) %>% 
  select (-casetype,
          -bedtype) 
...
courthouse_asset <- read_csv("input/mag/Court asset data.csv") %>% 
  select(year = Collection_Year, bid = BID, name = BuildingName, address = Address1,
         ...
         building.type = BuildingType, courtrooms = Courtrooms, region = Region, rentable.square.feet = Rentable_square_feet,
...
saveRDS(courthouse_hours_overlay, "cache/mag/courthouse_hours.rds")

#issue long form of court hour dataset for easy use
court_overlay_long <- courthouse_hours_overlay %>%
...
court_overlay_long$casetypes <- substring(court_overlay_long$casetypes, 2)

# Save the wide court hour dataset
saveRDS(court_overlay_long, "cache/mag/courthouse_hours.rds")
...
# CD as the base courthouse, create an ID system for the courthouse asset
...
courthouse_asset_overlay <- create_overlay(courthouse_asset_xy, treso_shp, type = "court") %>% 
  ...
  select(-courtrooms) %>% 
  left_join(select_at(courthouse_hours_overlay, vars(bid, contains("number_of_courtrooms"))), by = "bid")

# Distribute the base courthouse's courtrooms to satellite courthouses
multiple_courthouse_asset <- courthouse_asset_overlay %>% 
  filter(building.type == "Base Court") %>% 
  group_by(cdname) %>% 
  summarise(mutilple.basecourts.tag = ifelse(n() > 1, 1, 0))

courthouse_asset_overlay <- left_join(courthouse_asset_overlay, multiple_courthouse_asset, by = "cdname") 

multiple_basecourts <- filter(courthouse_asset_overlay, mutilple.basecourts.tag == 1) %>% 
  select_at(vars(bid, cdname, cduid, name, lat, long, building.type, contains("number_of_courtrooms"))) %>%    
  mutate_at(vars(contains("number_of_courtrooms")), funs(ifelse(building.type == "Satellite Court", 1, .)))

single_basecourts <- filter(courthouse_asset_overlay, mutilple.basecourts.tag == 0) %>% 
  select_at(vars(bid, cdname, cduid, name, building.type, contains("number_of_courtrooms"))) %>%    
  mutate_at(vars(contains("number_of_courtrooms")), funs(ifelse(building.type == "Satellite Court", 1, .))) %>% 
  group_by(cdname) %>% 
  mutate_at(vars(contains("number_of_courtrooms")), list(sum = sum)) %>% 
  ungroup() %>% 
  # Didn't spend time to find an easy way to generalize this...so here is explicit code
  mutate(`2014_number_of_courtrooms_distributed` = 
           `2014_number_of_courtrooms` +  (`2014_number_of_courtrooms` - `2014_number_of_courtrooms_sum`)) %>% 
  mutate(`2014_number_of_courtrooms` = ifelse(building.type == "Base Court", 
                                              `2014_number_of_courtrooms_distributed`,
                                              `2014_number_of_courtrooms`)) %>% 
  mutate(`2015_number_of_courtrooms_distributed` = 
           `2015_number_of_courtrooms` +  (`2015_number_of_courtrooms` - `2015_number_of_courtrooms_sum`)) %>% 
  mutate(`2015_number_of_courtrooms` = ifelse(building.type == "Base Court", 
                                              `2015_number_of_courtrooms_distributed`,
                                              `2015_number_of_courtrooms`)) %>% 
  mutate(`2016_number_of_courtrooms_distributed` = 
           `2016_number_of_courtrooms` +  (`2016_number_of_courtrooms` - `2016_number_of_courtrooms_sum`)) %>% 
    mutate(`2016_number_of_courtrooms` = ifelse(building.type == "Base Court", 
                                              `2016_number_of_courtrooms_distributed`,
                                              `2016_number_of_courtrooms`)) %>% 
  mutate(`2017_number_of_courtrooms_distributed` = 
           `2017_number_of_courtrooms` +  (`2017_number_of_courtrooms` - `2017_number_of_courtrooms_sum`)) %>% 
  mutate(`2017_number_of_courtrooms` = ifelse(building.type == "Base Court", 
                                              `2017_number_of_courtrooms_distributed`,
                                              `2017_number_of_courtrooms`)) %>% 
  mutate(`2018_number_of_courtrooms_distributed` = 
           `2018_number_of_courtrooms` +  (`2018_number_of_courtrooms` - `2018_number_of_courtrooms_sum`)) %>% 
  mutate(`2018_number_of_courtrooms` = ifelse(building.type == "Base Court", 
                                              `2018_number_of_courtrooms_distributed`,
                                              `2018_number_of_courtrooms`)) %>% 
  select_at(vars(bid, ends_with("number_of_courtrooms")))

saveRDS(courthouse_asset, "cache/mag/courthouse_master.rds")
```
