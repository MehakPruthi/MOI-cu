...
- Combine the asset data with the 5 year historical data
- There is a mismatch between asset data and historical data, the asset data splits up the satellite courthouses while the historical data combines them as one entry

```{r clean courthouse}

courthouse_asset <- courthouse_asset %>%
   # Remove entries wh ich are not base court or satelite court
  filter(building.type %in% c("Base Court", "Satellite Court")) %>%
...
# Join the courthouse_asset dataset to get lat/long information
...
court_xy <- create_court_xy(courthouse_hours_wide_location)
court_overlay <- create_overlay(court_xy, treso_shp, type = "court")
...
court_overlay_long <- unique(court_overlay) %>% #Remove unique after Paul fixes duplicates
...
# There is a difference between courthouse_hours data and courthouse_asset data in terms of the number of courtrooms
print(paste0("There is a difference between the number of courtrooms in the asset data (", sum(courthouse_asset$courtrooms), "), and the number of courtrooms in the historical data (", sum(courthouse_hours_wide_location$`2018_number_of_courtrooms`), "). Thus, certain assumptions must be made to combine the two dataset"))
