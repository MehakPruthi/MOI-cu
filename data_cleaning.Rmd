...
schoolCapSlim <- schoolCap %>%
  ...
  filter(status == "Open", dataset.id == 2, !is.na(school.lat), !is.na(school.long)) %>%
  group_by(sfis) %>%
  summarise(
...
# TODO ask ALEC regarding postal codes with two SLI == 1
pccfSlim %>%
...
num_unmatched_students <- sum(is.na(schoolCatchment$student.lat))
print(paste0('There are ', num_unmatched_students, ' students without a match between postal code and lat/long'))
...
temp_dist <- schoolCatchment %>%
  filter(panel == 'Elementary', dsb.index == 1)
...
rm(schoolCatchment, schoolCatchmentDistPercent, percentileList)
```
