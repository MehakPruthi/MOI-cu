...
courthouse_hours <- read_csv("input/mag/MAG_CRT_HRS_DB.csv") %>% 
  ...
  mutate(city = capitalize(tolower(city)),
...
courthouse_asset <- courthouse_asset %>%
  ...
  filter(!(grepl("Barrie", name) & courtrooms == 0))

print(paste0("There is a difference between the number of courthouses in the asset data (", nrow(courthouse_asset), "), and the number of courthouses in the historical data (", nrow(courthouse_hours_wide), "). Thus, certain assumptions must be made to combine "))
