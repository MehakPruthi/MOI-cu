...
schoolCapSlim <- schoolCap %>%
  select(Sch_yr, DSB_Index, BoardName, Panel, SFIS = `SFIS_ID`, schoolName = `FacilityName`, ADE, OTG, 
         schoolLat = `Latitude`, schoolLong = `longitude`, Status, dataSetID) %>%
  mutate(utilization = ADE / OTG) %>%
  filter(Status == "Open", dataSetID == 2, Sch_yr == 2011) %>%
  arrange(desc(utilization), Sch_yr, SFIS)

# Combine with the forecast data
schoolForecastSlim <- schoolForecast %>%
  select(Sch_yr = `SCH_YR`, SFIS, BSID, DSB_Index = `DSBINDEX`, schoolName = `School.Name`, ADE_Forecast = `ADE`, BSID) %>%
  filter(Sch_yr == 2017)
```
...
schoolPostSlim <- schoolPost %>%
  select(Sch_YR, BSID, DSB_INDEX = `dsbindex`, schoolName = `School.Name`, studentPostalCode = `Student.Postal.Code`, Enrolment) %>%
  filter(Sch_YR == 2011) %>%
  filter(str_length(studentPostalCode) == 6) %>%
  mutate(Enrolment = as.integer(Enrolment), student_postal_id = row_number())

#Step 2: Obtain school long and lat
schoolSFIS <- left_join(schoolCapSlim, schoolForecastSlim, by = c('SFIS')) %>%
  group_by(SFIS) %>%
  summarise_all(
    funs(first)
  ) %>%
  rename(schoolName = schoolName.x, DSB_Index = DSB_Index.x) %>%
  arrange(SFIS)
...
schoolCatchment <- schoolSFIS %>%
  select(SFIS, BSID, schoolName, schoolLat, schoolLong, DSB_Index, Panel) %>%
  left_join(select(schoolPostSlim, BSID, studentPostalCode, Enrolment, student_postal_id), by = c('BSID' = 'BSID')) %>%
  left_join(select(pccfSlim, studentLat = LAT, studentLong = LONG, PostalCode), by = c('studentPostalCode' = 'PostalCode')) %>%
  drop_na(studentLat, studentLong, schoolLat, schoolLong)

#Step 4: Calculate straight-line home-to-school distances by SFIS
...
schoolCatchmentDistPercent <- schoolCatchmentDist %>%
  ...
  group_by(SFIS, schoolName, schoolLat, schoolLong) %>%
...
for (catchment_dist in percentileList){
  schoolCatchmentDist %>%
    left_join(select(schoolCatchmentDistPercent, SFIS, percDist, perc_value), by = c('SFIS' = 'SFIS')) %>%
...
```{r}
student_travel <- readRDS(file = 'output/student_travel_0.9.rds') %>%
  as.tibble() %>%
  group_by(schoolName) %>%
  summarise(distance = first(percDist),
            schoolLat = first(schoolLat),
            schoolLong = first(schoolLong))
