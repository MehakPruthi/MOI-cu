...
treso_travel_time_prep <- function(travel_time_skim_square) {
  
  # Read in TRESO shapefile
...
# TRESO time skim
travel_time_skim_square_2011 <- read_csv("input/treso/mf224.csv.gz")
travel_time_skim_square_2016 <- read_csv("input/treso/mf381.csv")

# Apply cleaning
travel_time_2011 <- treso_travel_time_prep(travel_time_skim_square_2011)
saveRDS(travel_time_2011, "cache/treso/treso_travel_time_2011.rds")

travel_time_2016 <- treso_travel_time_prep(travel_time_skim_square_2016)
saveRDS(travel_time_2016, "cache/treso/treso_travel_time_2016.rds")
```

## TRESO Travel Distance Skim Prep

- The intra-zonal travel distance from TRESO is 0, so it is assumed that intra-zonal travel distance is half of the square root of its area.

```{r treso travel distance prep, echo=FALSE, eval=FALSE}

treso_travel_distance_prep <- function(travel_distance_skim_square) {
  # Read in TRESO shapefile
  treso_shp <- readOGR(dsn = "input/treso", layer = "TRESO_Zones_SocioData_Gatineau_LCC")
  
  distance_skim <- travel_distance_skim_square %>%
    reshape2::melt(id = c("p/q/[val]")) %>%
    rename(orig = "p/q/[val]", dest = variable) %>%
    mutate(orig = as.numeric(orig), dest = as.numeric(levels(dest))[dest]) %>%
    filter(orig %in% treso_shp@data$Treso_ID) %>%
  ...
  # Create intra-zonal distance skims
  treso_intra_zonal_distance <- treso_shp@data %>%
    select(treso.id = Treso_ID, area = Area) %>%
    mutate(area.km = area / 1000000) %>%
    mutate(distance = sqrt(area.km) / 2) %>% 
    select(treso.id, distance)
  
  # Update intra-zonal distance skim and arrange the indecies
  distance_skim1 <- distance_skim %>% 
    left_join(treso_intra_zonal_distance, by=c("orig"="treso.id")) %>% 
    mutate(value = ifelse((value == 0.0 & orig == dest), distance, value)) %>% 
    select(treso.id.por = orig, treso.id.pos = dest, value) %>%
    arrange(treso.id.por, treso.id.pos)
  
  return(distance_skim1)
}

# TRESO distance skim
travel_distance_skim_square_2016 <- read_csv("input/treso/mf382.csv")

# Apply distance cleaning
treso_distance_2016 <- treso_travel_distance_prep(travel_distance_skim_square_2016)
saveRDS(treso_distance_2016, "cache/treso/treso_distance_2016.rds")
```
...
## TRESO Shapefile Centroids

```{r treso centroids, eval=FALSE, include=FALSE}
# Read in TRESO shapefile
treso_shp <- readOGR(dsn = "input/treso", layer = "TRESO_Zones_SocioData_Gatineau_LCC")

# Produce the TRESO zone centroids
centroids <- SpatialPointsDataFrame(gCentroid(treso_shp, byid=TRUE), 
                                      treso_shp@data, match.ID=FALSE) %>% 
  spTransform(CRS('+proj=longlat +datum=WGS84'))

# Find the nearest PCCF lat/long to obtain postal code
pccfSlim_sp <- pccfSlim
coordinates(pccfSlim_sp) <- c("long", "lat")

centroids_latlong <- centroids %>% 
  as.tibble() %>% 
  select(treso_id = Treso_ID, long = x, lat = y) %>% 
  left_join(treso_zone_def, by=c("treso_id"))

nn <- get.knnx(coordinates(pccfSlim_sp), coordinates(centroids), k = 1)
nearest_postal_code <- nn$nn.index %>%
  as.tibble()

pccfSlim_df <- as.tibble(pccfSlim) %>%
  mutate(index = row_number())

nearest_treso_centroid_postal_code <- cbind(nearest_postal_code, centroids_latlong) %>%
  left_join(select(pccfSlim_df, index, postal.code), by=c("V1" = "index")) %>% 
  drop_na(mof_region) %>% 
  select(treso_id, lat, long, postal.code)

write_csv(nearest_treso_centroid_postal_code, "cache/treso/treso_centroids.csv")
```
##
...
school_treso_master <- school_treso_data %>%
  ...
  mutate(capacity.portable = ifelse(panel == 'Elementary', 
                                    num.units.calculated * ELE_STUDENTS_UNIT_CLASS, 
                                    num.units.calculated * SEC_STUDENTS_UNIT_CLASS)) %>%
  rename(portable.units = num.units.calculated) %>% 
  left_join(schoolAssetsSlim, by = 'sfis')

# Export to RDS
saveRDS(school_treso_master, 'cache/edu/school_treso_master.rds')
```

### Map DSB to TRESO Zones

```{r}
schoolBoardTypes <- read_csv('input/edu/school_board_types.csv')

# Summarize the unique DSB Index for every CD in Ontario
dsb_mapping <- school_treso_master <- readRDS('cache/edu/school_treso_master.rds') %>%
  filter(year == 2017, !is.na(cduid), dsb.index != 99) %>%
  select(year, dsb.index, board.name, panel, cduid) %>% 
  group_by(dsb.index, board.name, panel, cduid) %>% 
  summarise(num.schools = n()) %>% 
  ungroup() %>% 
  left_join(schoolBoardTypes, by=c("dsb.index"="dsb"))

print(paste0("there are ", length(unique(dsb_mapping$dsb.index)), " DSB Index in this dataset"))

# In most CDs, there are 4 boards. However, there are CD where there is an overlap of boards
dsb_overlap <- dsb_mapping %>% 
  group_by(panel, cduid) %>% 
  summarise(num.boards = n()) %>% 
  filter(num.boards > 4) %>% 
  nrow()

print(paste0("there are ", dsb_overlap, " CDs where there are multiple DSBs for a panel"))

# If there are two DSB of the same board type in a CD, take the DSB with most schools
dsb_mapping <- dsb_mapping %>% 
  group_by(cduid, panel, board_type) %>% 
  filter(num.schools == max(num.schools)) %>% 
  ungroup()

# Check that all CD has 4 or less unique boards
dsb_overlap <- dsb_mapping %>% 
  group_by(cduid, panel, board_type) %>% 
  mutate(num.boards = n()) %>% 
  filter(num.boards > 4) %>% 
...
dsb_mapping %>% 
  select(cduid, panel, board_type_name, dsb.index, board.name) %>% 
  saveRDS("cache/edu/dsb_mapping.RDS")
```
...
# TODO: Look at all years of student postal code data.
