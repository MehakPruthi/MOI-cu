...
ontario <- read_csv('input/ontario.csv') %>%
  select(treso_zone, cduid)
cduid_definition <- read_csv('input/treso_zone_system.csv') %>%
  group_by(cduid) %>%
  summarise(
    cdname = first(cdname),
    area = first(area),
...
ontario <- left_join(ontario, cduid_definition, by='cduid') %>%
  rename(treso_id = treso_zone) %>%
  write_csv('input/treso_zone_system2.csv')
```
...
saveRDS(schoolSFIS, file = 'output/school_sfis_2017.rds')
```
