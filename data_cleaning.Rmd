...
# Create a table called schoolLoc to house master list for school geocoordinates
schoolLoc <- schoolCap %>%
  select(year = Sch_yr, 
         dsb.index = DSB_Index, 
         panel = Panel, 
...
schoolLocUpdated <- schoolLoc %>%
  left_join(select(missingCoords, SFIS_ID, altLat = finalLatitude, altLon = finalLongitude), by = c('sfis' = 'SFIS_ID')) %>%
...
school_xy <- create_school_xy_simple(schoolLocClean)
...
school_treso_master_hist <- schoolTRESOcsd %>%
...
# Simplify school forecast data (ADE and OTG)
schoolForecastSimple <- schoolForecast %>%
  select(year = SCH_YR, 
         ...
         longitude,
         cduid = CDUID) %>% 
  mutate(portable.units = 0, capacity.portable = 0) %>% 
...
school_recent_data <- schoolLocClean %>%
  group_by(sfis) %>%
  filter(year == max(year), status != 'Closed') %>%
  ungroup()
  
schoolForecastSimpleJoin <- schoolForecastSimple %>% 
  left_join(select(school_recent_data, sfis, board_type_name, school.lat, school.long, otg), by = c('sfis')) %>% 
...
schoolForecastSimpleJoin <- schoolForecastSimpleJoin %>%
  left_join(distinct(school_treso_master_hist, sfis, treso.id.pos, csduid), by = c('sfis')) %>%
  mutate(area.m2 = 0)

schoolForecastSimpleJoin %>%
  filter(sfis == 24623)

schoolForecastSimpleJoin %>%
  anti_join(select(school_treso_master_hist, sfis, treso.id.pos, csduid), by = ('sfis'))

# Add simplified school forecast data to school_treso_master
names(school_treso_master_hist)
school_treso_master <- rbind(school_treso_master_hist, schoolForecastSimple) 
