...
saveRDS(ALC_idcrosswalk, file = 'cache/moh/ALC_crosswalk.rds')
...
hospitallocations <- read_csv('input/moh/dbo.MOH_Hospital_Locations.csv') %>% 
  ...
  # left_join(select(ALC_idcrosswalk, siteid, ALCsite_id, type), by = c('siteid', 'caretype' = 'type')) %>% 
  distinct()
...
saveRDS(ALC_madd_raw, file = 'cache/moh/ALC_madd_raw.rds')

ALC_lhin <- ALC_madd_raw %>% 
  filter(number < 15)

saveRDS(ALC_lhin, file = 'cache/moh/ALC_lhin.rds')

ALC_hosp_vol <- ALC_madd_raw %>% 
  filter(number > 15)

saveRDS(ALC_hosp_vol, file = 'cache/moh/ALC_hosp_vol.rds')
...
hospital_ALC_master <- ALC_idlinkage %>% 
  filter(!is.na(type)) %>% 
  group_by(siteid, facilityid, type, name) %>% 
  distinct()
```
