...
hospitallocations <- read_csv('input/health/dbo.MOH_Hospital_Locations.csv') %>% 
  ...
  left_join(select(HBAMprojected, siteid, caretype), by = c('siteid', 'caretype')) %>% 
  distinct()
...
saveRDS(HBAMprojected, file = 'cache/hospitals/hbamprojected.rds')

# Projected HBAM Master Dataset
HBAMprojected_master <- HBAMprojected %>% 
  group_by(year, siteid, caretype, agegroup) %>% 
  summarise(totalcases = sum(totalcases),
            los = sum(lengthofstay)) %>% 
  mutate(beddays = (totalcases*los)) %>% 
  left_join(hospitallocations, by = c('siteid', 'caretype')) %>% 
  filter(!is.na(pcode))

saveRDS(HBAMprojected_master, file = 'cache/hospitals/hbamprojected_master.rds')

## Hosp Site IDs that unmatched between projected HBAM and Hosp PAI (total from 2016-2026 = ~10.3m cases, averaging ~1m to ~900k cases p.a.)
