...
hospitaldischarges <- read_csv('input/health/cap.MOH_Discharges.csv') %>%
  ...
  mutate(agegroup = replace(agegroup, agegroup == 0, 1),
         agegroup = replace(agegroup, agegroup == 7, 6))
...
```
##HBAM Data
  - Join HBAM historical data with TRESO CSD 
  - Join TRESO MOF Pop data with HBAM for Origin caseloads
```{r}
...
# create a column named anything and just fill it with a constant of anything "x"
# do the same on c_total 
# join on that column

# join p-total into c-total and then new column in c-total will be the division
...
caserate_ct_agegroup <- left_join(cases_ct_agegroup, pop_agegroup, by = 'index_link') %>% 
    mutate(caserate_tot = cases_tot/pop_tot,
         caserate_ag1 = cases_ag1/pop_ag1,
         caserate_ag2 = cases_ag2/pop_ag2,
         caserate_ag3 = cases_ag3/pop_ag3,
