...
courthouse_asset_overlay <- create_overlay(courthouse_asset_xy, treso_shp, type = "court") %>% 
  ...
  mutate(building.type = replace(building.type, building.type %in% c("Office", "Court Support Office"), "Base Court")) %>% 
  select(-courtrooms) %>% 
...
multiple_courthouse_asset <- courthouse_asset_overlay %>% 
  mutate_at(vars(contains("number_of_courtrooms")), funs(ifelse(is.na(.), 1, .))) %>% 
  group_by(cdname, bid, name, lat, long, treso.id.pos, building.type) %>%
  summarise_at(vars(contains("number_of_courtrooms")), sum) %>% 
