...
```{r}
# Bed Census Data Initial Data Preparation
dailybedcensus1 <- dailybedcensus %>% 
...
# Hospital Discharge Data Preparation
hospitaldischarges1 <- hospitaldischarges %>%
  replace_na(list(Discharges = 0), list(Total_Days = 0)) %>% 
...
hospitallocations1 <- hospitallocations %>%
...
## Manipulating Input Data Sets for Cleaning Stage

```{r postal code and lat/long prep, include=FALSE, eval=TRUE}
...
hospitallocations <- hospitallocations1 %>% 
...
#output cache hospital location dataset: hospitallocations for use in hospital.rmd
saveRDS(hospitallocations, file = 'cache/moh/hospitallocations.rds')

```

```{r manipulate historical & projected HBAM data, include=FALSE, eval=TRUE}
# HBAM Historical
HBAMhistorical1 <- HBAMhistorical %>% 
  select(year = Year, 
...
HBAMhistorical_master <- HBAMhistorical1 %>% 
  mutate(caretype = replace(caretype, caretype == "ER", "AM"),
         caretype = replace(caretype, caretype == "DS", "AM")) %>% 
  left_join(distinct(hospitallocations, id, name, facilityid, pcode, lat, long), by = c('id'))

#output cache HBAM historical master dataset: HBAMhistorical_master for use hospitals.rmd
saveRDS(HBAMhistorical_master, file = 'cache/moh/hbamhistorical_master.rds')

# HBAM Projected
HBAMprojected1 <- HBAMprojected %>% 
  select(year = Year, 
         siteid = Hospital_Site_ID, 
         hospitalname = Hospital_Site_Name,
         caretype = Care_Type,
...
HBAMprojected_master <- HBAMprojected1 %>% 
...
#output cache HBAM projected master dataset: HBAMprojected_master for use in hospitals.rmd
...
```{r ALC & ltc dataset prep, include=FALSE, eval=TRUE}

#ALC Datasets
#CCO Linkage Table
# DO WE STILL NEED THIS TABLE? IF SO, FIX CODE

ALC_idcrosswalk1 <- ALC_idcrosswalk %>%
  ...
  distinct() %>% 
  left_join(distinct(hospitallocations, id, pcode), by = 'id') %>% 
  mutate(pcode.clean = gsub("\\s+", "", pcode)) %>%
...
saveRDS(ALC_idcrosswalk1, file = 'cache/moh/ALC_crosswalk.rds')

ALCannualvolume1 <- ALCannualvolume %>%
  select(year = Year,
         siteid = SiteNumber,
         casetype = CaseType,
...
saveRDS(ALC_madd_raw1, file = 'cache/moh/ALC_madd_raw.rds')
...
saveRDS(ltc_homes1, file = 'cache/moh/ltc_homes.rds')
...
dailybedcensus2 <- dailybedcensus1 %>%
  filter(year == 2017) %>% 
  group_by(censusdate) %>% 
  mutate(sumbeds = sum()) 
  group_by(year) %>%
...
beddaysobserved <- dailybedcensus2 %>% 
  group_by(siteid,
           type,
           year) %>% 
...
annualizationfactor <- dailybedcensus2 %>% 
  group_by(siteid,
           type,
           year) %>% 
...
#Physcial Capacity
Hospitalphyscap <- dailybedcensus2 %>% 
...
three_digit_ALC <- ALCannualvolume1 %>%
  filter(siteid < 999,
         siteid > 99) %>% 
  select (-casetype,
          -bedtype) 
...
four_digit_ALC <- ALCannualvolume1 %>% 
  filter(siteid > 999) %>% 
  select(-casetype,
          -bedtype)
...
# Referenced Code - Not Used/Potential Future Use
##Health Data Import
### SQL Health Data Import
```{r sql health data import, include=FALSE}
...
### Unused Import Files
```{r unused health import, include=FALSE}
#LHIN Data
...
##Unused Data Cleaning Code
### Health Data Cleaning
```{r unused health data cleaning}

# TODO: Test code for comparing ALC siteID to regular siteID linkages - ##Move to end
# a <- 3949
...
## Output Tables/Files Not Used
### Health Unused Outputs
```{r health outputs not used}
