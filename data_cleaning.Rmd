...
# HBAM Historical
HBAMhistorical <- read_csv('input/moh/cap.MOH_HBAM_HIS_2018.csv')

# HBAM Projected
HBAMprojected <- read_csv('input/moh/cap.MOH_HBAM_Proj_2018.csv')
...
HBAMhistoricalprep <- HBAMhistorical %>% 
  select(year = Year, 
         ...
         orig_csd = CSD,
         LOS = Length_of_Stay,
         weighted_cases = Number_of_Weighted_Cases) %>% 
  # filter(year == 2016) %>% 
  # select(-year) %>% 
...
```{r}
#Count Daily Entries by Year: 2017:294 / 2018: 365
dailybedcensusprepagg <- dailybedcensusprep %>%
  #replace year with Calendar year instead of fiscal year 
  mutate(year = format(as.Date(censusdate, format="%d/%m/%Y"),"%Y")) %>%
  filter (year == 2018) %>%  #use 2018-calendar for daily bed data, which matches HBAM 2018 data
  group_by(censusdate) %>% 
  mutate(sumbeds = sum())
  # group_by(year) %>% 
  # distinct(censusdate) %>% 
