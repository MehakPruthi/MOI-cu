...
HBAMhistorical_master <- HBAMhistorical %>% 
  mutate(caretype = replace(caretype, caretype == "ER", "AM"),
         caretype = replace(caretype, caretype == "DS", "AM")) %>% 
  group_by(year, siteid, caretype, agegroup, csd) %>% 
...
#CIHI Data
CIHIhospitalbeds <- read_csv('input/health/cap.CIHI_Hospital_Beds.csv') %>% 
  select(year = Year,
         prov = Province,
         name = 'Hospital Name',
...
saveRDS(CIHIhospitalbeds, file = 'cache/hospitals/CIHI_hospitalbeds.rds')
...
#Import complete TRESO Population Dataset
treso_zone_system <- read_csv('input/treso/treso_zone_system.csv') %>% 
  select(-mof_region,
         -area)

treso_population_moh <- readRDS('input/health/treso_population_moh.rds')
...
#Total Bed Counts by Site ID
num_beds <- dailybedcensus %>% 
  mutate(siteid = as.numeric(siteid)) %>% 
  group_by(siteid, type, year) %>% 
  summarise(maxstaffedbeds = max(staffedbeds, na.rm = TRUE)) %>% 
  mutate(maxstaffedbeds = replace(maxstaffedbeds, maxstaffedbeds == -Inf, 0)) #-Inf occuring as a result of NA entries calculated within max() function, as it will result in either Inf/-Inf

saveRDS(num_beds, file = 'cache/hospitals/num_beds.rds')
...
HBAMhistorical_csd <- HBAMhistorical_master %>% 
  select(year,
         ...
         csd) %>% 
  ungroup() %>% 
  group_by(year, csd, caretype, agegroup) %>% 
  summarise(totalcases = sum(totalcases))
...
csd_caserates <- left_join(HBAMhistorical_csd, csd_pop_long, by = c('year', 'csd' = 'csduid', 'agegroup' = 'age_group')) %>% 
  mutate(caserate = totalcases/population) %>% 
  select(-totalcases, 
         -population,
         csd_caserates = caserate) %>% 
  mutate(cd = as.numeric(substring(csd, 0,4)))

#Calculte Mismatched CSDs
mismatched_csd <- csd_caserates %>% 
  filter(is.na(population)) %>% 
  summarise(sum(totalcases)) #total of 57,951 mismatched cases from 89 CSDs
...
HBAMhistorical_cd <- HBAMhistorical_master %>% 
  ...
  select(year, 
         ...
         cd) %>% 
  ungroup() %>% 
  group_by(year, cd, caretype, agegroup) %>% 
  summarise(totalcases = sum(totalcases))
...
cd_caserates <- left_join(HBAMhistorical_cd, cd_pop_long, by = c('year', 'cd', 'agegroup'='age_group')) %>% 
  ungroup() %>% 
  mutate(caserate = totalcases/population,
         cd = as.numeric(cd))
...
caserates_comp <- full_join(csd_caserates, cd_caserates, by = c('year', 'agegroup', 'caretype', 'cd'))

saveRDS(caserates_comp, file = 'cache/hospitals/caserates_comp.rds')
...
csd_caserates_plot <- csd_caserates %>% 
  ...
  summarise(avg_cr = mean(csd_caserates))

csd_caserates_plot <- ungroup(csd_caserates_plot)
