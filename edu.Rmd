...
pos_overfill <- school_forecast %>% 
  ...
  select(treso.id.pos, overfill.ade) %>% 
  group_by(treso.id.pos) %>% 
  summarise(overfill.ade = sum(overfill.ade)) %>% 
  right_join(pos_full, by=c("treso.id.pos" = "dest")) %>% 
...
prop_matrix_balanced

# Summarize a POR vector of origins of schools over OTG Threshold - por_

# Scale t
```


# Step 5 - Create final pos vector using simulated ADE
...
school_forecast <- filter(school_master, year == 2017) %>% 
  ...
  select(sfis, school.name, otg, ade, simulated.ade.2017, simulated.ade.20xx) %>% 
  select(treso.id.pos, sfis, school.name, otg, ade, simulated.ade)
