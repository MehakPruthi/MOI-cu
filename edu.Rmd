...
schoolz <- school_master %>% 
      bind_rows(new_school_slim)

tripz <- trip_list %>% 
  anti_join(distinct(schoolz, treso.id.pos), by = c('treso.id.pos')) %>% 
  filter(round(trips, 0) > 0)
...
school_summary_20xx %>% 
  filter(sfis > 99999) %>% 
  distinct(sfis) %>% 
  anti_join(select(new_school, sfis), by = c('sfis'))
```
...
schools_consolidated_closed <- existing_school_utilization %>% 
