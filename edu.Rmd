...
result <- distribute_students_to_schools(school_base, new_school=NULL, por, pos_full, prop_matrix, 
                                         is_weight_continuous=TRUE)
school_summary_2017 <- result[[1]]
...
school_forecast_final <- calculate_delta_between_simulated_scenarios(school_base, school_summary_2017, school_summary_20xx,
                                                               new_school, schools_consolidated_closed)
...
school_summary_20xx_final <- forecast_school_ade(prop_matrix, trip_list, school_master, eqao_2017, new_school)[[1]]
...
# Cache FINAL results temporarily
saveRDS(school_base, "cache/edu/school_base_final.rds")
saveRDS(trip_list, "cache/edu/trip_list_final.rds")
saveRDS(por_additional, "cache/edu/por_additional_final.rds")
saveRDS(school_forecast_final, "cache/edu/school_forecast_final_final.rds")
saveRDS(school_summary_20xx_final, "cache/edu/school_summary_20xx_final_final.rds")
...
saveRDS(por, "cache/edu/por_final.rds")
saveRDS(pos, "cache/edu/pos_final.rds")
saveRDS(schools_consolidated_closed, "cache/edu/schools_consolidated_closed_final.rds")
#saveRDS(schools_consolidated_final, "cache/edu/schools_consolidated_final.rds")

school_base <- readRDS("cache/edu/school_base_final.rds")
trip_list <- readRDS("cache/edu/trip_list_final.rds")
por_additional <- readRDS("cache/edu/por_additional_final.rds")
school_forecast_final <- readRDS("cache/edu/school_forecast_final_final.rds")
school_summary_20xx_final <- readRDS("cache/edu/school_summary_20xx_final_final.rds")
por <- readRDS("cache/edu/por_final.rds")
pos <- readRDS("cache/edu/pos_final.rds")
school_summary_20xx <- readRDS(school_summary_20xx, "cache/edu/school_summary_20xx.rds")
school_summary_2017 <- readRDS(school_summary_2017, "cache/edu/school_summary_2017.rds")
schools_consolidated_closed <- readRDS("cache/edu/schools_consolidated_closed_final.rds")
#schools_consolidated_final <- readRDS("cache/edu/schools_consolidated_final.rds")

# Cache results temporarily
# saveRDS(school_master, "cache/edu/school_master_testing.rds")
# saveRDS(trip_list, "cache/edu/trip_list_testing.rds")
# saveRDS(por_additional, "cache/edu/por_additional_testing.rds")
# saveRDS(school_forecast_final, "cache/edu/school_forecast_final_testing.rds")
# saveRDS(por_full, "cache/edu/por_full_testing.rds")
# saveRDS(pos_full, "cache/edu/pos_full_testing.rds")

# school_master <- readRDS("cache/edu/school_master_testing.rds")
# trip_list <- readRDS("cache/edu/trip_list_testing.rds")
# por_additional <- readRDS("cache/edu/por_additional_testing.rds")
# school_forecast_final <- readRDS("cache/edu/school_forecast_final_testing.rds")
# por_full <- readRDS("cache/edu/por_full_testing.rds")
# pos_full <- readRDS("cache/edu/pos_full_testing.rds")
...
# Run Steps 3 to get adjusted simulated ADE for each school (stored under school_forecast dataframe)
school_forecast <- calculate_delta_between_simulated_scenarios(school_base, school_summary_2017, school_summary_20xx,
                                                               new_school_iteration, schools_consolidated_closed)
...
school_forecast <- school_forecast %>% 
  ...
  mutate(existing.school.underused.flag = ifelse((simulated.utilization < CONSOLIDATION_THRESHOLD) &
                                                 (sfis <= 100000), 1, 0)) 

# Temp dataframe containing schools not consolidated as per step above
schools_kept <- school_forecast %>% 
  filter(existing.school.underused.flag == 0)

# Temp dataframe containing schools closed by scenario year (as projected in EDU data) and schools consolidated by decision-making layer
schools_consolidated <- school_forecast %>% 
  filter(existing.school.underused.flag == 1) 
...
# Run Steps 3 to get adjusted simulated ADE for each school (stored under school_forecast dataframe)
school_forecast <- calculate_delta_between_simulated_scenarios(school_master_base, school_summary_2017, school_summary_20xx,
                                                               new_school_iteration, schools_consolidated_closed, panel_id, board_id)
...
existing_otg <- school_master %>% 
  select(otg) %>% 
  sum()

# Extract enrolment for the board-type/panel in question  
scenario_enrolment <- treso_forecast_population_board %>% 
  filter(board_type_name == board_id, panel == panel_id) %>% 
  select(potential.enrolment) %>% 
  sum()

# Calcualte OTG needed based on user-defined target utilization
needed_otg <- scenario_enrolment / target_utilization
new_otg_reqd <- needed_otg - existing_otg
...
new_school <- new_school %>% 
  ...
  mutate(sum.otg = sum(otg),
         revised.otg = round(new_otg_reqd / sum.otg * otg, 0)) %>% 
  select(-otg) %>% 
...
treso_forecast_population_2017 <- readRDS("input/edu/treso_population_edu.rds") %>% 
  select(treso.id.por = treso_zone, panel = age.group, csduid, 
         potential.enrolment = paste0("population.", 2017)) %>%
  filter(panel != "other")
treso_forecast_population_2025 <- readRDS("input/edu/treso_population_edu.rds") %>% 
  select(treso.id.por = treso_zone, panel = age.group, csduid, 
         potential.enrolment = paste0("population.", 2025)) %>%
  filter(panel != "other")

# Apply the board sampling probabilities to the student population from TRESO
treso_forecast_population_board_2017 <- apply_sampling_to_population(
  forecast_population = treso_forecast_population_2017,
  board_type_sample = board_type_sample_spread) %>% 
  filter(panel == panel_id, board_type_name == board_id)

treso_forecast_population_board_2025 <- apply_sampling_to_population(
  forecast_population = treso_forecast_population_2025,
  board_type_sample = board_type_sample_spread)  %>% 
  filter(panel == panel_id, board_type_name == board_id)
