...
if (FORECAST_METHOD == "EDU") {
  ...
  print(paste0("Scaling TRESO forecast from - ", round(treso_forecast_sum, 0), " to EDU forecast of - ", round(edu_forecast, 0)))
...
trip_length_check <- cbind(trip_list, select(treso_travel_time, value)) %>%
  ...
  ungroup() %>% 
  # Filter out any home (origin)
  filter(total.por.trips > 10)

# school_20xx_backup <- school_20xx
# school_20xx <- school_20xx_backup

school_20xx <- school_20xx %>% 
  left_join(select(trip_length_check, treso.id.por, total.por.trips), by = c('treso.id.pos' = 'treso.id.por')) %>% 
  left_join(select(trip_length_check, treso.id.pos, travel.time), by = c('treso.id.pos')) %>% 
  # Replace is.consolidated flag with 0 if total.por.trips or travel.time is not NA (i.e., if that treso zone is in trip_length_check)
  mutate(is.consolidated = ifelse(is.na(total.por.trips), is.consolidated, 0),
         is.consolidated = ifelse(is.na(travel.time), is.consolidated, 0)) %>% 
  select(-total.por.trips, -travel.time) %>% 
  distinct()
