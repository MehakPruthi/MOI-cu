...
- *TOOD:* there are duplicated schools in the school_tb, check where that is coming from
...
school_tb_filtered %>%
  ...
  select(school_name.x, n_pop_sum, ade_2011)
...
linreg <- school_tb_filtered %>%
  ...
  cbind(select(school_tb_filtered, sfis, school_name.x))
