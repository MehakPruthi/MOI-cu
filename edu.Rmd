...
min_tt_threshold <- 1500 # TODO:Replace this with value provided by user through the visualization interface?
...
consolidation_threshold <- 0.40 # TODO:Replace this with value provided by user through the visualization interface?

# Set a base year for which to pull existing schools
base_year <- 2017 # TODO:Replace this with value provided by user through the visualization interface?
...
year_id <- 2017 # TODO: Replace this hard-coded value with user-input value from visualization
...
number_of_zero_simulated_schools <- school_master %>% 
  filter(panel == 'Elementary', board_type_name == 'English Public', year == 2017) %>% 
...
df_list <- edu_dm(travel_time_skim, trip_list, treso_zone_def, travel_time_threshold_factor, zone_proximity_threshold, min_tt_threshold)
build_df <- df_list[[1]] %>% 
  rename(treso.id.pos = zone) %>% 
  mutate(panel = panel_id,
         board_type_name = board_id,
         year = year_id)
# Remove zones which are to have schools built from the proximity_df (where a zone appears in both dataframes, the build_df is 'true')
...
7.	Run model capacity unconstrained to see how many students choose each school, new and existing
```{r}
# Create list of schools containing existing schools and proposed new schools

# Filter list of existing schools for existing schools (most recent year) of interest
school_master <- school_master %>% 
  filter(year == base_year)

#######
# TODO: DON'T PLAN TO 'EXPAND' EXISTING SCHOOLS, SIMPLY BUILD OR NOT IN ANY ZONE NEEDING NEW PLACES
#######

build_df_schools <- build_df %>% 
  mutate(build.priority = 1:n(),
         sfis.temp = as.numeric(paste0(treso.id.pos, '000'))) %>% 
  left_join(select(school_master, treso.id.pos, sfis, otg), by = c('treso.id.pos')) %>% 
  # Set any new potential school to have OTG of 1; set any identified build zone which already has schools to 
  mutate(otg = replace_na(otg, 1),
         expand.school = max(1 - otg, 0)
  
school_master <- school_master %>% 
  full_join(build_df_schools, by = c('sfis', 'treso.id.pos')) %>% 
  mutate(replace_na(build.priority, 0)) %>% 
  mutate(replace_na(expand.school, 0)) %>% 
  # Choose up to one school in each TRESO zone to expand if expansion is warranted
  # Choose smallest school to expand
  arrange(sfis, otg) %>% 
  group_by(sfis) %>% 
  mutate(school.count.per.treso = n(),
         expand.school = ifelse(expand.school == 1 & school.count.per.treso == 1, 1, 0)) %>% 
  ungroup()
  
# Rerun Steps 2, 3, 4 in gravity model
```


8.	Delete any new schools with ADE < minimum OTG required for new school

9.	Reassign ADE from deleted schools based on TRESO origin distribution propensity matrix

10.	IF USER WISHES TO CONSOLIDATE EXISTING SCHOOLS: 
...
11.	Set OTG at new schools to be a function of total ADE determined in Step 10

12.	Rerun model capacity constrained to forecast ADE by school for all schools (new and existing)

13.	Compare results of Step 12 to Step 3b to determine incremental ADE by school

14.	Add Results of Step 13 to actual 2017 (i.e., observed) ADE by school to determine final ADE 

15. If travel times > acceptable travel times for any zone, see if an existing school has been consolidated from the zone, in which case, reinstate
...
```{r}
treso_forecast_population <- readRDS("input/edu/treso_population_edu.rds") %>% 
  select(treso.id.por = treso_zone, panel = age.group, csduid, enrolment = population.2027) %>%
  filter(panel != "other")
...
write_csv(treso_forecast_population_board, "input/edu/treso_population_edu_2027.csv")
