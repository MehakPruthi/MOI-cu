...
simulated_compare <- readRDS("output/trip_list_schools.rds") %>%
  ...
  group_by(sfis, school.name.list, dsb.index.ist) %>%
  summarise(sim.enrol.sum = sum(enrol.value)) %>%
  left_join(select(school_sfis_2017, sfis, ade), by = "sfis") %>%
  rename(ade_2017 = ade) %>%
  mutate(ade_2017_to_sim_diff = ade_2017 - sim.enrol.sum)%>%
  mutate(perc_ade_diff = (sim.enrol.sum - ade_2017)/ade_2017) %>%
