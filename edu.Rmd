...
Eight segments are completed, one for each panel + board_type combination

####- English Public Primary:
...
observed_trips_EPE <- readRDS("output/observed_por_pos_90.rds") %>%
  # Drop NAs in POR, it didn't overlay on the TRESO shapefile
  drop_na(treso.id.por) %>%
  left_join(select(school_board_def, dsb, board_type_name), by = c("dsb.index" = "dsb")) %>%
  left_join(select(treso_zone_def, treso_id, area, mof_region), by = c("treso.id.por" = "treso_id")) %>%
...
travel_time_skim_select_EPE <- select(observed_trips_EPE, treso.id.por, treso.id.pos, value)

# Calculate observed average trip travel time
tlfd_obs <- sum(observed_trips_EPE$value * observed_trips_EPE$enrolment) / sum(observed_trips_EPE$enrolment)
print(paste0("For English Public Elementary, the average travel time to be calibrated to is " , tlfd_obs))
  
cost <- travel_time_skim_select_EPE
...
for (j in 1:iterations) {
  ...
  simulated_trips_EPE <- select(observed_trips_EPE, treso.id.por, enrolment, value) %>%
    group_by(treso.id.por) %>%
    summarise(enrolment = sum(enrolment)) %>%
    left_join(t, by = "treso.id.por") %>%
    mutate(enrolment = enrolment * prob_scaled) %>%
    left_join(travel_time_skim_select_EPE, by = c("treso.id.por", "treso.id.pos"))
  
  # Calculate simulated average trip lengths
  tlfd_sim <- sum(simulated_trips_EPE$value * simulated_trips_EPE$enrolment) / sum(simulated_trips_EPE$enrolment)
  ...
  obs_tlfd <- select(observed_trips_EPE, treso.id.por, treso.id.pos, enrolment, value) %>%
    mutate(bin = cut(value, seq(0, max_value, bin_size), labels = seq(bin_size, max_value, bin_size))) %>%
    group_by(bin) %>%
    summarise(enrolment = sum(enrolment)) %>%
    transform(., flag = "obs")
  
  sim_tlfd <- select(simulated_trips_EPE, treso.id.por, treso.id.pos, enrolment, value) %>%
    mutate(bin = cut(value, seq(0, max_value, bin_size), labels = seq(bin_size, max_value, bin_size))) %>%
    group_by(bin) %>%
    summarise(enrolment = sum(enrolment)) %>%
...
generate_tlfd <- function(observed_trips_EPE, simulated_trips_EPE, max_value=85, bin_size=1) {
  obs_tlfd <- select(observed_trips_EPE, treso.id.por, treso.id.pos, enrolment, value) %>%
    mutate(bin = cut(value, seq(0, max_value, bin_size), labels = seq(bin_size, max_value, bin_size))) %>%
    group_by(bin) %>%
    summarise(enrolment = sum(enrolment)) %>%
...
combined_tlfd <- generate_tlfd(observed_trips_EPE, simulated_trips_EPE, 85, 1)
...
saveRDS(simulated_trips_EPE, "cache/simulated_trips_EPE.rds")
```
...
por_total_EPE <- observed_por_pos_90 %>%
  filter(panel == "Elementary", board_type_name == "English Public") %>%
  group_by(treso.id.por) %>%
  summarise(enrolment = sum(enrolment))
...
por_full <- travel_time_skim %>%
  distinct(orig) %>%
  left_join(por_total_EPE, by = c("orig" = "treso.id.por")) %>%
  replace_na(list(enrolment = 0)) %>%
  column_to_rownames(var = "orig") %>%
  data.matrix()

pos_total_EPE <- observed_por_pos_90 %>%
  filter(panel == "Elementary", board_type_name == "English Public") %>%
...
pos_full <- travel_time_skim %>%
  distinct(dest) %>%
  left_join(pos_total_EPE, by = c("dest" = "treso.id.pos")) %>%
  replace_na(list(enrolment = 0)) %>%
...
prop_matrix2_EPE <- matrix_balancing(prop_matrix_EPE, por_full, pos_full, "rows", 1000, 0.001)

```

```{r profiling transpose vs sweep, eval=FALSE, include=FALSE}
sum <- rowSums(prop_matrix_EPE)
sc <- por_full / sum
sc <- as.vector(sc)

Rprof(tmp <- tempfile(),interval = 0.001)
t(t(prop_matrix2_EPE) * sc) # first option
Rprof()
MyTimerTranspose=summaryRprof(tmp)$sampling.time
unlink(tmp)

Rprof(tmp <- tempfile(),interval = 0.001)
sweep(prop_matrix2_EPE, MARGIN = 1, sc, `*`) # sweep option
Rprof()
...
saveRDS(trip_list_EPE, "cache/trip_list_EPE.rds")
```
...
prop_matrix2_EPS <- matrix_balancing(prop_matrix_EPS, por_full, pos_full, 1000, 0.001)

```

```{r profiling transpose vs sweep, eval=FALSE, include=FALSE}
sum <- rowSums(prop_matrix_EPS)
sc <- por_full / sum
sc <- as.vector(sc)

Rprof(tmp <- tempfile(),interval = 0.001)
t(t(prop_matrix2_EPS) * sc) # first option
Rprof()
MyTimerTranspose=summaryRprof(tmp)$sampling.time
unlink(tmp)

Rprof(tmp <- tempfile(),interval = 0.001)
sweep(prop_matrix2_EPS, MARGIN = 1, sc, `*`) # sweep option
Rprof()
MyTimerSweep=summaryRprof(tmp)$sampling.time
unlink(tmp)
```
...
prop_matrix2_ECE <- matrix_balancing(prop_matrix_ECE, por_full, pos_full, 1000, 0.001)

```

```{r profiling transpose vs sweep, eval=FALSE, include=FALSE}
sum <- rowSums(prop_matrix_ECE)
sc <- por_full / sum
sc <- as.vector(sc)

Rprof(tmp <- tempfile(),interval = 0.001)
t(t(prop_matrix2_ECE) * sc) # first option
Rprof()
MyTimerTranspose=summaryRprof(tmp)$sampling.time
unlink(tmp)

Rprof(tmp <- tempfile(),interval = 0.001)
sweep(prop_matrix2_ECE, MARGIN = 1, sc, `*`) # sweep option
Rprof()
MyTimerSweep=summaryRprof(tmp)$sampling.time
unlink(tmp)
```
...
prop_matrix2_ECS <- matrix_balancing(prop_matrix_ECS, por_full, pos_full, 1000, 0.001)

```

```{r profiling transpose vs sweep, eval=FALSE, include=FALSE}
sum <- rowSums(prop_matrix_ECS)
sc <- por_full / sum
sc <- as.vector(sc)

Rprof(tmp <- tempfile(),interval = 0.001)
t(t(prop_matrix2_ECS) * sc) # first option
Rprof()
MyTimerTranspose=summaryRprof(tmp)$sampling.time
unlink(tmp)

Rprof(tmp <- tempfile(),interval = 0.001)
sweep(prop_matrix2_ECS, MARGIN = 1, sc, `*`) # sweep option
Rprof()
MyTimerSweep=summaryRprof(tmp)$sampling.time
unlink(tmp)
```
...
prop_matrix2_FPE <- matrix_balancing(prop_matrix_FPE, por_full, pos_full, 1000, 0.001)

```

```{r profiling transpose vs sweep, eval=FALSE, include=FALSE}
sum <- rowSums(prop_matrix_FPE)
sc <- por_full / sum
sc <- as.vector(sc)

Rprof(tmp <- tempfile(),interval = 0.001)
t(t(prop_matrix2_FPE) * sc) # first option
Rprof()
MyTimerTranspose=summaryRprof(tmp)$sampling.time
unlink(tmp)

Rprof(tmp <- tempfile(),interval = 0.001)
sweep(prop_matrix2_FPE, MARGIN = 1, sc, `*`) # sweep option
Rprof()
MyTimerSweep=summaryRprof(tmp)$sampling.time
unlink(tmp)
```
...
simulated_compare_FPE.data <- data.frame("y" = simulated_compare_FPE$rounded_perc_diff, "x" = simulated_compare_FPE$sim_enrol.diff_to_ade_2017, "mof_region" = simulated_compare_FPE$mof_region)
ggplot(data=simulated_compare_FPE.data) +
  ...
  facet_wrap(~ mof_region, scale = "fixed") +
  theme(legend.position = "none") + 
  labs(x="Enrollment Difference (Count)", y="Percent Difference (%)", subtitle="Elementary - French Public", title="Simulated vs. Observed Demand (2017)") #+
  #geom_point(data = subset(simulated_compare.data, group=="Central"),
             #aes(x=x, y=y),
             #colour = "red")
  #coord_cartesian(ylim=c(-25,50), xlim=c(-10,50))
...
prop_matrix2_FPS <- matrix_balancing(prop_matrix_FPS, por_full, pos_full, 1000, 0.001)

```

```{r profiling transpose vs sweep, eval=FALSE, include=FALSE}
sum <- rowSums(prop_matrix_FPS)
sc <- por_full / sum
sc <- as.vector(sc)

Rprof(tmp <- tempfile(),interval = 0.001)
t(t(prop_matrix2_FPS) * sc) # first option
Rprof()
MyTimerTranspose=summaryRprof(tmp)$sampling.time
unlink(tmp)

Rprof(tmp <- tempfile(),interval = 0.001)
sweep(prop_matrix2_FPS, MARGIN = 1, sc, `*`) # sweep option
Rprof()
MyTimerSweep=summaryRprof(tmp)$sampling.time
unlink(tmp)
```
...
prop_matrix2_FCS <- matrix_balancing(prop_matrix_FCS, por_full, pos_full, 1000, 0.001)

```

```{r profiling transpose vs sweep, eval=FALSE, include=FALSE}
sum <- rowSums(prop_matrix_FCS)
sc <- por_full / sum
sc <- as.vector(sc)

Rprof(tmp <- tempfile(),interval = 0.001)
t(t(prop_matrix2_FCS) * sc) # first option
Rprof()
MyTimerTranspose=summaryRprof(tmp)$sampling.time
unlink(tmp)

Rprof(tmp <- tempfile(),interval = 0.001)
sweep(prop_matrix2_FCS, MARGIN = 1, sc, `*`) # sweep option
Rprof()
MyTimerSweep=summaryRprof(tmp)$sampling.time
unlink(tmp)
```
...
generate_tlfd <- function(observed_trips_FCE, simulated_trips_FCS, max_value=85, bin_size=1) {
  obs_tlfd <- select(observed_trips_FCE, treso.id.por, treso.id.pos, enrolment, value) %>%
    mutate(bin = cut(value, seq(0, max_value, bin_size), labels = seq(bin_size, max_value, bin_size))) %>%
    group_by(bin) %>%
    summarise(enrolment = sum(enrolment)) %>%
...
prop_matrix2_FCE <- matrix_balancing(prop_matrix_FCE, por_full, pos_full, 1000, 0.001)

```

```{r profiling transpose vs sweep, eval=FALSE, include=FALSE}
sum <- rowSums(prop_matrix_FCE)
sc <- por_full / sum
sc <- as.vector(sc)

Rprof(tmp <- tempfile(),interval = 0.001)
t(t(prop_matrix2_FCE) * sc) # first option
Rprof()
MyTimerTranspose=summaryRprof(tmp)$sampling.time
unlink(tmp)

Rprof(tmp <- tempfile(),interval = 0.001)
sweep(prop_matrix2_FCE, MARGIN = 1, sc, `*`) # sweep option
Rprof()
MyTimerSweep=summaryRprof(tmp)$sampling.time
unlink(tmp)
```
...
print(paste0("The average percent change between simulated and 2017 observed ADE for English Public Elementary schools is ", round(avg_perc2, 2)))
...
ggplot(data=result_school2.data) +
  ...
  facet_wrap(~ mof_region, scale = "fixed") +
  theme(legend.position = "none") + 
  labs(x="Enrollment Difference (Count)", y="Percent Difference (%)", subtitle="Elementary - English Public", title="Closest School vs. Observed Demand (2017)")
