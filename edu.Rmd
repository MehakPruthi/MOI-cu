...
board_type_sample_spread <- board_type_sample %>%
...
treso_level_sampling <- readRDS("output/student_overlay_90.rds") %>%
  select(treso.id.por, enrolment) %>%
  drop_na(treso.id.por) %>%
  left_join(select(treso_zone_def, treso_id, csduid), by = c("treso.id.por" = "treso_id")) %>%
  left_join(select(board_type_sample_spread, -enrolment.total, -sample.total), by = "csduid") %>%
  group_by(treso.id.por) %>%
  summarise(
    enrolment = sum(enrolment),
    `English Catholic` = first(`English Catholic`),
    `English Public` = first(`English Public`),
    `French Catholic` = first(`French Catholic`),
    `French Public` = first(`French Public`)
  ) %>%
