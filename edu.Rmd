...
student_travel_90 <- read_csv("output/student_travel_0.9.csv")
# Students within 80th percentile
student_travel_80 <- read_csv("output/student_travel_0.8.csv")
# Students within 70th percentile
student_travel_70 <- read_csv("output/student_travel_0.7.csv")
# Students within 50th percentile
student_travel_50 <- read_csv("output/student_travel_0.5.csv")
# Students within 10th percentile
student_travel_10 <- read_csv("output/student_travel_0.1.csv")
...
student_spdf <- select(student_travel_90, c(studentLat, studentLong, dist, schoolName, Enrolment, student_postal_id)) %>%
  rename(
    lat = studentLat,
    long = studentLong,
    euclidean_dist = dist,
...
school_spdf <- select(student_travel_90, c(schoolName, schoolLat, schoolLong, DSBINDEX, SFIS, percDist)) %>%
  group_by(schoolName) %>%
...
student_overlay <- over(student_xy, treso_shp, returnList = FALSE) %>%
  cbind(euclidean_dist = student_xy@data$euclidean_dist,
        student_postal_id = student_xy@data$student_postal_id,
...
## Straight Line distances frequency plots {.tabset .tabset-fade}

- Convert this to use the TRESO distance once we obtain the skims.
- The code use `uncount()` to convert the enrollment number to individual entries in the tibble

### 90th Percentile plots
```{r tlfd plots for 90th percentile, fig.width = 10, fig.height = 7, echo = FALSE}
student_tlfd <- student_travel_90 %>% 
  select(DSBINDEX, dist, Enrolment, PANEL, student_postal_id) %>%
  rename(dsb = DSBINDEX) %>%
  left_join(select(school_board_def, dsb, board_type_name), by = "dsb") %>%
  left_join(select(student_overlay, student_postal_id, treso_id_por), by = "student_postal_id") %>%
...
ggplot(student_tlfd, aes(x = dist, color = as.factor(board_type_name))) +
...
ggplot(student_tlfd, aes(dist, color = as.factor(mof_region))) +
  geom_freqpoly(stat = "bin", binwidth = 0.1, size = 1) + 
  facet_wrap(vars(board_type_name), scales = "free_y") +
...
```{r tlfd plots for 80th percentile, fig.width = 10, fig.height = 7, echo = FALSE}
student_tlfd <- student_travel_80 %>% 
  select(DSBINDEX, dist, Enrolment, PANEL, student_postal_id) %>%
  rename(dsb = DSBINDEX) %>%
  left_join(select(school_board_def, dsb, board_type_name), by = "dsb") %>%
  left_join(select(student_overlay, student_postal_id, treso_id_por), by = "student_postal_id") %>%
...
```{r tlfd plots for 70th percentile, fig.width = 10, fig.height = 7, echo = FALSE}
student_tlfd <- student_travel_70 %>% 
  select(DSBINDEX, dist, Enrolment, PANEL, student_postal_id) %>%
  rename(dsb = DSBINDEX) %>%
  left_join(select(school_board_def, dsb, board_type_name), by = "dsb") %>%
  left_join(select(student_overlay, student_postal_id, treso_id_por), by = "student_postal_id") %>%
...
```{r tlfd plots for 50th percentile, fig.width = 10, fig.height = 7, echo = FALSE}
student_tlfd <- student_travel_50 %>% 
  select(DSBINDEX, dist, Enrolment, PANEL, student_postal_id) %>%
  rename(dsb = DSBINDEX) %>%
  left_join(select(school_board_def, dsb, board_type_name), by = "dsb") %>%
  left_join(select(student_overlay, student_postal_id, treso_id_por), by = "student_postal_id") %>%
...
## Buffer Operation for Linear Regression
...
``` {r buffer treso zones to schools, eval = FALSE, echo = FALSE}
...
```{r prepare data for linear regression, eval = FALSE, echo = FALSE}
