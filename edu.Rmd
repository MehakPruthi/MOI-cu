...
school_summary_20xx_final <- forecast_school_ade(prop_matrix, trip_list, school_20xx, eqao_2017, new_school,
...
new_school <- df_list[[1]] %>% 
  ...
  rename(otg = avg.otg) %>% 
  # IFfthere are no existing schools in the TRESO zone, assign a default value of 1 to that new school
...
###	Set OTG at new schools to be a function of unconstrained ADE (formerly considering OTG from consolidated schools in the CSD)

```{r}
# Calculate number of new student places needed to achieve target utilization
otg_existing <- school_20xx %>% 
  select(otg) %>% 
  sum()

# Extract enrolment for the board-type/panel in question  
scenario_enrolment <- sum(por)

# Calcualte OTG needed based on user-defined target utilization
otg_final <- scenario_enrolment / TARGET_UTILIZATION
net_otg_required <- otg_final - otg_existing
...
new_school_iteration <- new_school_iteration %>% 
  ...
  # Change OTG in new schools to sum to new_otg_reqd
  mutate(sum.otg = sum(otg),
         revised.otg = round(net_otg_required / sum.otg * otg, 0)) %>% 
  select(-otg) %>% 
  rename(otg = revised.otg) %>% 
  # Calculate number of new schools needed per TRESO zone
  mutate(schools.needed = ceiling(otg / max.otg))
...
new_school_iteration <- new_school_iteration_max %>% 
...
print(paste0("The number of new schools after target utilization is: ", nrow(new_school_iteration)))
...
new_school_list_area <- new_school_list %>% 
  left_join(area_per_otg_long, by = c('user_input_school_size' = 'OTG', 'user_input_panel' = 'panel')) %>% 
