...
treso_tb <- socio_economic_tb %>%
  ...
  summarize(
    ...
    n_sec_pop = length(treso_zone[age <= 18 & age >= 14]),
    n_ele_pop = length(treso_zone[age <= 13 & age >= 6]),
    n_pre_pop = length(treso_zone[age <= 5]),
...
school_of_interest <- "A Y Jackson SS"
...
school_tb_temp <- as.tibble(datalist_df) %>%
  ...
  left_join(select(treso_zone_def, treso_id, area), by = c("treso_id")) %>%
  replace(is.na(.), 0) %>%
...
school_tb_filtered <- filter(school_tb, ade_2011 > 0, panel == "Elementary", board_type_name == "English Public", 
                             area == "york")
...
ggplot(linreg, aes(y = ade_2011, x = n_ele_pop_sum_sqrt)) +
  geom_point(alpha = 0.5, color = "#F8766D") +
  geom_smooth(method = "lm", se = FALSE)

# Build models
mod.base <- lm(ade_2011 ~ n_ele_pop_sum_sqrt, data = linreg)
mod.expand <- lm(ade_2011 ~ n_ele_pop_sum_sqrt + n_pre_pop_sum_sqrt, data = linreg)
