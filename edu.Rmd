...
travel_times <- travel_time_skim %>% # TODO: Replace this with year-appropriate travel-time skim
  arrange(orig, dest)
...
shortlist2 <- shortlist_zones %>% 
  filter(proximity.flag == 1) %>%
  distinct(treso.id.por)

shortlist3 <- shortlist_zones %>% 
  filter(proximity.flag + build.flag >= 1)
...
row_check <- 1
start.time <- Sys.time()
success_tibble <- tibble(zone = integer())
proximity_tibble <- tibble(zone = integer())
...
while (row_check > 0) {
  ...
  proximity_tibble <- rbind(proximity_tibble, shortlist_pairs2)
