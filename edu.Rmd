...
otg_needed <- scenario_enrolment / TARGET_UTILIZATION
otg_required <- otg_needed - otg_existing
...
schools_consolidated_closed_otg <- school_20xx %>% 
  filter(is.consolidated == 1) %>%
  group_by(csduid) %>% 
...
new_school_iteration <- new_school_iteration %>% 
  ...
  mutate(sum.otg = sum(otg),
         revised.otg = round(otg_required / sum.otg * otg, 0)) %>% 
  select(-otg) %>% 
...
# Combine lists of unconsolidated schools
# unconsolidated_schools <- unconsolidate_treso_por %>% 
#   bind_rows(., unconsolidate_treso_pos)

# Redefine school_master to include existing schools which are in zones with otherwise high travel times
# school_master_backup2 <- school_master
# school_master <- unconsolidated_schools %>% 
#   distinct(sfis, time.critical.flag) %>% 
#   left_join(schools_consolidated, by = c('sfis')) %>% 
#   bind_rows(., school_master)
# 
# schools_consolidated_closed <- schools_consolidated_closed %>% 
#   anti_join(unconsolidated_schools, by = c('sfis'))
# 
# schools_consolidated_final <- schools_consolidated %>% 
#   anti_join(unconsolidated_schools, by = c('sfis'))

######## REPLACE THE NEXT STEP WITH FUNCTION(S) FROM STEP 2
...
# Rerun step 4 until treshoold (takes about 55 iterations for 100 threshold)
school_forecast_final <- distribution_model(school_base, filter(school_20xx, is.consolidated == 0), school_summary_2017,
                                            ...
                                            pos_full, overfill_threshold = 2000, capacity_constrained = TRUE)[[1]]
...
school_summary_20xx_final <- school_summary_20xx_final %>% 
  mutate(portable.ade = pmax(simulated.ade - otg, 0), 
         portable.count = roundup(portable.ade / portable_class_size))
```
