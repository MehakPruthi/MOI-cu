...
min_tt_threshold <- 750 # TODO:Replace this with value provided by user through the visualization interface?
...
# Create pos vector
pos <- create_pos_vector(school_master, new_school=NULL, pos_full, year_id=year_id, panel_id=panel_id, board_id=board_id)
...
number_of_zero_simulated_schools <- school_master %>% 
  filter(panel == panel_id, board_type_name == board_id, year == year_id) %>% 
...
# Create pos vector and re-run key components of Step 2
pos <- create_pos_vector(school_master, new_school_slim, pos_full, year_id=year_id, panel_id=panel_id, board_id=board_id)
...
# Produce the raw simulated ADE for each school
school_summary_df_list <- forecast_school_ade(prop_matrix, trip_list, school_master, eqao_2017, new_school_slim,
                                           year_id = year_id, panel_id = panel_id, board_id = board_id)
...
- IF USER WISHES TO CONSOLIDATE EXISTING SCHOOLS: 
...
school_master <- existing_school_utilization
...
-	Set OTG at new schools to be a function of total ADE determined in Step 10
...
new_school_slim <- new_school %>% 
  right_join(new_school_ade, by = c('sfis')) %>% 
  mutate(otg = simulated.ade)

```

-	Rerun model capacity constrained to forecast ADE by school for all schools (new and existing)
...
# Start by finding zones with schools which were eliminated due to under-utilization
underutilized_schools <- school_master %>% 
  filter(year == year_id, panel == panel_id, board_type_name == board_id) %>% 
...
# Filter trip list down to include only zones which have more than 5 trips to a destination zone AND have long trips
trip_length_check <- cbind(trip_list, travel_times) %>%
  rename(travel.time = value) %>%
  filter(travel.time > 60) %>% 
  group_by(treso.id.por) %>% 
  mutate(total.por.trips = sum(trips)) %>% 
  ungroup() %>% 
  filter(total.por.trips > 5) %>% 
  ...
  select(-orig, -dest) %>% 
  inner_join(underutilized_schools, by = c('treso.id.por' = 'treso.id.pos')) %>%
...
school_master_slim_backup <- school_master
school_master <- school_master_backup %>% 
  filter(year == year_id, panel == panel_id, board_type_name == board_id) %>% 
  select(-simulated.ade) %>% 
  left_join(select(school_forecast, sfis, simulated.ade), by = c('sfis')) %>% 
  mutate(simulated.utilization = simulated.ade / otg) %>% 
