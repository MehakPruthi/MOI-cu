...
closest_school <- readRDS('output/closest_school.rds') %>%
...
closest_school_summary %>%
  ...
  geom_text_repel(aes(label = percentage.lable), size = 2) +
  facet_grid(rows = vars(board.type.name), cols = vars(panel)) +
  theme_minimal() + 
  theme(axis.text.x = element_blank()) +
  scale_fill_brewer(palette = 'Dark2') +
  xlab('') +
  ylab('') +
  labs(fill = 'Schools within travel distance') +
  ggsave('output/closest_school_pie.png')
