...
simulated_trips <- calculate_simulated_trips(observed_trips, treso_travel_time, alpha=0.157586314065864, beta=-0.380749188846144)
...
#""The mean observed travel time is: 7.41 ,and observed 90th percentile travel time is: 15.39 , and observed mean travel distance is: 3.48 , and the observed 90th percentile travel distance is: 8.53"

tlfd_parameter <- read_csv("cache/tlfd_parameters.csv")
alpha = filter(tlfd_parameter, panel == panel_id, board_type == board_id)$alpha
beta = filter(tlfd_parameter, panel == panel_id, board_type == board_id)$beta
alpha=0.157586314065864
beta=-0.380749188846144

simulated_trips <- readRDS("cache/balanced_trip_list_EPE.rds") %>%
  rename(enrolment = trips) %>%
  filter(enrolment != 0) %>% 
  left_join(treso_travel_time, by = c('treso.id.por', 'treso.id.pos'))  
...
# "The mean simulated school travel time is: 11.87 ,and simulated school 90th percentile travel time is: 22.79"
