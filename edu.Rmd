...
# TODO: Replace these values provided by user through the visualization interface
...
test <- treso_forecast_population %>% 
    filter(panel == panel_id) %>% 
    left_join(board_type_sample_spread, by = c("panel", "cduid")) %>% 
    mutate(`English Public Enrolment` = potential.enrolment * `English Public`)

full_test <- left_join(test, forecast_population_by_board, by=c("treso.id.por", "panel", "cduid")) %>% 
  mutate(diff = `English Public.y`-`English Public Enrolment`)
...
df_list <- distribution_model(school_base, school_20xx, school_summary_2017, school_summary_20xx, prop_matrix,
                              ...
                              overfill_threshold = 2000, USER_OTG_THRESHOLD, capacity_constrained = CAPACITY_CONSTRAINED)
...
new_school_iteration <- new_school_iteration[rep(seq_len(nrow(new_school_iteration)), new_school_iteration$schools.needed), 1:ncol(new_school_iteration)]
...
school_forecast <- distribution_model(school_base, filter(school_20xx, is.consolidated == 0), school_summary_2017, 
                                      ...
                                      overfill_threshold = 2000, USER_OTG_THRESHOLD, capacity_constrained = TRUE)[[1]]
...
school_forecast_final <- distribution_model(school_base, filter(school_20xx, is.consolidated == 0), school_summary_2017,
                                            ...
                                            pos_full, overfill_threshold=20000, USER_OTG_THRESHOLD, capacity_constrained=TRUE)[[1]]
