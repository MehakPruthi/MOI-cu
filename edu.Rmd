...
observed_distance_mean <- trip_mean(observed_trips, 'distance')

observed_distance_percentile <- trip_percentile(observed_trips, 'distance', 0.90)
...
#"The mean observed travel time is: 7.44961690404022 ,and observed 90th percentile travel time is: 15.4265651702881 , and observed mean travel distance is: 3.49375116383719 , and the observed 90th percentile travel distance is: 8.68149776452613"
...
simulated_trips <- readRDS("cache/balanced_trip_list_EPS.rds") %>%
  rename(treso.id.por = orig, treso.id.pos = dest, enrolment = trips) %>%
  filter(enrolment != 0) %>% 
  left_join(treso_travel_time, by = c('treso.id.por', 'treso.id.pos'))  
...
simulated_time_mean <- trip_mean(simulated_trips, 'time')

simulated_time_percentile <- trip_percentile(simulated_trips, 'time', 0.90)
...
closest_temp <- readRDS("cache/temp_result_student_tlfd.rds") %>% 
  filter(panel == panel_id, board_type_name == board_id) %>% 
  ungroup() %>% 
  ...
  summarise (sum.enrolment = sum(enrolment)) %>% 
  left_join(treso_travel_time, by = c("treso.id.por", "treso.id.pos")) %>% 
  rename(enrolment = sum.enrolment) 
  
closest_time_mean <- trip_mean(closest_temp, 'time')

closest_time_percentile <- trip_percentile(closest_temp, 'time', 0.90)
...
#combined_tlfd_2 <- generate_tlfd(observed_trips, closest_temp, max_value, bin_size) %>% 
  #drop_na()
combined_tlfd_2 <- generate_tlfd(observed_trips, closest_temp, max_value, bin_size)
...
#"The mean observed travel time is: 6.36874175037372 ,and observed 90th percentile travel time is: 11.1917533874512 , and observed mean travel distance is: 1.71913424841703 , and the observed 90th percentile travel distance is: 3.65137937913444"
...
simulated_trips <- readRDS("cache/balanced_trip_list_ECE.rds") %>%
...
#"The mean simulated school travel time is: 6.31919775823977 ,and simulated school 90th percentile travel time is: 11.1588888168335"
...
#"The mean closest school travel time is: 6.09702373860623 ,and closest school 90th percentile travel time is: 10.8471269607544"
...
prop_matrix_EPS_2011 <- readRDS("cache/cfunc_EPS.rds")
...
pos_school_weight <- calculate_school_weight_forecasting(trip_list_EPS_2011_sum, school_treso_master, eqao_2017, 
...
ggplot(data_2011, aes(abs.diff, perc.diff, color=method)) +
  ...
  #facet_wrap(~ mof_region, scale = "fixed") +
  labs(x="Enrollment Absolute Difference (Count)", y="Percent Difference (%)",
       subtitle="Secondary - English Public", title="Estimated vs. Observed Demand (2011)")
```
