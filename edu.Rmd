...
### Application of Gravity Model to 2011

```{r application of gravity model to 2011}

#TRESO Population sampling

  # Apply the board sampling probabilities to the students
Observed_2011_treso_ele <- readRDS("cache/treso_tb.rds") %>%
  select(treso_zone, n_ele_pop) %>%
  filter(n_ele_pop != 0) %>%
  rename(enrolment = n_ele_pop) %>%
  mutate(panel = "Elementary")

Observed_2011_treso_sec <- readRDS("cache/treso_tb.rds") %>%
  select(treso_zone, n_sec_pop) %>%
  filter(n_sec_pop != 0) %>%
  rename(enrolment = n_sec_pop) %>%
  mutate(panel = "Secondary")

observed_2011_treso <- Observed_2011_treso_ele %>% ##do rbind in definition
  full_join(Observed_2011_treso_sec) %>%  
  left_join(select(read.csv("input/treso/treso_zone_system.csv"), csduid, csdname, treso_id), by = c("treso_zone" = "treso_id")) %>%
...
treso_level_sampling_2011 <- observed_2011_treso %>%
  ...
  drop_na() %>%  #summarise the number of enrolment of dropped students
...
schoolEnrolSlim <- schoolEnrol %>%
  ...
  filter(year == 2011, status == "Open", dataset.id == 2) %>%
  left_join(select(school_board_def, dsb, board_type_name), by = c("dsb.index" = "dsb"))
  # Remove schools with capacity of 0
  #filter(otg != 0, ade != 0) %>%

#Scale TRESO sampled enrollemnt to observed school demand by board type and panel
schoolEnrolSlim_sum <- schoolEnrolSlim %>%
  group_by(board_type_name, panel)%>%
  summarise (ade.sum = sum(ade)) 

ECE_sum <- filter(schoolEnrolSlim_sum, board_type_name == "English Catholic", panel == "Elementary")$ade.sum
EPE_sum <- filter(schoolEnrolSlim_sum, board_type_name == "English Public", panel == "Elementary")$ade.sum
FCE_sum <- filter(schoolEnrolSlim_sum, board_type_name == "French Catholic", panel == "Elementary")$ade.sum
FPE_sum <- filter(schoolEnrolSlim_sum, board_type_name == "French Public", panel == "Elementary")$ade.sum
ECS_sum <- filter(schoolEnrolSlim_sum, board_type_name == "English Catholic", panel == "Secondary")$ade.sum
EPS_sum <- filter(schoolEnrolSlim_sum, board_type_name == "English Public", panel == "Secondary")$ade.sum
FCS_sum <- filter(schoolEnrolSlim_sum, board_type_name == "French Catholic", panel == "Secondary")$ade.sum
FPS_sum <- filter(schoolEnrolSlim_sum, board_type_name == "French Public", panel == "Secondary")$ade.sum
...
treso_level_sampling_2011_sum <- treso_level_sampling_2011 %>%
  ...
  left_join(schoolEnrolSlim_sum, by=c("panel", "board_type_name")) %>%
  mutate(ratio = ade.sum/enrolment.sum)

treso_por_2011 <- select(treso_level_sampling_2011, treso.id.por, panel, csduid, csdname, `English Catholic`:`French Public`) %>%
...
tresoOtgSlim <- readRDS("output/school_treso_master.rds") %>%
  filter(year == "2011", otg != 0) %>% 
  group_by(treso.id.pos, board_type_name, panel) %>% 
  summarise(otg = sum(otg)) %>% 
  ungroup()

# # The school capacity dataset contains the correct lat/long for schools - use that to get treso zone otg
# schoolEnrol <- read.csv("input/edu/school_capacity.csv", stringsAsFactors = FALSE)
# tresoEnrolSlim <- tempd %>%
#   arrange(year, dsb.index, panel, sfis) %>%
#   filter(year == 2011, status == "Open", dataset.id == 2) %>% 
#   left_join(select(tempd, sfis, treso.id.pos), by = "sfis") %>%
#   left_join(select(school_board_def, dsb, board_type_name), by = c("dsb.index" = "dsb")) %>%
#   group_by(treso.id.pos, board_type_name, panel) %>% 
#   summarise(otg = sum(otg)) %>% 
#   ungroup()

saveRDS(tresoOtgSlim, "cache/tresoOtgSlim.rds")
...
1D balance the POR and POS demand using the calibrated cfunc stored in simulated trips
...
input_pos <- readRDS("cache/tresoOtgSlim.rds") %>% 
  filter(panel == panel_id, board_type_name == board_id) %>% 
  select(otg, treso.id.pos)
...
prop_matrix_EPS_2011 <- readRDS("output/prop_matrix_EPS.rds") #read prop_matrix_EPS from output file and jump to matrix balancing

# 1D balacne the matrix
prop_matrix2_EPS_2011 <- matrix_balancing_1d(prop_matrix_EPS_2011, por, pos, constrained=TRUE)

# Save the balanced POR-POS trip list
trip_list_EPS_2011 <- melt(prop_matrix2_EPS_2011)
colnames(trip_list_EPS_2011) <- c('orig', 'dest', 'trips')
saveRDS(trip_list_EPS_2011, "cache/balanced_trip_list_EPS_2011.rds")

#distribute treso level pos demand to individual schools

trip_list_EPS_2011_sum <- trip_list_EPS_2011 %>% 
  group_by(dest) %>% 
  summarise(trips = sum(trips))

# temptemp <- readRDS("output/school_treso_master.rds") %>% 
#   filter(otg != 0)

temptemp <- readRDS("output/school_treso_master.rds")

pos_school_weight <- calculate_school_weight_forecasting(trip_list_EPS_2011_sum, ayear = "2011", temptemp,
...
trip_list_EPS_2011 <- readRDS(paste0("cache/balanced_trip_list_EPS_2011.rds")) %>%
  left_join(pos_school_weight, by = c("dest"))

# Apply bucket rounding to chunks of data by TRESO POS
results_EPS <- by(trip_list_EPS_2011$trips, trip_list_EPS_2011[c("dest")], smart_round, simplify = TRUE)

# Convert the output of by() to a dataframe
results2_EPS <- sapply(results_EPS, I)
colnames(results2_EPS) <- colnames(prop_matrix2_EPS_2011)
rownames(results2_EPS) <- rownames(prop_matrix2_EPS_2011)

df <- results2_EPS %>%
  melt()
colnames(df) <- c('treso.id.por', 'treso.id.pos', 'enrolment.rounded')

trip_list2_EPS_2011 <- bind_cols(trip_list_EPS_2011, df) %>%
  filter(enrolment.rounded != 0) %>%
  select(-treso.id.por, -treso.id.pos)

# Sample the schools by weight
plan(multiprocess)
results_EPS <- future_apply(trip_list2_EPS_2011, 1, sample_by_row)
results_tb_EPS <- t(as.data.table(results_EPS))
trip_list_schools_EPS_2011 <- cbind(trip_list2_EPS_2011, results_tb_EPS)


trip_list_schools_EPS_2011_summed <- trip_list_schools_EPS_2011 %>% 
...
saveRDS(trip_list_schools_EPS_2011_summed, "output/trip_list_schools_EPS_2011_summed.rds")
...
schoolEnrolObserved <- read.csv("input/edu/school_capacity.csv", stringsAsFactors = FALSE) 
schoolEnrolSlimObserved <- schoolEnrolObserved %>%
  select(year = Sch_yr, 
         dsb.index = DSB_Index, 
         board.name = BoardName, 
         panel = Panel, 
  ...
  left_join(select(school_board_def, dsb, board_type_name), by = c("dsb.index" = "dsb")) %>% 
  filter(panel == panel_id, board_type_name == board_id) %>% 
  select(panel, sfis, school.name, ade, board_type_name)
  
temptemp2 <- readRDS("output/school_treso_master.rds") %>% 
  filter(year == 2011)

# marketshare 2011 enrolment diff and join with gravity table
market_2011 <- readRDS("cache/school_2007_market_share.rds") %>% #update with RDS market share file
  mutate(AbsDiffMarket =  (enrol2011.by2007MS - ade2011)) %>% #switch with enrolment variable
  mutate(percent_enrol_diff_mk = round((AbsDiffMarket/ade2011*100),2))%>% 
  left_join(select(temptemp, sfis, board_type_name, treso.id.pos), by="sfis") %>% 
  filter(panel==panel_id, board_type_name==board_id)
  


#gravity 2011 enrolment diff 
gravity_2011 <- readRDS("output/trip_list_schools_EPS_2011_summed.rds") %>% 
  left_join(select(schoolEnrolSlimObserved, sfis, ade), by = "sfis") %>% 
  mutate(AbsDiffGrav= (enrol.value - ade)) %>%
  mutate(percent_enrol_diff_gr = round((AbsDiffGrav/ade)*100,2)) %>% 
  filter(percent_enrol_diff_gr < 1000, !is.infinite(percent_enrol_diff_gr)) 

  
  
gravity_2011.data <- data.frame("y" = gravity_2011$percent_enrol_diff_gr,
                                         "x" = gravity_2011$AbsDiffGrav)
ggplot(data=gravity_2011.data) +
  ...
  #geom_point(aes(x = x, y = y), size = 1.5, alpha = 0.15) +
  coord_cartesian(xlim=c(0,400), ylim=c(0,250)) +
  #facet_wrap(~ mof_region, scale = "fixed") +
  theme(legend.position = "none") +
  labs(x="Enrollment Absolute Difference (Count)", y="Percent Difference (%)",
       subtitle="Secondary - English Public", title="Simulated vs. Observed Demand (2011)")



market_2011.data <- data.frame("y" = market_2011$percent_enrol_diff_mk,
                                         "x" = market_2011$AbsDiffMarket)
ggplot(data=market_2011.data) +
  geom_hline(yintercept = 0, colour="pink", size=1) +
  geom_vline(xintercept = 0, colour="pink", size=1) +
  geom_point(aes(x = x, y = y), size = 1.5, alpha = 0.15) +
  #geom_point(aes(x = x, y = y), size = 1.5, alpha = 0.15) +
  ...
  theme(legend.position = "none") +
  labs(x="Enrollment Absolute Difference (Count)", y="Percent Difference (%)",
       subtitle="Secondary - English Public", title="Market Share vs. Observed Demand (2011)")
...
## Questions:
## - Why are we using 'observed_por_pos_90.rds'? Why not _80, _70, etc.? Does it make a difference?
## - Does it matter that we are using travel_time_skim in the observed_trips dataframe but treso_travel_time in the simulated dataframe?
## - Why does observed_trips use Euclidean Travel Time where no other time is available? Let's use manhattan distance function in helpers --> manhattan(lat1, lon1, lat2, lon2)
...
observed_time_mean <- trip_mean(observed_trips, 'time')

observed_time_percentile <- trip_percentile(observed_trips, 'time', 0.90)
  

treso_travel_time <- readRDS('output/treso_travel_time.rds')
tripListBal <- readRDS('cache/trip_list2_EPE.rds') %>%
  rename(enrolment = enrolment.rounded) %>%
  left_join(treso_travel_time, by = c('treso.id.por', 'treso.id.pos')) %>% 
  rename(value = travel.time) 
  
tripListBalFilt <- tripListBal %>% 
  filter(value > 0)

simulated_trips <- calculate_simulated_trips(observed_trips, cost, alpha=0.5, beta=-0.11)
combined_tlfd <- generate_tlfd(observed_trips, simulated_trips, max_value, bin_size)

# combined_tlfd_bal <- generate_tlfd(observed_trips, tripListBal, max_value, bin_size)

ggplot(data=combined_tlfd, aes(x=bin, y=enrolment, group=flag, color=flag, alpha=0.3)) +
  geom_col(size = 1.25, position = 'identity') +
...
prop_matrix_EPE <- travel_time_skim %>%
  ...
  column_to_rownames(var = "orig") %>%
  data.matrix()
saveRDS(prop_matrix_EPE, "output/prop_matrix_EPE.rds")

# 1D balacne the matrix
prop_matrix2_EPE <- matrix_balancing_1d(prop_matrix_EPE, por, pos, constrained=TRUE)
...
# Save the simulated trip list
saveRDS(simulated_trips, "cache/simulated_trips_EPS.rds")
```
...
simulated_trips_EPS <- readRDS("cache/simulated_trips_EPS.rds") #read prop_matrix_EPS from output file and jump to matrix balancing
...
prop_matrix_EPS <- travel_time_skim %>%
  ...
  column_to_rownames(var = "orig") %>%
  data.matrix()
saveRDS(prop_matrix_EPS, "output/prop_matrix_EPS.rds")

# 1D balacne the matrix
prop_matrix2_EPS <- matrix_balancing_1d(prop_matrix_EPS, por, pos, constrained=TRUE)
...
# Save the simulated trip list
saveRDS(simulated_trips, "cache/simulated_trips_ECE.rds")
```
...
prop_matrix_ECE <- travel_time_skim %>%
  ...
  column_to_rownames(var = "orig") %>%
  data.matrix()
saveRDS(prop_matrix_ECE, "output/prop_matrix_ECE.rds")

# 1D balacne the matrix
prop_matrix2_ECE <- matrix_balancing_1d(prop_matrix_ECE, por, pos, constrained=TRUE)
...
# Save the simulated trip list
saveRDS(simulated_trips, "cache/simulated_trips_ECS.rds")
```
...
prop_matrix_ECS <- travel_time_skim %>%
  ...
  column_to_rownames(var = "orig") %>%
  data.matrix()
saveRDS(prop_matrix_ECS, "output/prop_matrix_ECS.rds")

# 1D balacne the matrix
prop_matrix2_ECS <- matrix_balancing_1d(prop_matrix_ECS, por, pos, constrained=TRUE)
...
# Save the simulated trip list
saveRDS(simulated_trips, "cache/simulated_trips_FPE.rds")
```
...
prop_matrix_FPE <- travel_time_skim %>%
  ...
  column_to_rownames(var = "orig") %>%
  data.matrix()
saveRDS(prop_matrix_FPE, "output/prop_matrix_FPE.rds")

# 1D balacne the matrix
prop_matrix2_FPE <- matrix_balancing_1d(prop_matrix_FPE, por, pos, constrained=TRUE)
...
# Save the simulated trip list
saveRDS(simulated_trips, "cache/simulated_trips_FPS.rds")
```
...
prop_matrix_FPS <- travel_time_skim %>%
  ...
  column_to_rownames(var = "orig") %>%
  data.matrix()
saveRDS(prop_matrix_FPS, "output/prop_matrix_FPS.rds")

# 1D balacne the matrix
prop_matrix2_FPS <- matrix_balancing_1d(prop_matrix_FPS, por, pos, constrained=TRUE)
...
# Save the simulated trip list
saveRDS(simulated_trips, "cache/simulated_trips_FCE.rds")
```
...
prop_matrix_FCE <- travel_time_skim %>%
  ...
  column_to_rownames(var = "orig") %>%
  data.matrix()
saveRDS(prop_matrix_FCE, "output/prop_matrix_FCE.rds")

# 1D balacne the matrix
prop_matrix2_FCE <- matrix_balancing_1d(prop_matrix_FCE, por, pos, constrained=TRUE)
...
# Save the simulated trip list
saveRDS(simulated_trips, "cache/simulated_trips_FCS.rds")
```
...
prop_matrix_FCS <- travel_time_skim %>%
  ...
  column_to_rownames(var = "orig") %>%
  data.matrix()
saveRDS(prop_matrix_FCS, "output/prop_matrix_FCS.rds")

# 1D balacne the matrix
prop_matrix2_FCS <- matrix_balancing_1d(prop_matrix_FCS, por, pos, constrained=TRUE)
