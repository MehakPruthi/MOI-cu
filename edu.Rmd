...
# There are a handful of schools that does not get any simulated ADE to due to a variety of reasons. 
...
### Step 4 - Apply delta to 2017 Observed ADE. Create POR and POS vectors with overflown students.
...
zone_proximity_threshold <- 6 # TODO:Replace this with value provided by user through the visualization interface

# Combine travel time matrix with trip matrix
travel_times <- travel_time_skim %>% # TODO: Replace this year-appropriate travel-time skim
  arrange(orig, dest)
...
# Initialize dataframe and while-loop check
shortlist_zones <- threshold_tt_zones
row_check <- 0
tester <- read_csv("dmtest.csv")
shortlist_zones <- tester
success_tibble <- tibble(treso.id)
...
while (row_check == 0) {
  ...
  shortlist_zones <- shortlist_zones %>% 
    mutate(build.flag = ifelse(treso.id.por == build.zone | build.flag == 1, 1, 0)) %>%
    # Determine which other TRESO zones are too close to proposed new school
    mutate(proximity.flag = ifelse(proximity.flag == 1, 1, ifelse(treso.id.pos == build.zone & travel.time <= zone_proximity_threshold, 1, 0))) %>% 
    # Apply proximity flag to all instances of zones identified above
    group_by(treso.id.por) %>% 
    mutate(proximity.flag = max(proximity.flag)) %>% 
    ungroup() %>% 
    # Move any zones already set to build a new school, or already filtered out due to proximity, to the bottom of list of   potential sites
    arrange(build.flag, proximity.flag) 
  
  # Check to see if any rows (zones) remain to be considered for building a school
  row_check <- sum(first(shortlist_zones$build.flag), first(shortlist_zones$proximity.flag))
} 
...
while (row_check == 0) {
  ...
  shortlist_zones <- shortlist_zones %>% 
    mutate(build.flag = ifelse(treso.id.por == build.zone, 1, 0)) %>%
    filter(build.flag == 0) %>% 
    # Determine which other TRESO zones are too close to proposed new school
    mutate(proximity.flag = ifelse(treso.id.pos == build.zone & travel.time <= zone_proximity_threshold, 1, 0)) %>% 
    # Apply proximity flag to all instances of zones identified above
    group_by(treso.id.por) %>% 
    mutate(proximity.flag = max(proximity.flag)) %>%
    filter(proximity.flag == 0) %>% 
    ungroup()
  
  success_tibble <- success_tibble %>% 
    add_row(., treso.id = build.zone)
