...
TESTID <- 523550
datalist <- readRDS("output/closest_school_full_manhattan_distance.rds")
# Convert buffer into df
...
closest_school <- readRDS("output/closest_school_full_manhattan_distance.rds") %>%
...
closest_school_summary <- closest_school %>%
 ...
 closest_school_summary <- closest_school_summary %>%
   ...
   saveRDS("output/closest_school_manhattan_summary.rds")
...
g_subtotal <- readRDS("output/closest_school_manhattan_summary.rds") %>%
  drop_na(mof_region) %>%
...
g <- readRDS("output/closest_school_manhattan_summary.rds") %>%
  drop_na(mof_region) %>%
  ...
  ggsave("output/closest_school_manhattan_summary.png", height = 12, width = 10, units = "in")
...
treso_travel_time <- readRDS("output/treso_travel_time.rds")
school_travel_time <- readRDS("output/observed_por_pos_90.rds") %>% 
  drop_na(treso.id.por) %>%
  left_join(treso_travel_time, by = c("treso.id.por", "treso.id.pos")) %>%
  rename(travel.time = value) %>%
  group_by(sfis) %>% 
...
saveRDS(school_travel_time, "output/school_travel_2017.rds")
```
...
trip_list_EPE <- reshape2::melt(prop_matrix2_EPE) %>% 
  arrange(Var1, Var2)
...
trip_list_EPE <- readRDS(paste0("cache/balanced_trip_list_EPE.rds")) %>%
...
# 1D balance the matrix
prop_matrix2_EPS <- matrix_balancing_1d(prop_matrix_EPS, por, pos, constrained=FALSE)

# Save the balanced POR-POS trip list
trip_list_EPS <- reshape2::melt(prop_matrix2_EPS) %>%
  arrange(Var1, Var2)
...
```{r simulate english catholic elementary trips, eval=FALSE, echo=FALSE}
...
# Save the balanced POR-POS trip list
trip_list_ECE <- reshape2::melt(prop_matrix2_ECE) %>% 
  arrange(Var1, Var2)
...
# Save the balanced POR-POS trip list
trip_list_ECS <- reshape2::melt(prop_matrix2_ECS) %>% 
  arrange(Var1, Var2)
...
# Save the balanced POR-POS trip list
trip_list_FPE <- reshape2::melt(prop_matrix2_FPE) %>% 
  arrange(Var1, Var2)
...
# Save the balanced POR-POS trip list
trip_list_FPS <- reshape2::melt(prop_matrix2_FPS) %>% 
  arrange(Var1, Var2)
...
# Save the balanced POR-POS trip list
trip_list_FCE <- reshape2::melt(prop_matrix2_FCE) %>% 
  arrange(Var1, Var2)
...
# Save the balanced POR-POS trip list
trip_list_FCS <- reshape2::melt(prop_matrix2_FCS) %>% 
  arrange(Var1, Var2)
...
# Manually create 2025 student population for zones in Sarnia
por <- readRDS("cache/treso_por_2017.rds") %>%
  # Create 2025 scenario manually
  mutate(enrolment.scaled = replace(enrolment.scaled,
                                    (treso.id.por == 6879 & panel == "Elementary" & board_type_name == "English Catholic"), 
                                    380)) %>% 
  mutate(enrolment.scaled = replace(enrolment.scaled,
                                    (treso.id.por == 6877 & panel == "Elementary" & board_type_name == "English Catholic"), 
                                    350)) %>% 
  saveRDS("cache/treso_por_2025.rds")

por <- readRDS("cache/treso_por_2025.rds") %>%
  filter(panel == panel_id, board_type_name == board_id) %>%
...
prop_matrix <- readRDS("cache/cfunc_EPE.rds")

# 1D balance the matrix
prop_matrix2 <- matrix_balancing_1d(prop_matrix, por, pos, constrained=TRUE)

# Save the balanced POR-POS trip list
trip_list <- reshape2::melt(prop_matrix2) %>%
  arrange(Var1, Var2)
colnames(trip_list) <- c('treso.id.por', 'treso.id.pos', 'trips')

# Calculate the school weight
pos_school_weight <- calculate_school_weight_forecasting(trip_list, school_master, eqao_2017, NULL,
...
test <- school_master %>%
  ...
  mutate(utilization = round(ade / otg, 2)) %>% 
  replace_na(list(ade = 0, simulated.utilization = 0))

# 2011 ----
por_2011 <- readRDS("output/treso_por_2011.rds") %>%
  filter(panel == panel_id, board_type_name == board_id) %>%
  select(treso.id.por, enrolment.scaled) %>%
  right_join(por_full, by=c("treso.id.por" = "orig")) %>%
  replace_na(list(enrolment.scaled = 0)) %>%
...
prop_matrix2_2011 <- matrix_balancing_1d(prop_matrix, por_2011, pos_2011, constrained=TRUE)

# Save the balanced POR-POS trip list
trip_list_2011 <- reshape2::melt(prop_matrix2_2011) %>%
  arrange(Var1, Var2)
colnames(trip_list_2011) <- c('treso.id.por', 'treso.id.pos', 'trips')

# Calculate the school weight
pos_school_weight_2011 <- calculate_school_weight_forecasting(trip_list_2011, school_master, eqao_2017, 2011, panel_id, board_id)
print(paste0("For ", panel_id, "-", board_id, ", there are ",
             nrow(filter(pos_school_weight_2011, length(school.weight.prob.list) == 1)),
             " TRESO zones with a single school, and ",
             nrow(filter(pos_school_weight_2011, length(school.weight.prob.list) != 1)),
             " TRESO zones with multiple schools."))

# Apply bucket rounding to chunks of data by TRESO POS
results_2011 <- by(trip_list_2011$trips, trip_list_2011[c("treso.id.pos")], smart_round, simplify = TRUE)

# Convert the output of by() to a dataframe
results2_2011 <- sapply(results_2011, I)
colnames(results2_2011) <- colnames(prop_matrix)
rownames(results2_2011) <- rownames(prop_matrix)

df_2011 <- reshape2::melt(results2_2011) %>%
  arrange(Var1, Var2)
colnames(df_2011) <- c('treso.id.por', 'treso.id.pos', 'enrolment.rounded')

pos_travel_time_2011 <- bind_cols(df_2011, select(treso_travel_time, value)) %>%
  group_by(treso.id.pos) %>%
  summarise(avg.travel.time = weighted.mean(value, enrolment.rounded)) %>%
  replace_na(list(avg.travel.time = 0))
...
# Combine the school weight
trip_list_2011 <- left_join(df_2011, pos_school_weight_2011, by=c("treso.id.pos"))

plan(multiprocess)
results_2011 <- future_apply(trip_list_2011, 1, sample_by_row)
results_tb_2011 <- t(as.data.table(results_2011))
trip_list_schools_2011 <- cbind(trip_list_2011, results_tb_2011)

trip_list_schools_summary_2011 <- trip_list_schools_2011 %>% 
  unnest(results_tb_2011) %>%
  mutate(enrol.value = 1) %>%
  rename(sfis = results_tb_2011) %>% 
  group_by(sfis) %>% 
  summarise(ade = sum(enrol.value))

# New School ----
new_school <- tibble(lat = 42.96597, long = -82.396048, school.name = "AAA", sfis = 9999,
                     otg = 300, board_type_name = "English Catholic", panel = "Elementary", year = 2025)
coordinates(new_school) <- c('long', 'lat')
# Project the student and school points from lat/long to TRESO's LCC specification
proj4string(new_school) <- CRS('+proj=longlat +datum=WGS84')
treso_projarg = treso_shp@proj4string@projargs

new_school_xy <- spTransform(new_school, CRS(treso_projarg))
school_new <- create_overlay(new_school_xy, treso_shp, type="marker")

school_master_new <- school_master %>% 
  bind_rows(school_new)

pos_new <- school_master_new %>%
  filter(year == year_id, panel == panel_id, board_type_name == board_id) %>%
  select(treso.id.pos, otg) %>%
  group_by(treso.id.pos) %>%
  summarise(otg = sum(otg)) %>%
...
# 1D balance the matrix
prop_matrix2_new <- matrix_balancing_1d(prop_matrix, por, pos_new, constrained=TRUE)

# Save the balanced POR-POS trip list
trip_list_new <- reshape2::melt(prop_matrix2_new) %>%
  arrange(Var1, Var2)
colnames(trip_list_new) <- c('treso.id.por', 'treso.id.pos', 'trips')

# Calculate the school weight
pos_school_weight <- calculate_school_weight_forecasting(trip_list_new, school_master, eqao_2017, 
                                                         school_new, year_id, panel_id, board_id)
print(paste0("For ", panel_id, "-", board_id, ", there are ",
             nrow(filter(pos_school_weight, length(school.weight.prob.list) == 1)),
             " TRESO zones with a single school, and ",
             nrow(filter(pos_school_weight, length(school.weight.prob.list) != 1)),
...
# Apply bucket rounding to chunks of data by TRESO POS
results_new <- by(trip_list_new$trips, trip_list_new[c("treso.id.pos")], smart_round, simplify = TRUE)

# Convert the output of by() to a dataframe
results2_new <- sapply(results_new, I)
colnames(results2_new) <- colnames(prop_matrix)
rownames(results2_new) <- rownames(prop_matrix)

df_new <- reshape2::melt(results2_new) %>%
  arrange(Var1, Var2)
colnames(df_new) <- c('treso.id.por', 'treso.id.pos', 'enrolment.rounded')
...
# Combine the school weight
trip_list_new <- left_join(df_new, pos_school_weight, by=c("treso.id.pos"))

plan(multiprocess)
results_new <- future_apply(trip_list_new, 1, sample_by_row)
results_tb_new <- t(as.data.table(results_new))
trip_list_schools_new <- cbind(trip_list_new, results_tb_new)

trip_list_schools_summary_new <- trip_list_schools_new %>% 
  unnest(results_tb_new) %>%
  mutate(enrol.value = 1) %>%
  rename(sfis = results_tb_new) %>% 
  group_by(sfis) %>% 
  summarise(ade = sum(enrol.value))
...
test <- school_master %>%  
  ...
  bind_rows(school_new) %>% 
  left_join(trip_list_schools_summary, by=c("sfis")) %>%
  mutate(ade = ifelse(is.na(simulated.ade), ade, simulated.ade)) %>% 
  left_join(pos_travel_time, by=c("treso.id.pos")) %>%
  mutate(avg.travel.time = ifelse(is.na(sim.avg.travel.time), avg.travel.time, sim.avg.travel.time)) %>%
  mutate(utilization = round(ade / otg, 2)) %>%
  replace_na(list(utilization = 0))
...
saveRDS(trip_list_EPE_2011, "cache/balanced_trip_list_EPE_2011.rds")
...
```{r calibrate tlfd, include = FALSE}
panel_id = "Elementary"
board_id = "English Public"
...
## Creating Forecasted TRESO POR Data

```{r forecast TRESO POR, include = FALSE}
YEAR = 2025

board_type_sample_spread <- readRDS("cache/board_type_sample_spread.rds")

treso_forecast_population <- readRDS("input/edu/treso_population_edu.rds") %>% 
  select(treso.id.por = treso_zone, panel = age.group, csduid, enrolment = paste0("population.", YEAR)) %>% 
  filter(panel != "other")
  
# Apply the board sampling probabilities to the students
treso_forecast_population_board <- treso_forecast_population %>%
  left_join(select(board_type_sample_spread, -enrolment.total, -sample.total), by = c("panel", "csduid")) %>%
  group_by(treso.id.por, panel) %>%
  summarise(
    csduid = first(csduid),
...
forecast_por <- treso_forecast_population_board %>% 
...
school_master <- readRDS("output/edu/school_treso_master.rds") %>%
  filter((status == "Open" | is.na(status)), otg != 0, ade != 0)
board_type_sample_spread <- readRDS("cache/board_type_sample_spread.rds") 

school_board_list <- c("English Public", "English Catholic", "French Public", "French Catholic")
panel_list <- c("Elementary", "Secondary")
year_id <- 2017

treso_forecast_population <- readRDS("input/edu/treso_population_edu.rds") %>% 
  select(treso.id.por = treso_zone, panel = age.group, csduid, enrolment = paste0("population.", year_id)) %>%
  filter(panel != "other")

for (board_id in school_board_list) {
  for (panel_id in panel_list) {
    ...
    prop_matrix <- readRDS(paste0("cache/cfunc_", segment_id, ".rds"))
    
    # Create pos vector
    pos <- create_pos_vector(school_master, pos_full, year_id=year_id, panel_id=panel_id, board_id=board_id)
    ...
    # 1D balancing
    prop_matrix_balanced <- matrix_balancing_1d(prop_matrix, por, pos, axis=1, constrained=TRUE)
    ...
    school_summary_2017 <- forecast_school_ade(prop_matrix, trip_list, school_master, eqao_2017, new_school = NULL,
...
for (board_id in school_board_list) {
  for (panel_id in panel_list) {
    ...
    por <- treso_forecast_population_board %>% 
      select(treso.id.por, panel, csduid, csdname, `English Catholic`:`French Public`) %>%
      gather(key="board_type_name", value="enrolment", `English Catholic`:`French Public`) %>% 
      filter(panel == panel_id, board_type_name == board_id) %>% 
    ...
    # Calculate the school weight
    pos_school_weight <- calculate_school_weight_forecasting(trip_list, school_master, eqao_2017, new_school = NULL,
                                                             year_id, panel_id, board_id)
    print(paste0("For ", panel_id, "-", board_id, ", there are ",
                nrow(filter(pos_school_weight, length(school.weight.prob.list) == 1)),
                " TRESO zones with a single school, and ",
    ...
    trip_list_schools_summary_20xx <- trip_list_schools %>% 
        unnest(results_tb) %>%
        mutate(enrol.value = 1) %>%
        rename(sfis = results_tb) %>% 
        group_by(sfis) %>% 
...
### Step 3 - Calculate difference between Base and Forecast Models

```{r application of gravity model - step 3}
# Take the difference between the simulated ADEs
trip_list_schools_summary_difference <- full_join(trip_list_schools_summary_2017, trip_list_schools_summary_20xx, by="sfis", suffix=c(".2017", ".20xx")) %>% 
  replace_na(list(simulated.ade.2017 = 0, simulated.ade.20xx = 0)) %>% 
  mutate(change.ade = simulated.ade.20xx - simulated.ade.2017)
```

### Step 4 - Apply the difference to 2017 Observed to obtain forecast ADE for 20xx. Iterate the distribution until no school is above OTG Threshold

```{r application of gravity model - step 4}
USER_OTG_THRESHOLD = 1.1 # Percentage of current OTG

school_forecast <- filter(school_master, year == 2017) %>% 
  filter((status == "Open" | is.na(status)), otg != 0, ade != 0) %>% 
  filter(panel == panel_id, board_type_name == board_id) %>% 
...
# Create a POS vector with schools over OTG Threshold - pos_overfill
...
por_overfill <- prop_matrix_balanced %*% pos_overfill %>% 
  ...
  # scale the number of origin ADE to match the overflown ADE total
  rownames_to_column(var = "treso.id.por") %>% 
  group_by() %>% 
  mutate(ade.overfill = overfill.flag * overfill_ade / sum(overfill.flag)) %>% 
  ungroup() %>% 
...
### Step 5 - Check the iterated results
...
print(paste0("ADE is: ", sum(por_overfill), ". OTG is: ", sum(pos_underfill), "."))
    
# 1D balancing
prop_matrix_balanced <- matrix_balancing_1d(prop_matrix, por_overfill, pos_underfill, axis=1, constrained=TRUE)
trip_list <- reshape2::melt(prop_matrix_balanced) %>%
    arrange(Var1, Var2)
colnames(trip_list) <- c("treso.id.por", "treso.id.pos", "trips")
...
trip_list_schools_summary_20xx_iterated <- trip_list_schools %>% 
    unnest(results_tb) %>%
    mutate(enrol.value = 1) %>%
    rename(sfis = results_tb) %>% 
    group_by(sfis) %>% 
...
# Create a school list of ADE overfill, so it can be removed in summary
school_overfill_ade <- school_forecast %>% 
  mutate(overfill.flag = ifelse(simulated.ade > otg.threshold, 1, 0),
         overfill.ade = simulated.ade - otg.threshold) %>% 
  filter(overfill.flag == 1) %>% 
  select(sfis, overfill.ade)

trip_list_schools_summary_20xx_final <- full_join(trip_list_schools_summary_20xx,
                                                  trip_list_schools_summary_20xx_iterated,
  ...
  left_join(school_overfill_ade, by="sfis") %>% 
  replace_na(list(overfill.ade = 0)) %>% 
  mutate(simulated.ade = simulated.ade - overfill.ade) %>% 
  select(-overfill.ade)

# Take the difference between the simulated ADEs
trip_list_schools_summary_difference <- full_join(trip_list_schools_summary_2017,
                                                  trip_list_schools_summary_20xx_final, 
                                                  by="sfis", suffix=c(".2017", ".20xx")) %>%
  replace_na(list(simulated.ade.2017 = 0, simulated.ade.20xx = 0)) %>% 
  mutate(change.ade = simulated.ade.20xx - simulated.ade.2017)

school_forecast_final <- filter(school_master, year == 2017) %>% 
  filter((status == "Open" | is.na(status)), otg != 0, ade != 0) %>% 
  filter(panel == panel_id, board_type_name == board_id) %>% 
  full_join(select(trip_list_schools_summary_difference, sfis, simulated.ade.20xx, simulated.ade.2017, change.ade), by="sfis") %>% 
  replace_na(list(ade = 0, change.ade = 0)) %>% 
...
# 2D balancing, scale the total to the POS side 
prop_matrix_balanced <- matrix_balancing_2d(prop_matrix, por, pos, totals_to_use="columns", rel_error = 0.01)
trip_list <- reshape2::melt(prop_matrix_balanced) %>%
    arrange(Var1, Var2)
colnames(trip_list) <- c("treso.id.por", "treso.id.pos", "trips")
...
new_school_list <- tibble(user_input_school_size = c(500, 750), user_input_panel = c('Elementary', 'Secondary'), user_input_pcode = c('k0b1e4', 'P6A5K1')) # Replace this hard-coded value with user-input value from visualization
...
user_input_inflation <- 0.02 # Replace this hard-coded value with user-input value from visualization
user_input_scenario_year <- 2025 # Replace this hard-coded value with user-input value from visualization
...
annualized_inflated_cost <- construction_cost(user_input_inflation, user_input_scenario_year, currency_year, current_year, new_school_list_complete)
```
