...
min_tt_threshold <- 550 # TODO:Replace this with value provided by user through the visualization interface?
...
USER_OTG_THRESHOLD = 1.1 # TODO: Replace this hard-coded value with user-input value from visualization

# Define portable size
ELE_STUDENTS_UNIT_CLASS = 23 # TODO: Replace this hard-coded value with user-input value from visualization?
SEC_STUDENTS_UNIT_CLASS = 21 # TODO: Replace this hard-coded value with user-input value from visualization?

# Target utilization for future year
target_utilization <- 0.9 # TODO: Replace this hard-coded value with user-input value from visualization
...
treso_forecast_population_board <- apply_sampling_to_population(
  forecast_population = treso_forecast_population,
...
school_summary_2017 <- forecast_school_ade(prop_matrix, trip_list, school_master, eqao_2017, new_school,
                                           year_id = base_year, panel_id = panel_id, 
                                           board_id = board_id)[[1]]
...
school_forecast <- create_forecast_school_list(school_master_base, new_school, panel_id, board_id, trip_list_schools_summary_difference,
...
# TODO ask Alec as to what is this for again?
...
# Take the difference between the simulated ADEs
trip_list_schools_summary_difference <- full_join(school_summary_2017, school_summary_20xx, by="sfis", suffix=c(".2017", ".20xx")) %>% 
  replace_na(list(simulated.ade.base = 0, simulated.ade.20xx = 0)) %>% 
  mutate(change.ade = simulated.ade.20xx - simulated.ade.base)

school_forecast_final <- school_master_base %>% 
  filter((status == "Open" | is.na(status)), otg != 0, ade != 0) %>% 
  filter(panel == panel_id, board_type_name == board_id) %>% 
  # Bind in list of new schools
  bind_rows(., new_school) %>% 
  full_join(select(trip_list_schools_summary_difference, sfis, simulated.ade.20xx, simulated.ade.base, change.ade), 
            by="sfis") %>% 
