...
print(paste0("There are ", nrow(filter(pos_school, length(school.weight.prob.list) == 1)), " TRESO zones with a single school, and ", nrow(filter(pos_school, length(school.weight.prob.list) != 1)), " TRESO zones with multiple schools."))
...
simulated_compare <- readRDS("cache/trip_list.rds") %>%
  mutate(simulated_ade = trips * school.weight) %>%
  group_by(sfis, school.name, dsb.index) %>%
