...
travel_time1 <- travel_time %>%
  ...
  mutate(travel.time = ifelse((travel.time == 0 & treso.id.por == treso.id.pos), intra.zonal.travel.time, travel.time))
...
prop_matrix_FCS <- readRDS("output/prop_matrix_FCS.rds")
...
pos_travel_time <- bind_cols(df, select(treso_travel_time, travel.time)) %>%
  group_by(treso.id.pos) %>%
  summarise(avg.travel.time = weighted.mean(travel.time, enrolment.rounded)) %>%
