...
[] Validate if the distributed patterns matches the observed closest school patterns
...
ggsave(file = "output/tlfd_sample.png", plot_travel_time_tlfd(student_tlfd_90), height = 28, width = 10)
```
...
Intra-zonal travel time is assumed to be performed by walking at a speed of 5 km/hr over the euclidean distance between the student's postal code and school's postal code

Extremely long travel time is defined by any travel time that is 30 min longer than travelling the euclidean distance at 30 km/hr. If it is over 30 min, the OD pair use the euclidean travel time (30 km/hr) instead.
...
observed_trips <- readRDS("output/observed_por_pos_90.rds") %>%
  ...
  left_join(travel_time_skim, by = c('treso.id.por' = 'orig', 'treso.id.pos' = 'dest')) %>%
  # Change intra-zonal travel time to walk speed of 5km/hr
  # Cap the large travel times for zones up north to euclidean travel time assuming speeds of 30km/hr
  # TODO check with others regarding this
  mutate(euclidean.travel.time = euclidean.dist / 30 * 60) %>%
  ...
  mutate(value = ifelse(value == 0, (euclidean.dist / 5 * 60), value)) %>%
...
ggplot(data=combined_tlfd, aes(x=bin, y=enrolment, group=flag, color=flag)) +
  ...
  ggtitle(paste0("EP/ELEM - alpha: ", initalpha, " beta: ", initbeta))
...
simulated_trips <- readRDS(paste0("cache/simulated_trips.RDS")) %>%
  left_join(pos_school, by = "treso.id.pos")
  
  
f2 <- function(row) {
  x = row['sfis.list'][[1]]
  size = round(row['enrolment'][[1]], 0)
  ...
  print(paste0("Size of x is: ", length(x), " and size of prob is: ", length(prob)))
  results = sample(x, size=size, replace=TRUE, prob=prob)

  print(results)
}

apply(slice(simulated_trips, 1:10), 1, f2)


```

## Explore Closest School Probability Choice
...
```{r applying closest school probability}
...
observed_students <- readRDS("output/observed_por_pos_90.rds") %>%
  ...
  # The reason for empty list is the 25 meter buffer was not enough to capture the
...
f2 <- function(row) {
  ...
  results <- list(sample(x, size=size, replace=TRUE, prob=prob))
}
...
# TODO maybe including mof_region could further improve the results
...
for (i in c("English Catholic", "English Public", "French Catholic", "French Public")) {
  for (j in c("Elementary", "Secondary")) {
    ...
    school_student_assigned <- rbind(school_1, school_2, school_3, school_4, school_5) %>%
      group_by(sfis) %>%
      summarise(enrolment.assigned = sum(enrolment))
    
    result <- rbind(result, school_student_assigned)
