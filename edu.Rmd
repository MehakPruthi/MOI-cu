...
- [] Distribute TRESO zones with multiple schools based on EQAO schools
...
school_tb <- left_join(school_tb, select(eqao_2017, sfis, eqao.standardized), by = "sfis") %>%
  filter(school_tb, is.na(eqao.standardized))
...
results <- by(simulated_trips$enrolment, simulated_trips[c('treso.id.pos')], smart.round, simplify = FALSE)

# Convert the output of by() to dataframe
results2 <- t(sapply(results, I))
df <- data.frame(t(as.data.frame(results2)))
colnames(df) <- "enrolment.rounded"
df2 <- unnest(df, enrolment.rounded)

simulated_trips2 <- arrange(simulated_trips, treso.id.pos) %>%
  cbind(df2)

summary(simulated_trips2$diff)
...
plan(multiprocess)
results <- future_apply(simulated_trips2, 1, f2)
results_tb = t(as.data.table(results))
simulated_trips_schools <- cbind(simulated_trips2, results_tb)
```
