...
# Student travel data
student_travel_90 <- readRDS("cache/student_travel_0.9.rds") #TODO this rds is not updated to include manhattan distance
...
school_master <- readRDS("output/school_treso_master.rds") %>% 
  mutate(panel = ifelse(panel == "E", "Elementary", panel)) %>% 
  mutate(panel = ifelse(panel == "S", "Secondary", panel))
...
```{r}
trip_list_2011 %>% select(enrolment.rounded) %>% sum()
trip_list %>% select(enrolment.rounded) %>% sum()
trip_list_new %>% select(enrolment.rounded) %>% sum()
school_master %>% 
  filter(year ==  2011, board_type_name == "English Catholic", panel == "Elementary") %>% 
  filter(!(otg == 0 & ade == 0)) %>% 
  filter(ade / otg <=2) %>% 
  select(ade) %>% 
  sum()

trip_list %>% filter(treso.id.por == 6879, enrolment.rounded > 1) %>% arrange(desc(enrolment.rounded))
trip_list_new %>% filter(treso.id.por == 6879, enrolment.rounded > 1) %>% arrange(desc(enrolment.rounded))
```
...
```{r sample treso population in 2017}
observed_2011_treso <- readRDS("cache/treso_tb.rds") %>% 
  select(treso_zone, n_ele_pop, n_sec_pop) %>% 
  gather(key = "panel", value = "enrolment", n_ele_pop, n_sec_pop) %>% 
  filter(enrolment > 0) %>% 
