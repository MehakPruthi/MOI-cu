...
gaf <- 1 # Geographic adjustment factor for specific municipalities on a postal code basis - to be looked up if possible 

# Extract relevant area per ADE
sqm_per_student <- area_per_ade %>% 
  filter(OTG == user_input_school_size) %>%
  mutate(sqm = ifelse(user_input_panel == 'Elementary', elementary_sqm_pp, secondary_sqm_pp)) %>% 
  summarize(sqm = first(sqm)) %>% 
  pull()

cost_rate <- area_rates %>% 
  filter(panel == user_input_panel) %>% 
  summarize(rate = first(cost_per_sqm)) %>% 
  pull()

new_school_cost <- round(cost_rate * sqm_per_student * user_input_school_size * gaf, 0)
