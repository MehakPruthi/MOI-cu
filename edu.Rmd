...
# Save the balanced POR-POS trip list
trip_list_2011 <- melt(prop_matrix2_2011) %>%
  arrange(Var1, Var2)
...
# Save the balanced POR-POS trip list
trip_list_new <- melt(prop_matrix2_new) %>%
  arrange(Var1, Var2)
...
## Creating TRESO 2017 POR Data

```{r sample treso population in 2017, include = FALSE}
observed_2011_treso <- readRDS("cache/treso_tb.rds") %>% 
  select(treso_zone, n_ele_pop, n_sec_pop) %>% 
  gather(key = "panel", value = "enrolment", n_ele_pop, n_sec_pop) %>% 
  filter(enrolment > 0) %>% 
...
# Observed school demand for scaling
school_sfis_2017 <- readRDS("output/school_treso_master.rds") %>%
  filter(year == 2017) %>% 
  filter(zero_otg_flag == 0 | excessive_util_flag == 0) %>%
  mutate(utilization = ifelse(otg != 0, ade / otg, 0))

#Scale TRESO sampled enrollemnt to observed school demand by board type and panel
board_panel_summary_2017 <- school_sfis_2017 %>%
  group_by(board_type_name, panel)%>%
  summarise (ade.sum = sum(ade)) 

treso_level_sampling_2017_sum <- treso_level_sampling_2011 %>%
  group_by(panel)%>%
  summarise(`English Catholic` = sum(`English Catholic`), 
            `English Public` = sum(`English Public`),
            `French Catholic` = sum(`French Catholic`),
            `French Public` = sum(`French Public`)) %>% 
  gather(key="board_type_name", value="enrolment.sum", `English Catholic`:`French Public`) %>%
  left_join(board_panel_summary_2017, by=c("panel", "board_type_name")) %>%
  mutate(ratio = ade.sum/enrolment.sum)

treso_por_2017 <- select(treso_level_sampling_2011, treso.id.por, panel, csduid, csdname, 
                         `English Catholic`:`French Public`) %>%
  gather(key="board_type_name", value="enrolment", `English Catholic`:`French Public`) %>%
  left_join(select(treso_level_sampling_2017_sum, panel, board_type_name, ratio), by=c("panel", "board_type_name")) %>%
  mutate(enrolment.scaled = enrolment * ratio) %>%
  select(treso.id.por, enrolment.scaled, panel, board_type_name) 
saveRDS(treso_por_2017, "cache/treso_por_2017.rds")
```
...
for (board_id in school_board_list) {
  for (panel_id in panel_list) {
    ...
    # Create por vector
    por <- readRDS(paste0("cache/treso_por_", year_id, ".rds")) %>%
        filter(panel == panel_id, board_type_name == board_id) %>%
        select(treso.id.por, enrolment.scaled) %>%
        right_join(por_full, by=c("treso.id.por" = "orig")) %>%
        replace_na(list(enrolment.scaled = 0)) %>%
    ...
    trip_list <- melt(prop_matrix_balanced) %>%
        arrange(Var1, Var2)
    colnames(trip_list) <- c("treso.id.por", "treso.id.pos", "trips")
...
### Step 4 - Apply the difference to 20xx to obtain forecast ADE

### Step 5 - Using the forecast ADE, run 2D constrained distribution. Iterate the distribution until no school is above the OTG threshold
...
# a.	If net new OTG (per CSD) is less than the minimum required for a new school, add OTG to existing school(s) in CSD, or if no school in CSD, don’t build school and ignore ‘need’ for more capacity
# b.	If net new OTG is negative but doesn’t justify closing a school, do nothing
...
# TODO: Replace tibble with actual OTG per CSD table
otg_per_csd <- tibble(csduid = c(1000, 1001, 1002, 2124, 2981),
                          otg = c(12000, 4000, 1000, 500, 3000))

# Set minimum and maximum school size
school_sizes <- tibble(bound = c('min', 'max'), elementary = c(200, 1200), secondary = c(300, 1700)) 
