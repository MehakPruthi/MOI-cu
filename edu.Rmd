...
treso_distance <- readRDS("cache/treso/treso_distance_2016.rds")
...
school_master <- readRDS("cache/edu/school_treso_master.rds") %>% 
  replace_na(list(zero_otg_flag = 1)) %>% 
  filter((zero_otg_flag == 0 & excessive_util_flag == 0), (status == "Open" | is.na(status))) %>% 
  # Portables are not forecasted, give values of NA to portable.units and capacity.portable and fill from 2017 onwards
  mutate(portable.units = replace(portable.units, year > 2017, NA),
         capacity.portable = replace(capacity.portable, year > 2017, NA)) %>% 
  arrange(sfis, year) %>% 
  fill(portable.units, capacity.portable)
...
# Percentage of current OTG to be set as maximum capacity (inclusive of portables) in model
USER_OTG_THRESHOLD = 1.1

#Percentage of OTG beyond which demand is served by portables
PORTABLE_THRESHOLD = 0.8

# Define portable size
ELE_STUDENTS_UNIT_CLASS = 23
SEC_STUDENTS_UNIT_CLASS = 21

# Target utilization for future year
...
school_base <- school_master %>% 
  filter(year == base_year, panel == panel_id, board_type_name == board_id) %>% 
        mutate(otg = otg * PORTABLE_THRESHOLD) %>%  # Update OTG to be effective OTG
  mutate(is.closed = 0)

# Percentage of students attending each board type, by home origin CD
# The market share begins from 2018, just using that as the base year application
board_type_sample_spread <- readRDS("cache/edu/board_type_sample_spread.rds") %>%
  filter(year == base_year)

treso_forecast_population <- readRDS("input/edu/treso_population_edu.rds") %>% 
  select(treso.id.por = treso_zone, panel = age.group, csduid, cduid, 
         potential.enrolment = paste0("population.", base_year)) %>%
  filter(panel != "other")

# Initialize new school dataframe
new_school <- tibble(treso.id.pos = integer(), panel = character(), board_type_name = character(), year = integer(), 
                     build.priority = integer(), sfis.temp = integer(), otg = integer())
...
por <- create_por_forecast_vector(treso_forecast_population_board, por_full, panel_id, board_id)

# Produce the simulated ADE for each school
school_summary_2017 <- distribute_students_to_schools(school_base, new_school=NULL, por, pos_full, prop_matrix,
                                                      treso_travel_time=NULL, treso_distance=NULL, is_weight_continuous=TRUE)[[1]]
...
number_of_zero_simulated_schools <- school_base %>% 
...
- The year is set by user

```{r application of gravity model - Step 2}
school_20xx <- school_master %>% 
  filter(year == year_id, panel == panel_id, board_type_name == board_id) %>% 
  mutate(otg = otg * PORTABLE_THRESHOLD) %>%  # Update OTG to be effective OTG
  mutate(is.consolidated = 0)

# Identify the schools which are projected to be closed by the ministry between 2017 and forecast year
schools_closed <- select(school_base, sfis) %>% 
  anti_join(select(school_20xx, sfis), by = c('sfis')) %>% 
  mutate(is.closed.update = 1)

if (nrow(schools_closed) != 0) {
  school_base <- school_base %>%
    left_join(schools_closed, by = c('sfis')) %>% 
    mutate(is.closed = ifelse(is.na(is.closed.update), is.closed, is.closed.update)) %>% 
    select(-is.closed.update)
}
...
treso_forecast_population <- readRDS("input/edu/treso_population_edu.rds") %>% 
  ...
  filter(panel != "other") %>% 
  mutate(cduid = as.integer(substr(csduid, 1, 4)))
...
treso_forecast_population_board <- apply_sampling_to_population(forecast_population = treso_forecast_population,
                                                                board_type_sample = board_type_sample_spread)
if (FORECAST_METHOD == "EDU") {
  # Scale the student population to EDU forecast if desired
  edu_forecast_sum <- sum(school_20xx$ade)
  treso_forecast_sum <- sum(filter(treso_forecast_population_board, panel == panel_id, board_type_name == board_id)$enrolment)
  scale_ratio = edu_forecast_sum / treso_forecast_sum
  
  print(paste0("Scaling TRESO forecast from - ", round(treso_forecast_sum, 0), " to EDU forecast of - ", round(edu_forecast_sum, 0)))
  
  treso_forecast_population_board <- treso_forecast_population_board %>% 
    mutate(enrolment = ifelse((panel == panel_id & board_type_name == board_id), round(enrolment * scale_ratio, 0), enrolment))
}

por <- create_por_forecast_vector(treso_forecast_population_board, por_full, panel_id=panel_id, board_id=board_id)

result <- distribute_students_to_schools(school_20xx, new_school=NULL, por, pos_full, prop_matrix, 
                                         treso_travel_time=NULL, treso_distance=NULL, is_weight_continuous=TRUE)
school_summary_20xx <- result[[1]]
prop_matrix_balanced <- result[[2]]
...
school_forecast <- calculate_delta_between_simulated_scenarios(school_base, school_20xx, school_summary_2017, school_summary_20xx,
                                                               new_school=NULL, USER_OTG_THRESHOLD)
```
...
# Rerun steps 3, 4 until treshoold (takes about 55 iterations for 100 threshold)
df_list <- distribution_model(school_base, school_20xx, school_summary_2017, school_summary_20xx, prop_matrix,
                              prop_matrix_balanced, new_school=NULL, pos_full,
                              overfill_threshold = 100, max_iter=2, USER_OTG_THRESHOLD, capacity_constrained = CAPACITY_CONSTRAINED)

school_forecast_final <- df_list[[1]]
por_additional <- df_list[[2]]
```

### Step 5 - Run 20xx Forecast Model with 2D constrained to the final ADE values to obtain OD pairs
...
print(paste0("The total ADE assigned is: ", sum(por), ". The total OTG available is: ", sum(pos), "."))

# 2D balancing, scale the total to the POR side to determine final distribution to TRESO zones
prop_matrix_balanced <- matrix_balancing_2d(prop_matrix, por, pos, totals_to_use="rows", max_iterations = 10)
...
result <- forecast_school_ade(prop_matrix, trip_list, school_20xx, eqao_2017, new_school,
                              treso_travel_time, treso_distance)
school_summary_20xx_final <- result[[1]]
pos_travel_time <- result[[2]]
pos_travel_distance <- result[[3]]

test <- left_join(school_summary_20xx_final, school_20xx, by="sfis") %>%
  left_join(pos_travel_time, by="treso.id.pos") %>% 
  left_join(pos_travel_distance, by="treso.id.pos") %>% 
  select(sfis, simulated.ade, treso.id.pos, ade, otg, sim.avg.travel.time, sim.avg.travel.distance) 

```

## Decision-Making Layer
### Determine high priority TRESO zones 

- TRESO zones should be considered for new schools as a function of 'excessive' travel time and population

```{r high prioirty treso zones}
# Cache results to save run time
saveRDS(school_base, "cache/edu/school_base.rds")
saveRDS(school_20xx, "cache/edu/school_20xx.rds")
saveRDS(trip_list, "cache/edu/trip_list.rds")
saveRDS(por_additional, "cache/edu/por_additional.rds")
saveRDS(school_forecast_final, "cache/edu/school_forecast_final.rds")
saveRDS(school_summary_20xx_final, "cache/edu/school_summary_20xx_final.rds")
saveRDS(school_summary_20xx, "cache/edu/school_summary_20xx.rds")
saveRDS(school_summary_2017, "cache/edu/school_summary_2017.rds")
saveRDS(por, "cache/edu/por.rds")
saveRDS(pos, "cache/edu/pos.rds")
saveRDS(prop_matrix, "cache/edu/prop_matrix_segmented.rds")
saveRDS(prop_matrix_balanced, "cache/edu/prop_matrix_balanced_segmented.rds")

school_base <- readRDS("cache/edu/school_base.rds")
school_20xx <- readRDS("cache/edu/school_20xx.rds")
trip_list <- readRDS("cache/edu/trip_list.rds")
por_additional <- readRDS("cache/edu/por_additional.rds")
school_forecast_final <- readRDS("cache/edu/school_forecast_final.rds")
school_summary_20xx_final <- readRDS("cache/edu/school_summary_20xx_final.rds")
school_summary_20xx <- readRDS("cache/edu/school_summary_20xx.rds")
school_summary_2017 <- readRDS("cache/edu/school_summary_2017.rds")
por <- readRDS("cache/edu/por.rds")
pos <- readRDS("cache/edu/pos.rds")
prop_matrix <- readRDS("cache/edu/prop_matrix_segmented.rds")
prop_matrix_balanced <- readRDS("cache/edu/prop_matrix_balanced_segmented.rds")

DECISION_MAKING_FLAG <- TRUE

# Determine potential TRESO zones in which to build schools
df_list <- edu_dm(treso_travel_time, trip_list, treso_zone_def, por_additional, 
                  TRAVEL_TIME_THRESHOLD_FACTOR, ZONE_PROXIMITY_THRESHOLD, MIN_TT_THRESHOLD)

# Temporarily set OTG for proposed schools being added to zones with existing schools in order to calculate 
# an appropriate weight factor. OTG to be set to average of existing schools in that zone.
treso_otg_avg <- school_20xx %>% 
  group_by(treso.id.pos) %>%
...
new_school <- df_list[[1]] %>% 
  ...
  # Join with TRESO centroid to get postal code
  left_join(treso_centroids, by=c("treso.id.pos"="treso_id")) %>% 
  rename(school.lat = lat, school.long = long) %>% 
  left_join(treso_otg_avg, by = c('treso.id.pos')) %>% 
  rename(otg = avg.otg) %>% 
  # If there are no existing schools in the TRESO zone, assign a default value of 1 to that new school
  mutate(otg = replace_na(otg, 1)) # OTG must be > 0 for some distribution functions to run

print(paste0("The number of new schools identified is: ", nrow(new_school)))

# Create list of TRESO zones ruled out from consideration due to proximity to higher-ranked zone
proximity_df <- df_list[[2]] 

# Distribute the students to see where they choose each school, new and existing; without constraining the capacity
school_summary_20xx <- distribute_students_to_schools(school_20xx, new_school, por, pos_full, prop_matrix,
                                                      treso_travel_time=NULL, treso_distance=NULL, is_weight_continuous=FALSE)[[1]]

# Run Steps 3 to get adjusted simulated ADE for each school (stored under school_forecast dataframe)
school_forecast <- calculate_delta_between_simulated_scenarios(school_base, school_20xx, school_summary_2017, school_summary_20xx, 
                                                               new_school, USER_OTG_THRESHOLD)

last_run_step <- 'Determine which TRESO zones should new schools.'
```
...
ade_threshold <- school_sizes %>% 
  filter(panel == panel_id) %>% 
  select(min.otg) %>% 
  pull()

# Identify new schools with enough simulated ADE to justify building
new_school_potential <- school_forecast %>%
  # Filter out existing schools
  filter(sfis > 99000) %>% 
  filter(simulated.ade >= ade_threshold)

# Keep the new_school worth building in an temporary dataframe
new_school_iteration <- new_school %>% 
  right_join(select(new_school_potential, sfis), by = c('sfis'))

print(paste0("The number of new schools after threshold is: ", nrow(new_school_iteration)))

# Reassign ADE after removing certain new schools
school_summary_20xx <- distribute_students_to_schools(school_20xx, new_school_iteration, por, pos_full, prop_matrix,
                                                      treso_travel_time=NULL, treso_distance=NULL, is_weight_continuous=FALSE)[[1]]

# Run Steps 3 to get adjusted simulated ADE for each school (stored under school_forecast dataframe)
school_forecast <- calculate_delta_between_simulated_scenarios(school_base, school_20xx, school_summary_2017, school_summary_20xx,
                                                               new_school_iteration, USER_OTG_THRESHOLD)
last_run_step <- 'Delete new schools with ADE < Threshold.'
```
...
```{r}
CONSOLIDATE_SCHOOLS_FLAG <- TRUE

# Check utilization of existing schools; flag underutilized schools for consolidation
schools_consolidated <- school_forecast %>% 
  mutate(simulated.utilization = simulated.ade / otg) %>% 
  mutate(is.consolidated.update = ifelse((simulated.utilization < CONSOLIDATION_THRESHOLD) &
                                           (sfis <= 100000), 1, 0)) %>% 
  filter(is.consolidated.update == 1) %>% 
  select(sfis, is.consolidated.update)

print(paste0("Algorithm consolidated ", nrow(schools_consolidated), " schools."))

if (nrow(schools_consolidated) != 0) {
  school_20xx <- school_20xx %>% 
    left_join(schools_consolidated, by = c('sfis')) %>% 
    mutate(is.consolidated = ifelse(is.na(is.consolidated.update), is.consolidated, is.consolidated.update)) %>% 
    select(-is.consolidated.update)
}

school_summary_20xx <- distribute_students_to_schools(filter(school_20xx, is.consolidated == 0), new_school_iteration,
                                                      por, pos_full, prop_matrix, treso_travel_time=NULL, treso_distance=NULL,
                                                      is_weight_continuous = FALSE)[[1]]
...
###	Set OTG at new schools to be a function of unconstrained ADE
...
# Determine number of new schools required per TRESO zone, adding >1 school where OTG > max school size 
max_otg <- school_sizes %>% 
  filter(panel == panel_id) %>% 
  select(max.otg) %>% 
  pull()

# Revise list of new schools to have only the previously identified new schools which exceed the minimum ADE threshold
new_school_iteration <- new_school_iteration %>% 
  right_join(select(new_school_potential, sfis, simulated.ade), by = c('sfis')) %>% 
  mutate(otg = simulated.ade) %>%
  # Calculate number of new schools needed per TRESO zone
  mutate(schools.needed = ceiling(otg / max_otg))

# Expand list of new schools to break schools exceeding max OTG into smaller schools within the TRESO zone
new_school_iteration <- new_school_iteration[rep(seq_len(nrow(new_school_iteration)), new_school_iteration$schools.needed),
                                             1:ncol(new_school_iteration)]

new_school_iteration <- new_school_iteration %>% 
  mutate(otg = round(otg / schools.needed)) %>% 
  group_by(treso.id.pos) %>% 
  mutate(count.per.treso = 1:n()) %>% 
  ungroup() %>% 
  mutate(sfis = sfis + count.per.treso,
         school.lat = school.lat + (count.per.treso / 100),
         school.long = school.long + (count.per.treso / 100)) %>% 
  select(-count.per.treso, -schools.needed, -simulated.ade)

print(paste0("The number of new schools after setting OTG to min(ADE, max_otg): ", nrow(new_school_iteration)))
...
# Rerun steps 3, 4 until threshold (takes about 55 iterations for 100 threshold)
school_forecast <- distribution_model(school_base, school_20xx, school_summary_2017, school_summary_20xx, 
                                      prop_matrix, prop_matrix_balanced, new_school_iteration, pos_full,
                                      overfill_threshold=2000, max_iter=5, USER_OTG_THRESHOLD, capacity_constrained = TRUE)[[1]]
...
trip_length_check <- cbind(trip_list, select(treso_travel_time, value)) %>%
  rename(travel.time = value) %>%
  filter(travel.time > 60) %>% 
  # Filter out any zone pairs with fewer than 5 trips occuring between zones
  filter(trips > 5) %>% 
  group_by(treso.id.por) %>% 
  mutate(total.por.trips = sum(trips)) %>% 
  ungroup() %>% 
  # Filter out any home (origin) with fewer than 10 trips originating there
  filter(total.por.trips > 10)
  
# Determine unique school zones to unconsolidate for any schools in trip_length_check POR
unconsolidation_list_por <- trip_length_check %>% 
  distinct(treso.id.por) %>% 
  rename(treso.id.pos = treso.id.por)

# Combine treso.id.por list with treso.id.pos list as potential zones with schools to unconsolidate
unconsolidation_list <- trip_length_check %>%
  select(treso.id.pos) %>% 
  rbind(., unconsolidation_list_por) %>% 
  distinct(treso.id.pos) %>% 
  mutate(unconsolidate.zone = 1)
  
school_20xx <- school_20xx %>% 
  left_join(select(unconsolidation_list, treso.id.pos, unconsolidate.zone), by = c('treso.id.pos')) %>% 
  mutate(is.consolidated = ifelse(is.na(unconsolidate.zone), is.consolidated, 0)) %>% 
  select(-unconsolidate.zone)
...
# Rerun step 4 until threshold (takes about 55 iterations for 100 threshold)
school_forecast <- distribution_model(school_base, school_20xx, school_summary_2017, school_summary_20xx, 
                                      prop_matrix, prop_matrix_balanced, new_school_iteration, pos_full, 
                                      overfill_threshold=20000, max_iter=10, USER_OTG_THRESHOLD, capacity_constrained=TRUE)[[1]]

school_forecast <- school_forecast %>% 
  left_join(select(school_20xx, sfis, is.consolidated), by = c('sfis')) %>% 
  # Add an 'is.consolidated' flag for new schools, set to 0, to avoid N/A values below
  mutate(is.consolidated = replace_na(is.consolidated, 0)) %>% 
  # Add a flag to see if each TRESO zone has at least one school open
  mutate(is.open = 1 - is.consolidated) %>% 
  group_by(treso.id.pos) %>% 
  mutate(treso.open = min(sum(is.open), 1)) %>% 
  ungroup() %>% 
  mutate(simulated.ade = treso.open * simulated.ade) %>% 
  select(-is.consolidated, -is.open, -treso.open)
...
# Produce the simulated ADE for each school within each TRESO zone
school_summary_20xx_final <- forecast_school_ade(prop_matrix, trip_list, filter(school_20xx, is.consolidated == 0), 
                                                 eqao_2017, new_school_iteration, treso_travel_time=NULL, treso_distance=NULL)[[1]]
...
### Calculate number of portables needed by school
```{r}
# Calculate number of students in portables and number of portables
if (panel_id == 'Elementary') {
  portable_class_size <- ELE_STUDENTS_UNIT_CLASS
} else if (panel_id == 'Secondary') {
  portable_class_size <- SEC_STUDENTS_UNIT_CLASS
}

school_summary_20xx_final <- school_summary_20xx_final %>% 
  left_join(select(school_20xx, sfis, otg), by = c('sfis')) %>% 
  left_join(select(new_school_iteration, sfis, otg), by = c('sfis')) %>% 
  mutate(otg.x = replace_na(otg.x, 0),
         otg.y = replace_na(otg.y, 0),
         otg = otg.x + otg.y) %>% 
  select(-otg.x, -otg.y) %>% 
  mutate(portable.ade = pmax(simulated.ade - otg * PORTABLE_THRESHOLD, 0), 
         portable.count = ceiling(portable.ade / portable_class_size))
```

### Plot Results from DM

```{r}
# Read in the school forecast dataframe
#school_forecast_final_treso_id <- readRDS("cache/edu/school_forecast_final_final.rds") %>% 
school_forecast_id <- school_forecast %>% 
  filter(sfis > 99999) %>% 
  distinct(treso.id.pos)

treso_forecast_population_2017 <- readRDS("input/edu/treso_population_edu.rds") %>% 
  select(treso.id.por = treso_zone, panel = age.group, csduid, cduid, 
         potential.enrolment = paste0("population.", base_year)) %>%
  filter(panel != "other")

treso_forecast_population_2025 <- readRDS("input/edu/treso_population_edu.rds") %>% 
  select(treso.id.por = treso_zone, panel = age.group, csduid, cduid, 
         potential.enrolment = paste0("population.", year_id)) %>%
  filter(panel != "other")

# Apply the board sampling probabilities to the student population from TRESO
treso_forecast_population_board_2017 <- apply_sampling_to_population(forecast_population = treso_forecast_population_2017,
                                                                     board_type_sample = board_type_sample_spread) %>% 
  filter(panel == panel_id, board_type_name == board_id)

treso_forecast_population_board_2025 <- apply_sampling_to_population(forecast_population = treso_forecast_population_2025,
                                                                     board_type_sample = board_type_sample_spread) %>% 
  filter(panel == panel_id, board_type_name == board_id)

treso_forecast_population_board_diff <- full_join(treso_forecast_population_board_2017,
                                                  treso_forecast_population_board_2025,
                                                  by=c("treso.id.por", "panel",
                                                       "board_type_name", "cduid")) %>% 
  mutate(potential.enrolment.diff = potential.enrolment.y - potential.enrolment.x,
         potential.enrolment.pct = potential.enrolment.diff / potential.enrolment.x,
         potential.enrolment.pct.ln = log1p(potential.enrolment.pct))


# Convert the SpatialDataframe to normal Dataframe for ggplot()
treso_shp@data$id <- treso_shp@data$Treso_ID
treso_shp_df <- fortify(treso_shp, region="id") %>%
  rename(
    x = long,
    y = lat,
  ) %>% 
  mutate(id = as.integer(id)) %>% 
  left_join(treso_zone_def, by=c("id"="treso_id")) %>% 
  left_join(treso_forecast_population_board_diff, by=c("id"="treso.id.por")) %>% 
  filter(csdname == "Toronto")

treso_shp_df_schools <- treso_shp_df %>% 
  right_join(school_forecast_final_treso_id, by=c("id"="treso.id.pos"))

g <- ggplot() +
  geom_polygon(data = treso_shp_df, mapping = aes(x = x, y = y, group = group,
                                                  fill = potential.enrolment.diff),
               color = "grey90", alpha = 0.8, size = 0.001) +
  geom_polygon(data = treso_shp_df_schools, mapping = aes(x = x, y = y, group = group),
               color = "red", alpha = 0.1, size = 0.001) +
  labs(x = "", y = "", title = "New Schools and Population Growth in Ontario") +
...
student_xy <- create_student_xy(student_travel_90, treso_shp)
school_xy <- create_school_xy(student_travel_90, treso_shp)
...
for (dist in percentile_dist) {
  ...
  student_xy <- create_student_xy(student_travel, treso_shp)
  school_xy <- create_school_xy(student_travel, treso_shp)
...
treso_travel_time <- readRDS("cache/treso/treso_travel_time.rds")
...
ssaveRDS(school_travel_time, "output/edu/school_travel_2017.rds")
```
...
ade_dsb_panel_sum <- ade_dsb_hist %>%
  select(year, dsb.index, panel, ade) %>%
  filter(year <= 2017) %>%
...
school_population_historical <- read.csv('input/mag/MOF_POP.csv') %>%
  select(year = Year,
         age = Age,
         sex = Sex,
         pop = Population,
         cduid = SGCId) %>%
  filter(year > 2006, year < 2016, age < 19, age > 3, sex == 3) %>%
  mutate(panel = ifelse(age < 14, 'Elementary', 'Secondary')) %>%
  group_by(year, cduid, panel) %>%
  summarise(population = sum(pop)) %>%
  ungroup()

school_population <- readRDS('input/edu/treso_population_edu.rds') %>% 
  filter(age.group != "other") %>% 
  group_by(cduid, age.group) %>% 
  summarise_at(vars(contains("population")), list(sum = sum)) %>% 
  ungroup() %>% 
  rename(panel = age.group) %>% 
  pivot_longer(cols = contains("population"),
               names_to = "year",
               values_to = "population") %>% 
  separate(year, c("pop", "year", "sum")) %>% 
  mutate(year = as.numeric(year)) %>% 
  select(-pop, -sum)

school_population <- rbind(school_population, school_population_historical)
...
panel_cd_mrktshr <- ade_panel_mrktshr %>%
  ...
  mutate(share = ade.scaled / population) %>%
  select(-ade, -population)
...
brd_cd_mrktshr <- ade_panel_mrktshr %>%
  left_join(school_population, by = c('year', 'cduid', 'panel')) %>%
  mutate(share = ade.scaled / population) %>%
  select(-ade, -population, -ade.scaled) %>%
...
# Join with 2017 base market share values
brd_cd_mrktshr_2017 <- brd_cd_mrktshr %>% 
  select(cduid, panel, dsb.index, mrktshr_yrx = `2017`) %>% 
  mutate(year = 2017)

brd_cd_mrktshr_proj <- rbind(brd_cd_mrktshr_proj, brd_cd_mrktshr_2017)

## remove temp tables
rm(brd_cd_mrktshr_chg_raw1, brd_cd_mrktshr_chg_raw2, brd_cd_mrktshr_proj_in,brd_cd_mrktshr_ext,
   cd_panel_proj_mrktshr, brd_cd_mrktshr_2017)
...
area_per_otg <- read.csv("input/edu/school_cost_benchmark_areas.csv", stringsAsFactors = FALSE)
...
new_school_list <- tibble(otg = c(500, 750), 
                          panel = c('Elementary', 'Secondary'), 
                          postal.code = c('k0b1e4', 'P6A5K1'))

# Convert area_per_otg to be long format
area_per_otg_long <- area_per_otg %>% 
  rename(otg = OTG, Elementary = elementary_sqm_pp, Secondary = secondary_sqm_pp) %>% 
...
new_school_list_area <- new_school_list %>% 
  left_join(area_per_otg_long, by = c('otg', 'panel')) %>% 
  mutate(total_area = otg * sqm_pp) %>% 
  left_join(area_rates, by = c('panel'))
...
total_cost <- sum(annualized_inflated_cost$annual_cost_inflated)
```
