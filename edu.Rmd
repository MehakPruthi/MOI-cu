...
saveRDS(simulated_trips, "cache/simulated_trips.rds")
```
...
simulated_trips <- readRDS("cache/simulated_trips.rds")
...
pos_school_weight_2 <- left_join(pos_school, pos_school_weight, by = "treso.id.pos") %>%
...
pos_school_weight_2 <- pos_school_weight_2 %>%
...
print(paste0("There are ", nrow(filter(pos_school_weight_2, length(school.weight.prob.list) == 1)), " TRESO zones with a single school, and ", nrow(filter(pos_school_weight_2, length(school.weight.prob.list) != 1)), " TRESO zones with multiple schools."))

trip_list <- readRDS(paste0("cache/trip_list.RDS")) %>%
  left_join(pos_school_weight_2, by = "treso.id.pos")
...
colnames(results2) <- colnames(prop_matrix2)
rownames(results2) <- rownames(prop_matrix2)

df <- results2 %>%
  melt()
colnames(df) <- c('treso.id.por', 'treso.id.pos', 'enrolment.rounded')

trip_list2 <- bind_cols(trip_list, df) %>%
...
saveRDS(trip_list_schools, "output/trip_list_schools.rds")
```

Compare simulated ADE to 2017 ADE by school

```{r simulated to observed comparison by school}

school_sfis_2017 <- readRDS("cache/school_sfis_2017.rds") %>%
  left_join(select(school_board_def, dsb, board_type_name), by = c("dsb.index" = "dsb")) %>%
  filter(panel == "Elementary", board_type_name == "English Public")

simulated_compare <- readRDS("output/trip_list_schools.rds") %>%
  mutate(simulated_ade = enrolment.rounded * school.weight.prob.list) %>%
  group_by(sfis.list, school.name.list, dsb.index.ist) %>%
  summarise(simulated_ade = sum(simulated_ade)) %>%
  left_join(select(school_sfis_2017, sfis, ade), by = c("sfis.list" = "sfis")) %>%
  rename(ade_2017 = ade) %>%
  mutate(ade_2017_to_sim_diff = ade_2017 - simulated_ade)%>%
  mutate(perc_ade_diff = (simulated_ade - ade_2017)/ade_2017) %>%
  mutate(rounded_perc = round(perc_ade_diff, 2))
  
avg_perc <- mean(simulated_compare$rounded_perc)
