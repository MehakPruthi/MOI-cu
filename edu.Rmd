...
# TRESO time skim
travel_time_skim <- read_csv("input/mf224.csv.gz") %>%
  melt(id = c("p/q/[val]")) %>%
...
# fill travel times of NA with 0, 1e20 with 0; and replace with assumed travel speed of:
# - 30KM/hr (if euclidean > 2KM)
# - 5km/hr (if euclidean <= 2KM)
student_tlfd_90 <- student_tlfd_90 %>%
  mutate(travel.time = replace_na(travel.time, 0)) %>%
  mutate(travel.time = replace(travel.time, travel.time >= 1e10, 0)) %>%
  mutate(travel.time = ifelse(travel.time == 0, ifelse(euclidean.dist <= 2, euclidean.dist / 5 * 60, euclidean.dist / 30 * 60), travel.time)) %>%
  ...
  mutate(compare.travel.time = abs(travel.time - euclidean.travel.time)) %>%
