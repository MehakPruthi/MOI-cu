...
```{r}
# TODO: Ask Nour if we can delete the hqo code below which appears to not be used
...
HBAMprojected_alos <- HBAMprojected_master_tz %>%
  ...
  select(year, id, caretype, agecluster, alos)

# Remove 2016 ALC from 2016 average length of stay in order to estimate actual length of stay without ALC
HBAMprojected_adj_alos <- HBAMprojected_alos %>% 
  # TODO: Before completing the join below, must add caretype to ALC_proj_hosp_2016 data
  left_join(ALC_proj_hosp_2016, by = c('year', 'id', 'agecluster', 'caretype')
...
treso_origin_cases <- HBAMhistorical_origcsd_cases %>% 
  ...
  left_join(treso_population_long, by = c('year', 'orig_csd' = 'csduid', 'agegroup' = 'age_group', 'agecluster' = 'age_cluster')) %>% 
  rename(orig.treso.zone = treso_zone,
         orig.treso.pop.agecluster = treso.population.agecluster) %>%
  select(-agegroup, -population) %>%
  distinct() %>% 
  # Divide market share from CSDs into TRESO zones
  group_by(year, id, caretype, agecluster, orig_csd) %>% 
  mutate(treso.mrkt = orig.treso.pop.agecluster / sum(orig.treso.pop.agecluster) * csd.mrkt,
         treso.hosp.cases = treso.mrkt * csd.hosp.cases) %>%
  mutate(treso.caserate = ifelse(treso.hosp.cases > 0, treso.hosp.cases / orig.treso.pop.agecluster, 0)) %>% 
  ungroup() %>%
  drop_na(orig.treso.zone) %>% # A few CSDs don't appear to exist in TRESO, but very small case volumes so removed for simplicity
  select(id,
         hospitalname,
         hosp.treso.zone,
         caretype,
...
ALC_hosp_vol_disag <- ALC_madd_clean_disag %>%
  ...
  ungroup()
  # This code checks for number of hospitals when incorporated into ALC_hosp_vol - can be removed if code working
...
ALC_hosp_siteid <- ALC_hosp_vol_disag %>% 
...
# TODO: Calculate ALC by caretype based on the above
ALC_hosp_caretype <- ALC_hosp_siteid %>% 
  

#ALC VOLUMES FOR INDIVIDUAL HOSPITALS BY AGE CLUSTER
ALC_hosp_vol <- ALC_hosp_siteid %>% 
...
# TODO: Ask Nour: Do we need this query? Not used anywhere.
...
new_ltc_cap_csd <- tibble(csduid = c(3506008, 3519049), added.ltc.cap = c(5000, 8000))
...
ALC_proj_cd <- ALC_proj_csd %>% 
  group_by(cduid) %>% 
  summarize(ltc.cd.demand.days = sum(ltc.csd.demand.days), 
