...
num_beds <- num_beds_siteid %>% 
  ...
  mutate(staffedbeds = sum(maxstaffedbeds)) %>%
...
hospital_lookup_ALC <- hospital_lookup_master %>% 
  ungroup() %>% 
  select(id, pcode, csduid, csdname, -year) %>% 
  distinct() %>% 
...
ALC_madd_raw <- readRDS('cache/moh/ALC_madd_raw.rds') %>% 
  filter(madd == 'LTC', year != 2011, year != 2017)
...
ALC_lhin_vol <-  ALC_madd_raw %>% 
  filter(madd == 'LTC', agegroup == 9, year != 2011, year != 2017, number < 15) %>% 
...
ALC_hosp_vol_ag <- ALC_madd_raw %>% 
  filter(madd == 'LTC', number > 999, agegroup == 9, year != 2011, year != 2017) %>% 
...
#   filter(year == 2016) %>% 
#   group_by(csduid) %>% 
#   mutate(csd.alc.rate = csd.demand.days / sum(treso.population.agecluster)) %>% 
#   mutate(cduid = substr(csduid, 1, 4)) %>% 
#   group_by(cduid) %>% 
#   mutate(cd.demand.days = sum(treso.csd.dem.days),
#          cd.alc.rate = cd.demand.days / sum(treso.population.agecluster)) %>% 
#   ungroup() %>% 
#   select(treso_zone, csduid, csdname, cduid, agecluster, csd.alc.rate, cd.alc.rate)

#CALCULATE PROVINCIAL RATE OF DEMAND / 65+
ALC_csd_rates <- treso_population_long %>%
  filter(age_cluster != 'ped', year > 2011, year < 2017) %>% 
  mutate(cduid = substr(csduid, 1, 4)) %>% 
  group_by(year, cduid, csduid) %>% 
  summarise(csd.population = sum(population)) %>% 
  left_join(select(ALC_csd_vol, year, csduid, csd.demand.patients, csd.demand.days), by = c('year', "csduid")) %>% 
  mutate(csd.demand.patients = replace_na(csd.demand.patients, 0),
         csd.demand.days = replace_na(csd.demand.days, 0),
         csd.alc.rate = csd.demand.days / csd.population) %>% 
  group_by(year, cduid) %>% 
  mutate(cd.demand.days = sum(csd.demand.days),
         cd.alc.rate = cd.demand.days / sum(csd.population)) %>% 
  ungroup()
  
#CALCULATE PROVINCIAL RATE OF LTC DEMAND = TOTAL-DEMAND / 75+ POPULATION
ALC_prov_rate <- ALC_csd_rates %>% 
  group_by(year) %>%  
  summarise(total.pop = sum(csd.population),
            total.dem.days = sum(csd.demand.days)) %>% 
  mutate(prov.alc.rate = total.dem.days / total.pop)
...
if (PROJ_YEAR_ALC == 2016) {
  ...
  ALC_proj_hosp <- ALC_hosp_vol %>% 
    group_by(year, id, siteid, name, csduid, csdname) %>% 
    filter(year == PROJ_YEAR_ALC) %>% 
    summarise_at(vars(hosp.alc.patients:hosp.alc.days), sum)
