...
crm_treso_tbl <- readRDS('cache/moh/treso_origin_cases.rds') %>% 
  ...
  left_join(select(hospital_lookup_master, id, pcode), by = 'pcode') %>%
  select(-siteid) %>% 
  left_join(proj_param, by = c('age_cluster', 'orig.treso.zone' = 'treso_zone', 'caretype')) %>% 
...
HBAM_proj_scale <- hospital_lookup_master %>% 
  filter(year != 2016) %>% 
...
crm2hbam <- left_join(crm_treso_tbl, HBAM_proj_scale, by = c('id', 'hospitalname' = 'name', 'pcode', 'hosp.treso.zone', 'caretype', 'age_cluster', 'year')) %>% 
  mutate(cases.scaled = hosp.treso.cases * scale.factor)

#TABLE IS CREATING DUPLICATES IN 3'S OF ORIGIN TRESO ZONES LEADING TO INFLATED FIGURES FOR HBAM AND CRM CASES; DUPLICATION CAN BE TRACED BACK TO CRM_TRESO_TBL BUT STILL HAVE NOT FIGURED OUT WHY
