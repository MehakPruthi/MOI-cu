...
## Link Hospital Site IDs to TRESO Zones for destination caseload/trip calculations:
  ...
  - Ensure that count of mismatched IDs are linked to original mismatched IDs and not mismatched from join function errors
  - Eliminate mismatched IDs in destination dataset
...
hospital_treso_join <- left_join(HBAMhistorical_destination, hospital_overlay, by = c('siteid', 'caretype')) %>% 
  rename('d_trips' = 'totalcases',
         'treso_zone' = 'treso.id.pos') %>% 
  filter(!is.na(treso_zone))
  
destination_trips <- hospital_treso_join %>% 
  group_by(treso_zone,
           caretype) %>% 
...
hospital_mismatch_treso <- left_join(HBAMhistorical_destination, hospital_overlay, by = c('siteid', 'caretype')) %>% 
  filter(is.na(treso.id.pos))

calcases_cd_destination <- destination_trips %>% 
  ungroup() %>% 
...
```{r}
HBAMhistorical_matrix <- HBAMhistorical_master %>% 
  select(siteid,
         name,
         caretype,
         agegroup,
         totalcases,
         csd) %>% 
  left_join(select(hospital_overlay, treso.id.pos, siteid, caretype), by = c('siteid', 'caretype'))

at_hbam_matrix <- HBAMhistorical_matrix %>% 
  filter(caretype == 'AT') %>% 
  group_by(csd, treso.id.pos) %>% 
  summarise(trips = sum(totalcases))

cr_hbam_matrix <- HBAMhistorical_matrix %>% 
  filter(caretype == 'CR') %>% 
  group_by(csd, treso.id.pos) %>% 
  summarise(trips = sum(totalcases))

gr_hbam_matrix <- HBAMhistorical_matrix %>% 
  filter(caretype == 'GR') %>% 
  group_by(csd, treso.id.pos) %>% 
  summarise(trips = sum(totalcases))

mh_hbam_matrix <- HBAMhistorical_matrix %>% 
  filter(caretype == 'MH') %>% 
  group_by(csd, treso.id.pos) %>% 
  summarise(trips = sum(totalcases))

am_hbam_matrix <- HBAMhistorical_matrix %>% 
  filter(caretype == 'AM') %>% 
  group_by(csd, treso.id.pos) %>% 
  summarise(trips = sum(totalcases))
```
...
calcases_cd_origin <- origin_trips %>% 
  ungroup() %>% 
  summarise(sum(o_trips, na.rm=TRUE))
```
...
### AM
```{r}
am_origin <- origin_trips %>% 
  filter(caretype == 'AM') %>% 
  ungroup() %>% 
  select(-caretype, -cd)

am_destination <- destination_trips %>% 
  filter(caretype == 'AM') %>% 
  select(-caretype)

am_origin_full <- travel_time_skim %>% 
  distinct(orig) %>% 
  left_join(am_origin, by = c('orig' = 'treso_zone')) %>% 
  replace_na(list(o_trips = 0)) %>%
  column_to_rownames(var = "orig") %>%
  data.matrix()

am_destination_full <- travel_time_skim %>% 
  distinct(dest) %>% 
  left_join(am_destination, by = c('dest' = 'treso_zone')) %>% 
  replace_na(list(d_trips = 0)) %>%
  column_to_rownames(var = "dest") %>%
  data.matrix()
```
...
at_csd_sum <- left_join(at_matrix_long, treso_zone_system, by = c('origin' = 'treso_id')) %>% 
...
at_cd_hbam_test <- left_join(at_hbam_matrix, at_csd_sum, by = c('csd'='csduid', 'treso.id.pos'='destination'), suffix = c('.hb', '.c'))

at_cd_hbam_test %>% 
  ungroup() %>% 
  summarise(sum(trips.hb), sum(trips.c, na.rm = TRUE))
```
...
# Aggregate Origin back to CSD
cr_csd_sum <- left_join(cr_matrix_long, treso_zone_system, by = c('origin' = 'treso_id')) %>% 
  select(-area, -mof_region) %>% 
  filter(!is.na(csduid)) %>% 
  ungroup() %>% 
  group_by(csduid, destination) %>% 
...
```{r}
# Create Propensity Matrix
am_prop_matrix <- matrix_balancing(prop_matrix, am_origin_full, am_destination_full, 1000, 0.001)

# Convert Prop Matrix to Long Format
am_matrix_long <- am_prop_matrix %>% 
  melt() %>% 
  rename('origin' = 'Var1',
         'destination' = 'Var2',
         'trips' = 'value')
...
# Aggregate Origin back to CSD
am_csd_sum <- left_join(am_matrix_long, treso_zone_system, by = c('origin' = 'treso_id')) %>% 
  select(-area, -mof_region) %>% 
  filter(!is.na(csduid)) %>% 
  ungroup() %>% 
  group_by(csduid, destination) %>% 
...
# Summarise and Calculate Comparison
am_prop_sum <- am_prop_matrix %>% 
  colSums() %>% 
  melt() %>% 
  summarise(sum(value))

am_origin_sum <- am_origin %>% 
  ungroup() %>% 
  summarise(sum(o_trips))

am_cd_caserate_ratio <- am_prop_sum/am_origin_sum
```
