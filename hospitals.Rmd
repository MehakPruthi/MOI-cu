...
rm(prop_matrix, treso_health_slim_tb, treso_travel_time)
...
##Base Scenario Calculation: CREM Rates
...
```{r}
#Update Master HBAM list to Include TRESO Zones (COPIED FROM CHUNKS ABOVE, REMOVE ONCE CODE IS WORKING/CLEANED, BUT INCLUDE ABOVE THE DEDUPLICATION CODE BELOW THE NEXT COMMENT)
...
treso_origin_cases <- left_join(HBAMhistorical_origcsd_cases, treso_population_long, by = c('year', 'orig_csd' = 'csduid', 'agegroup' = 'age_group')) %>% 
...
treso_origin_rate_table <- treso_origin_cases %>% 
  filter(year == 2016) %>% 
  select(year,
         siteid,
         hospitalname,
...
HBAMprojected_alos <- HBAMprojected_master_tz %>% 
  ...
  mutate(alos = ifelse(caretype == 'AM', (avg_edlos/24)/totalcases, total.los/totalcases)) %>% 
