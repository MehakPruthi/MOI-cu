...
UTILIZATION_TARGETS <- tibble(caretype = c('AT', 'MH', 'CR', 'GR', 'AM'), target = c(0.96, 0.96, 0.96, 0.96, 0.96))
...
DECISION_MAKING <- FALSE # TODO: Replace with value from user from visualization tool. #if FALSE, no new hospital beds are built on its own
...
### Add hospitals
```{r}
#specify new hospital details below
...
cost_new_hospital <- distinct(filter(hospital_wide_final, id > 999), id, beds.forecasted.AT, beds.forecasted.CR, beds.forecasted.GR, beds.forecasted.MH) %>% 
  # Add in costs per bed by caretype
  cbind(., COST_PER_NEW_BED) %>% 
  mutate(cost_2018 = round(USER_INPUT_BASE_HOSP_COST * 1000000 + beds.forecasted.AT * cost.per.bed.AT + beds.forecasted.CR * cost.per.bed.CR + beds.forecasted.GR * cost.per.bed.GR + beds.forecasted.MH * cost.per.bed.MH, 0)) %>% 
  select(id, cost_2018)
...
cost_existing_hospital <- distinct(filter(hospital_wide_final, id < 999), id, beds.forecasted.AT, beds.forecasted.CR, beds.forecasted.GR,
  ...
  # Add in costs per bed by caretype
  cbind(., COST_PER_NEW_BED) %>% 
  mutate(cost_2018 = round(BASE_HOSP_EXP_COST * 1000000 * pmin(beds.to.build.total, 1) + beds.to.build.AT * cost.per.bed.AT + beds.to.build.CR * cost.per.bed.CR + beds.to.build.GR * cost.per.bed.GR + beds.to.build.MH * cost.per.bed.MH, 0)) %>% 
  select(id, cost_2018)

# Combining costs for new hospitals with costs for expanding existing
cost_hospitals <- bind_rows(cost_new_hospital, cost_existing_hospital)
