...
## Decision-Making (Instead of 'Add hospitals')
...
# ALT: add hospitals based on decision-making calculations

  # TODO: Replace this with list of new hospitals provided by user through visualization tool
  new_hosp_beds <- overburden_csd_hosp_new %>% 
    distinct(orig.csd, caretype, csd.beds.needed) %>% 
    mutate(csd.beds.needed = ceiling(csd.beds.needed)) %>% 
    # Limit new hospital beds to be built only if minimum size threshold is crossed
    filter(csd.beds.needed >= min_beds) %>% 
    rename(beds.to.build = csd.beds.needed) %>% 
    ...
    mutate(row = group_indices(., orig.csd),
           id = 1111 * row) %>% 
    select(-row) %>%
    # Determine most populous TRESO zone in CSD and build new hospital there
    ...
    # Generate name based on hospital ID / TRESO and generate 0 beds for other care types? Or build beds based on their overburden?
...
crm_hosp_agecluster_all <- crm_hosp_agecluster_redist %>%
  ...
  mutate(beds = ifelse(caretype == 'AM' & id < 300, -1, beds)) %>% 
  ...
  group_by(id) %>% 
  mutate(AM.cases.no.bed.data = sum(cases[caretype == 'AM' & id < 300])) %>% 
  group_by(hosp.csd) %>% 
  mutate(AM.cases.bed.data = sum(cases[caretype == 'AM' & id < 300]) - AM.cases.no.bed.data,
         ...
         sum.tot.beds = sum(total.hosp.beds[id < 300]),
         total.hosp.beds = ifelse(total.hosp.beds == 0, AM.cases.weight * sum(total.hosp.beds[id < 300]), total.hosp.beds),
