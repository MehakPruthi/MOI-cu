...
## User Inputs
  - To be replaced with inputs read in from Shiny
  
```{r}
# Read in user-set forecast year
SCENARIO_YEAR <- 2026 # TODO: Replace with user-defined input from visualization tool
...
LTC_OP_DAYS = 365 # Set operating days at LTC facilities; set based on observed ALC in 2016 vs. theoretical 

# Number of beds added by CSD
USER_NEW_LTC_CAP_CSD <- tibble(csduid = c(3506008, 3519049), added.ltc.cap = c(15000, 15000)) # TODO: Replace with user-defined input from visualization tool

# Read in cost function parameters from model user: cost per bed (in CAD millions) plus a base startup cost applied to all hospitals regardless of size
USER_INPUT_COST_PER_BED <- 1 # TODO: Replace this hard-coded value with user-input value from visualization
USER_INPUT_BASE_HOSP_COST <- 5 # TODO: Replace this hard-coded value with user-input value from visualization
...
### ALC Base Scenario
```{r}
ALC_prov_rate <- base_scenario_alc_calculation(hospital_lookup_ALC, ALC_hosp_vol_disag, ltc_homes, treso_population_moh_agecluster, LTC_OP_DAYS)
```
...
if (DECISION_MAKING) {  
  new_hospital <- moh_new_hospitals(overburden_csd_hosp_new, treso_zone_system, treso_population_moh_agecluster, treso_centroids,
                                    new_hospital_user, MIN_BEDS_TOGGLE, MIN_BEDS, SCENARIO_YEAR)
} else {
...
result <- format_demand_output(treso_zone_system, treso_population_moh_agecluster, treso_travel_time,
...
if (nrow(new_hospital) != 0) {
  ...
  new_hosp_list_complete <- new_hosp_list %>% 
    mutate(cost_2018 = round(total.hosp.beds * USER_INPUT_COST_PER_BED * 1000000 + USER_INPUT_BASE_HOSP_COST * 1000000, 0))
  
  # Inflate cost for facility construction based on construction year; calculate total cost over time period of interest
  annualized_inflated_cost <- construction_cost(USER_INPUT_INFLATION, SCENARIO_YEAR, CURRENCY_YEAR,
                                              USER_INPUT_CURRENT_YEAR, new_hosp_list_complete)
}

```

## CRM vs. HBAM Reporting
```{r}
 
########################################
# Results if user chooses HBAM forecast
if (SCENARIO_YEAR <= 2016) {
  HBAM_control <- readRDS("cache/moh/hospital_lookup_master.rds") %>%
    filter(year == SCENARIO_YEAR) %>%
    gather(key = "segment", value = "cases", cases.at.ad:cases.am.sr) %>%
    separate(segment, c(NA, "caretype", "age_cluster")) %>%
    mutate(caretype = toupper(caretype)) %>%
    group_by(id, caretype, age_cluster) %>%
    summarise(cases = sum(cases)) %>%
    ungroup() %>% 
    # Calculate total cases by hospital, caretype
    group_by(id) %>% 
    mutate(sum.cases = sum(cases)) %>%
    ungroup() %>% 
    # Pivot table from long to wide for agecluster for demand (cases only since historical data, which does not include length of stay / bed-days)
    pivot_wider(names_from = c(caretype, age_cluster), values_from = c(cases), 
                values_fill = c(cases = 0)) %>% 
    setNames(paste0('cases.', names(.))) %>% 
    rename(sum.cases = cases.sum.cases, id = cases.id)
  ...
  HBAM_control <- HBAMprojected_master_tz %>%
    ...
    summarise_at(vars(cases:demand.days.total), sum, na.rm=TRUE) %>%
    group_by(id) %>% 
    mutate(sum.cases = sum(cases),
    ...
    pivot_wider(names_from = c(caretype, agecluster), values_from = c(cases, demand.days.total), 
                values_fill = c(cases = 0, demand.days.total = 0))
