...
# Incorporate new hospitals added by redistributing demand from other hospitals and csds
  ...
  new_hospital <- tibble(
    ...
    specialityfactor = 0 # 1 if hospital is speciality hospital, 0 if not
  )
...
# TODO: Replace this with list of new hospitals provided by user through visualization tool - NEW HOSPITALS SHOULD HAVE AGECLUSTERS SPECIFIED
  ...
  num_beds_revised <- new_hospital_overlay %>% 
    select(-agecluster, -hospitalname, -specialityfactor, -hosp.cd) %>% 
    distinct() %>% 
    bind_rows(., select(num_beds, id, caretype, beds = staffedbeds)) %>% 
  ...
  ## Prepare the origin demand vector for new hospitals for selected year
  crm_all_hosp <- crm_hosp_agecluster %>% 
    left_join(select(hospitallocations_tz, treso_zone, hosp.csd = csduid, id), by = c('id')) %>% 
    mutate(hosp.cd = substr(hosp.csd, 1, 4)) %>% 
    # Add in speciality hospital flag for existing hospitals
    left_join(select(speciality_hospitals, id, specialityfactor), by = "id") %>% 
    # Join to num_beds to add counts of beds by caretype at existing hospitals
    left_join(select(num_beds, id, caretype, beds = staffedbeds), by = c('id', 'caretype')) %>% 
    # Add in new hospitals which (at this point) have no cases, beds needed, etc.
    bind_rows(., new_hospital_overlay) %>% 
    select(-hospitalname) %>% 
