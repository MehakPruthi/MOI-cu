...
speciality_hospitals <- read.csv("cache/moh/specialty_hospital_list.csv")
...
# Incorporate new hospitals added by redistributing demand from other hospitals and csds
  ...
  crm_new_hosp <- crm_alc %>% 
    select(-demand.days.nonALC, -demand.days.ALC, -beds.needed,hosp.cd) %>% 
    left_join(select(speciality_hospitals, id, specialityfactor), by = "id")

  # New Hospital
  new_hospital <- tibble(
    id = "99999",
    hospital.lat = 43.90032, hospital.long = 	-78.85863,
    name = "New Hospital",
    caretype = c("AT", "CR", "GR", "MH", "AM"),
    beds = c(100,1000,1000,100,0),
    agecluster = c("all","all","all","all","all"), #all ages or just "ped"
    specialityfactor = 0, # 1 if hospital is speciality hospital, 0 if not
  )

  # Overlay with TRESO Shapefile to get the TRESO/CSD Information
  ...
  new_hospital_overlay <- create_overlay(new_hospital_xy, treso_shp, type = "hospital") %>% 
    ...
    select(-lat, -long, -csdname, -area, -mof_region, -hospital.lat, -hospital.long, -cdname, -treso.id.pos) %>% 
    rename(hosp.csd = csduid, hospitalname = name, hosp.cd = cduid) %>% 
  ...
  hospital_new_list <- bind_rows(new_hospital_overlay, crm_new_hosp) 
