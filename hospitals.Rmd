...
# This if function tests if the year is 2016 (with known historical data) or a future year. When it is 2016, it pulls actual historical ALC days. When it is a future year, it estimates ALC demand based on population growth and rates observed
...
# When running this set of queries for 2016, the model returns LTC demand days ()
...
ALC_proj_csd <- proj_pop_control %>%
  ...
  mutate(cduid = substr(csduid, 1, 4)) %>% 
  ungroup() %>% 
  distinct(cduid, total.csd.beds, ltc.bed.days.cap, csd.patients.demand, csd.patients.cap, ltc.csd.demand.days, csd.residual.alc.days, csd.residual.patients) %>%
# Use the code below to sum to the CD level if desired
  ...
  # ungroup() %>% 
  mutate(sumDemandDays = sum(ltc.csd.demand.days), sumCapDays = sum(ltc.bed.days.cap), sumResidualDays = sum(csd.residual.alc.days)) # TODO: delete this row once working
