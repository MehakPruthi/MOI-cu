...
numbeds %>%
  filter(year == 2017) %>% 
  ungroup() %>% 
  mutate(totbeds = sum(maxstaffedbeds))
```
...
hospitallocations_tz <- hospitallocations %>% 
  ...
  distinct(id, name, lhin, lhinlocation, treso_zone, csduid, csdname, mof_region, pcode, lat, long, address, city)
...
num_beds <- num_beds_siteid %>% 
  ...
  mutate(staffedbeds = max(staffedbeds)) %>%
