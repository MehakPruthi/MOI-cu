...
#Pediatric
for (ct in caretypes) {
    origin_ped <- origin_trips_ped %>% 
      filter(caretype == toupper(ct)) %>% 
      ungroup() %>% 
      select(-caretype, -cd)
    
    saveRDS(origin_ped, file = paste0("cache/hospitals/", ct, "_origin_ped.rds"))
    
    destination_ped <- destination_trips_ped %>% 
      filter(caretype == toupper(ct)) %>% 
      select(-caretype)
    
    saveRDS(destination_ped, file = paste0("cache/hospitals/", ct, "_destination_ped.rds"))
    
    origin_full_ped <- travel_time_skim %>% 
      distinct(orig) %>% 
      left_join(origin_ped, by = c('orig' = 'treso_zone')) %>% 
      replace_na(list(o_trips = 0)) %>%
      column_to_rownames(var = "orig") %>%
      data.matrix()
    
    saveRDS(origin_full_ped, file = paste0("cache/hospitals/", ct, "_origin_full_ped.rds"))
