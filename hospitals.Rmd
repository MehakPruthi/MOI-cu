...
# ALT: add hospitals based on decision-making calculations
  ...
  new_hosp_beds <- overburden_csd_hosp_new %>% 
    ...
    mutate(hospital.name = paste0('Hospital ', id)) %>% 
    # Assing zero beds to any caretypes at new hospitals which do not have demand observed
    
    # Calculate total beds by new hospital
    group_by(id) %>% 
    mutate(total.hosp.beds = sum(beds[caretype != 'AM'])) %>% 
    ungroup()
