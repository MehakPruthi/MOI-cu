...
ALC_csd_vol <- ALC_hosp_vol %>%
  ...
  mutate(csd.ltc.beds = replace_na(csd.ltc.beds, 0),
         ltc.bed.days = replace_na(ltc.bed.days, 0),
         csd.demand.days = ltc.bed.days + hosp.alc.days,
         csd.demand.beds = csd.ltc.beds + hosp.alc.patients)
...
ALC_treso <- ALC_csd_vol %>% 
  ...
  mutate(csd.pop = sum(treso.population.agecluster),
         ...
         treso.csd.dem.beds = csd.demand.beds * treso.csd.pop.perc,
...
ALC_rates <- ALC_treso %>% 
  ...
  mutate(treso.alc.rate = treso.csd.dem.beds / pop.65plus) %>% 
  group_by(year, csduid, agecluster) %>% 
  mutate(csd.alc.rate = csd.demand.beds / sum(pop.65plus)) %>% 
  ungroup() %>% 
...
ALC_proj_csd <- treso_population_moh %>%
  ...
  mutate(ltc.treso.demand = PROJ_ALC_RATE * get(paste0("population.", PROJ_YEAR_ALC))) %>% 
  group_by(csduid) %>% 
  mutate(ltc.csd.demand.beds = sum(ltc.treso.demand)) %>% #DIFFERENTIATION BY AGE-CLUSTER WAS HINDERING UNIFYING THE TOTAL NUMBER OF LTC BEDS
  ...
  mutate(csd.ltc.beds = replace_na(csd.ltc.beds, 0), #REPLACE CSD'S W/ ZERO LTC CAPACITY
         ltc.bed.days = replace_na(ltc.bed.days, 0),
         csd.residual.alc.beds = ifelse(ltc.csd.demand.beds - csd.ltc.beds > 0, ltc.csd.demand.beds - csd.ltc.beds, 0)) %>% 
  select(csduid, ltc.csd.demand.beds:csd.residual.alc.beds) %>% 
  distinct()
...
ALC_proj_hosp <- ALC_hosp_vol %>%
  ...
  left_join(select(ALC_proj_csd, csduid, csd.residual.alc.beds), by = c('csduid')) %>% 
  group_by(siteid) %>% 
  mutate(proj.alc.beds = csd.residual.alc.beds * hosp.alc.mrkt,
         proj.alc.beddays = proj.alc.beds * avg.los)
