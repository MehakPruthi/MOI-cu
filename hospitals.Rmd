...
#tibble(csduid = c(3506008, 3519049), added.ltc.cap = c(5000, 10000))
...
COST_PER_NEW_BED <- tibble(cost.per.bed.AT = 750000, cost.per.bed.CR = 850000, cost.per.bed.GR = 1100000, cost.per.bed.MH = 1500000)

# Cost parameters for new hospitals
USER_INPUT_BASE_HOSP_COST <- 5

# Cost parameters for expanding existing hospitals
BASE_HOSP_EXP_COST <- 4
...
new_hospital_user <- tibble(
  ...
  specialityfactor = c(rep(0, 5)) # 1 if hospital is speciality hospital, 0 if not
  ) %>%
  group_by(id) %>%
  mutate(total.hosp.beds = sum(beds[caretype != 'AM'])) %>%
  ungroup()

#BELOW TO TEST 2 NEW HOSPITALS - can remove once tested
#new_hospital_user <- tibble(
