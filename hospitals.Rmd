...
LTC_hosp_vol_disag <- LTC_madd_clean_disag %>%
  ...
  #here we need to group_by and sum rows now that we're combining multiple agegroups into ageclusters - to avoid dupicates later
  group_by(year,agecluster,lhin,siteid) %>% 
  summarise(patients = sum(patients),adj.hosp.days.tot = sum(adj.hosp.days.tot)) %>% 
  ungroup()
...
LTC_hosp_vol <- LTC_hosp_vol_disag %>% 
  ...
  #group_by(year, id, agecluster) %>% 
  group_by(year, id, agecluster,csduid, csdname,name =name.x) %>% 
  #mutate(hosp.alc.patients = sum(patients), #these should be summarize instead of mutate? there are multiple site ids per id that otherwise become duplicates
  summarise(hosp.alc.patients = sum(patients),
            hosp.alc.days = sum(adj.hosp.days.tot)) %>% 
  mutate(avg.los = hosp.alc.days / hosp.alc.patients) %>%
  ...
  ungroup() #%>% 
  #group_by(year, id, agecluster)   %>% 
  #mutate(count = n()) #duplicates here because of 1. combining agegroups into age clusters and 2. multiple site ids per id - both should be resolved ealier #distinct below selects only the first row to resolve these, which can create issues, as values are not always the same between duplicates
...
ALC_hosp_vol_ex_LTC <- ALC_hosp_vol_disag_ex_LTC %>% 
  ...
  #group_by(year, id, agecluster)   %>% 
  #mutate(count = n()) 
  #same as LTC_hosp_vol above -not clear whether distinct captures duplicates properly
  #distinct(year, id, name = name.x, csduid, csdname, agecluster, hosp.alc.patients, hosp.alc.days, avg.los, hosp.alc.mrkt)
...
HBAMprojected_alc <- HBAMprojected_alos %>% 
  ...
  mutate(adj.net.hosp.days = pmax(net.hosp.days, 0.5 * hosp.caretype.agecluster.days),
         other.caretype.alc.days = adj.net.hosp.days - net.hosp.days) #this factors up the net days,roughly 1% total - ok
...
# Calculate average length of stay adjusted for ALC (necessary to join to bring pediatric bed-days back into the model)
