...
treso_population_long <- gather(treso_population_moh, 'year', 'population', population.2011:population.2041, -age_group) %>% 
  ...
  select(-pop) %>% 
  left_join(agegroup_ref, by = c('age_group' = 'agegroup')) %>% 
...
HBAMprojected_alos <- HBAMprojected_master_tz %>% 
  ...
  group_by(year, caretype, age_cluster) %>% 
  summarise(totalcases = sum(cases),
            total.los = sum(los, na.rm = TRUE)) %>% 
  mutate(alos = total.los/totalcases) %>% 
  select(year, caretype, age_cluster, alos)
...
# Then claculate case rates for TRESO zones at the CSD-Hospital pair, segmented by caretype, year, and HBAM agegroup; also segmented seprately by caretype, year, and age_cluster (which lumps some age groups together into 'ped', 'adult', 'senior')
treso_origin_cases <- left_join(HBAMhistorical_origcsd_cases, treso_population_long, by = c('year', 'orig_csd' = 'csduid', 'agegroup' = 'age_group', 'age_cluster')) %>% 
  ...
  mutate(orig.treso.pop.agecluster = sum(orig.treso.pop),
         ...
         csd.mrkt.agecluster = csd.hosp.cases.agecluster / hosp.cases.agecluster) %>%
  group_by(year, agegroup, orig_csd, caretype, siteid) %>%
  mutate(csd.pop = sum(orig.treso.pop),
         treso.pop.perc = orig.treso.pop/csd.pop,
         treso.hosp.cases = csd.mrkt * treso.pop.perc * hosp.cases, 
         treso.caserate = treso.hosp.cases / orig.treso.pop) %>% 
  group_by(year, age_cluster, orig_csd, caretype, siteid) %>%
  mutate(csd.pop.agecluster = sum(orig.treso.pop.agecluster),
         treso.pop.perc.agecluster = orig.treso.pop.agecluster/csd.pop.agecluster,
         treso.hosp.cases.agecluster = csd.mrkt.agecluster * treso.pop.perc.agecluster * hosp.cases.agecluster, 
         treso.caserate.agecluster = treso.hosp.cases.agecluster / orig.treso.pop.agecluster) %>% 
  ungroup() %>% 
...
proj.year <- 2025
op.days <- 365 #number of operating days to determine beds needed

hospital_lookup_master <- readRDS('output/moh/hospital_lookup_master.rds')

#Projection population parameter table to simplify CRM calculation as multiple year-rows causing crash
proj_param <- left_join(select(treso_population_long, treso_zone, year, age_cluster, treso.population.agecluster), HBAMprojected_alos, by = c('age_cluster', 'year')) %>% 
  distinct() %>% 
  filter(year == proj.year) %>% 
  select(year, treso_zone, age_cluster, treso.population.agecluster, caretype, alos)
...
crm_treso_tbl <- readRDS('cache/moh/treso_origin_cases.rds') %>% 
  ...
  left_join(select(hospital_lookup_master, id, pcode), by = 'pcode') %>% #joins used to link siteid w/ id from look-up table
  distinct() %>% #remove duplicates from hospital_lookup: year variable
  select(-siteid) %>% 
  left_join(proj_param, by = c('age_cluster', 'orig.treso.zone' = 'treso_zone', 'caretype')) %>% #projection parameters include ALOS and population used to calculate cases and bed-days
  group_by(id, orig.treso.zone, caretype, age_cluster) %>% 
  mutate(hosp.treso.cases = treso.caserate.agecluster * treso.population.agecluster,
         hosp.treso.beddays = hosp.treso.cases * alos,
         hosp.treso.bedsneeded = hosp.treso.beddays / op.days) %>%
  ungroup() 

#Summarise hospital-treso zone cases to each hospital
crm_hosp_tbl <- crm_treso_tbl %>% 
  group_by(year, id, hospitalname, pcode, caretype, age_cluster) %>% 
  summarise_at(vars(hosp.treso.cases:hosp.treso.bedsneeded), sum, na.rm=TRUE) %>% 
  ungroup()

#SUM OF HOSP.TRESO.CASES TOO LOW (~4M); HBAM(2025) = 8.8M; 

#########################################################################

#Attach HBAM projected cases to look-up master list by converting to wide format and joining
HBAMprojected_spread <- HBAMprojected_master_tz %>% 
  group_by(year, name, pcode, age_cluster, caretype) %>% 
  ...
  mutate(cases_at_ad = ifelse(caretype %in% 'AT' & age_cluster %in% 'ad', cases, 0),
         ...
         cases_am_ped = ifelse(caretype %in% 'AM' & age_cluster %in% 'ped', cases, 0),
         cases_am_sr = ifelse(caretype %in% 'AM' & age_cluster %in% 'sr', cases, 0)) %>% 
         # los_at_ad = ifelse(caretype %in% 'AT' & age_cluster %in% 'ad', los, 0),
         # los_at_ped = ifelse(caretype %in% 'AT' & age_cluster %in% 'ped', los, 0),
         # los_at_sr = ifelse(caretype %in% 'AT' & age_cluster %in% 'sr', los, 0),
         # los_cr_ad = ifelse(caretype %in% 'CR' & age_cluster %in% 'ad', los, 0),
         # los_cr_ped = ifelse(caretype %in% 'CR' & age_cluster %in% 'ped', los, 0),
         # los_cr_sr = ifelse(caretype %in% 'CR' & age_cluster %in% 'sr', los, 0),
         # los_gr_ad = ifelse(caretype %in% 'GR' & age_cluster %in% 'ad', los, 0),
         # los_gr_ped = ifelse(caretype %in% 'GR' & age_cluster %in% 'ped', los, 0),
         # los_gr_sr = ifelse(caretype %in% 'GR' & age_cluster %in% 'sr', los, 0),
         # los_mh_ad = ifelse(caretype %in% 'MH' & age_cluster %in% 'ad', los, 0),
         # los_mh_ped = ifelse(caretype %in% 'MH' & age_cluster %in% 'ped', los, 0),
         # los_mh_sr = ifelse(caretype %in% 'MH' & age_cluster %in% 'sr', los, 0),
         # los_am_ad = ifelse(caretype %in% 'AM' & age_cluster %in% 'ad', los, 0),
         # los_am_ped = ifelse(caretype %in% 'AM' & age_cluster %in% 'ped', los, 0),
         # los_am_sr = ifelse(caretype %in% 'AM' & age_cluster %in% 'sr', los, 0)) %>% 
  group_by(year, name, pcode) %>% 
  summarise_at(vars(cases_at_ad:cases_am_sr), sum) %>% 
  left_join(hospital_lookup_latlong_id, by = c('pcode', 'name'))

#Calculate scale factor between HBAM and CRM by converting lookup 
HBAM_lookup <- hospital_lookup_master %>% 
  filter(year != 2016) %>% 
  bind_rows(HBAMprojected_spread) %>% 
  group_by(id) %>% 
  fill(at_beds_2017, cr_beds_2017, gr_beds_2017, mh_beds_2017, dbc_total_beds_2017,
       at_beds_2018, cr_beds_2018, gr_beds_2018, mh_beds_2018, dbc_total_beds_2018) %>% 
  ungroup() %>% 
  filter(year == proj.year) 

# #PART OF HBAM-CRM SCALE FACTOR CALCULATIONS; LEAVE COMMENTED-OUT FOR TIME BEING%>% 
#   gather('caretype', 'hbam.cases', cases_at_ad:cases_am_sr) %>% 
#   separate(caretype, into = c('caretype', 'age_cluster'), sep = "cases_") %>% 
#   select(-caretype) %>% 
#   separate(age_cluster, into = c('caretype', 'age_cluster'), sep = '_') %>% 
#   mutate(caretype = toupper(caretype)) %>% 
#   left_join(select(crm_hosp_tbl, id, year, caretype, age_cluster, crm.cases = hosp.treso.cases), by = c('year', 'id', 'caretype', 'age_cluster')) %>% 
#   mutate(scale.factor = hbam.cases/crm.cases) %>% 
#   # distinct() %>% 
#   # select(year, id, name, pcode, hosp.treso.zone = treso_zone, caretype, age_cluster, scale.factor) %>% 
#   ungroup()
# 
# crm2hbam <- left_join(crm_treso_tbl, HBAM_proj_scale, by = c('id', 'hospitalname' = 'name', 'pcode', 'hosp.treso.zone' = 'treso_zone', 'caretype', 'age_cluster', 'year')) %>% 
#   mutate(cases.scaled = hosp.treso.cases * scale.factor)


#TESTS FOR CALCULATION ERRORS RE HBAM-CRM SCALE FACTORS
# hbampjtest <- HBAMprojected_master_tz %>% 
#   group_by(year, name, pcode, caretype, age_cluster) %>% 
#   summarise(hbam.cases = sum(cases),
#             hbam.los = sum(los),
#             hbam.beddays = sum(beddays)) %>% 
#   mutate(hbam.los = replace_na(hbam.los, 0),
#          hbam.beddays = replace_na(hbam.beddays, 0)) %>% 
#   filter(year == proj.year) %>% 
#   left_join(crm_hosp_tbl, by = c('name' = 'hospitalname', 'year', 'pcode', 'caretype', 'age_cluster'))
# 
# sum(filter(hbampjtest, year == 2025)$hbam.cases) #TOTAL CASES SHOULD = 8,850,732
# 
# #Check sum of HBAM cases matches data source (above)
# hbampjtest %>% 
#   ungroup() %>% 
#   summarise(sum(hbam.cases),
#             sum(hosp.treso.cases, na.rm = TRUE))
```

NOT-SO-LOW HANGING FRUIT FOR PAUL:
1. COMIBINING CRM RATES AND ALOS W/ POPSYN FORECAST
2. ADD HBAM PROJECTED TO LOOKUP TABLE
3. CALCULATE SCALE FACTOR BETWEEN CRM BED-DAYS AND HBAM PROJ
