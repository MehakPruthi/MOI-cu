...
new_ltc_cap_csd <- tibble(csduid = c(3506008, 3519049), added.ltc.cap = c(0, 0)) # TODO: Replace with user-defined input from visualization tool
...
HBAMhistorical_origcsd_cases <- HBAMhistorical_master_tz %>%
  group_by(year, id, agecluster, caretype, orig_csd, hospitalname, treso_zone, caretype, agecluster, hosp_csduid) %>%
...
treso_origin_cases <- HBAMhistorical_origcsd_cases %>% 
  ...
  # Replace N/A populations with 0
  replace_na(list(orig.treso.pop.agecluster = 0)) %>% 
  drop_na(orig.treso.zone) %>% # A few CSDs don't exist in TRESO, but very small case volumes (~2,000 rows out of 2.64M) so removed for simplicity
  select(-age_group, -population) %>%
  group_by(id) %>%
  mutate(hospitalname = first(hospitalname)) %>%
  ungroup() %>%
  # Remove duplicate rows in going from age_group to agecluster, and any hospital name discrepancies
  distinct() %>% 
  ...
  replace_na(list(treso.agecluster.caretype.mrkt = 0)) %>% 
  mutate(treso.hosp.agecluster.caretype.cases = treso.agecluster.caretype.mrkt * hosp.csd.agecluster.caretype.cases) %>%
  mutate(sumperc = sum(csd.agecluster.caretype.mrkt)) %>%
...
hospital_wide <- crm_demand %>% 
  ...
  mutate(beds.forecasted.AT = max(beds.needed_AT, beds.existing.AT),
         beds.forecasted.CR = max(beds.needed_CR, beds.existing.CR),
         beds.forecasted.GR = max(beds.needed_GR, beds.existing.GR),
         beds.forecasted.MH = max(beds.needed_MH, beds.existing.MH)) %>% 
  # Calculate utilization by hospital, caretype
  group_by(id, caretype) %>% 
  mutate(utilization.AM = 0,
         utilization.AT = (sum(demand.days.total_ad_AT)+sum(demand.days.total_ped_AT)+sum(demand.days.total_sr_AT)) / beds.forecasted.AT, 
         utilization.CR = (sum(demand.days.total_ad_CR)+sum(demand.days.total_ped_CR)+sum(demand.days.total_sr_CR)) / beds.forecasted.CR,
         utilization.GR = (sum(demand.days.total_ad_GR)+sum(demand.days.total_ped_GR)+sum(demand.days.total_sr_GR)) / beds.forecasted.GR,
         utilization.MH = (sum(demand.days.total_ad_MH)+sum(demand.days.total_ped_MH)+sum(demand.days.total_sr_MH)) / beds.forecasted.MH) %>% 
  ungroup() %>% 
  replace_na(list(utilization.AT = 0, utilization.CR = 0, utilization.GR = 0, utilization.MH = 0))

    
########### USE USER-INPUT / DM LAYER BEDS AS 'EXISTING' FOR NEW HOSPITALS??? NO, BECAUSE THIS MEANS WE WILL UNDERCOUNT NUMBER OF BEDS TO BUILD. BUT HOW TO ACCOUNT FOR DIFFERENCE BETWEEN NUMBER OF BEDS PROPOSED/BUILT UNDER DM LAYER vs. NUMBER OF BEDS 'NEEDED' BASED ON DEMAND?
