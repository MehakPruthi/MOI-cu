...
crm_hosp_agecluster_all <- crm_hosp_agecluster_redist %>%
  ...
  mutate(new.hospital = 0) %>% 
  # Join in bed counts for existing hospitals
  left_join(select(num_beds, id, beds = staffedbeds, total.hosp.beds, caretype), by = c('id', 'caretype')) %>%
  # Set AM (Emerg) beds to -1 to resize in later step
  mutate(beds = ifelse(caretype == 'AM', -1, beds)) %>% 
  ...
  rename(specialityfactor = specialityfactor.x, beds = beds.x) %>% 
  ...
  select(-specialityfactor.y, -beds.y) %>%
