...
# Use the code below to sum to the CD level if desired
  ...
  mutate(ltc.cd.demand.days = sum(ltc.csd.demand.days), 
  ...
  ungroup() %>% 
  mutate(sumDemandDays = sum(ltc.csd.demand.days), sumCapDays = sum(ltc.bed.days.cap), sumResidualDays = sum(csd.residual.alc.days))

# TODO: Delete this query when above code working
# cd_avg_los <- ltc_los_per_year %>% 
#   mutate(cduid = substr(csduid, 1, 4)) %>% 
#   group_by(cduid) %>%
#   mutate(cd.avg.los <- max(avg.annual.los)) %>% 
#   ungroup()
