...
ltc_homes_csd <- ltc_homes %>% 
  ...
  mutate(ltc.bed.days.cap = csd.ltc.patients * OP_DAYS,
...
ALC_csd_vol <- ALC_hosp_vol %>%
  ...
  mutate(csd.ltc.patients = replace_na(csd.ltc.patients, 0),
         ltc.bed.days.cap = replace_na(ltc.bed.days, 0),
...
ALC_prov_rate <- ALC_csd_rates %>% 
  filter(year %in% (2000:2020)) %>% # Adjust this filter to choose specific years to include in base rate calculations
...
scenario.year <- 2022 # TODO: Replace with user-defined input from visualization tool
op.days = 365 # Days per year

# TODO: Delete these rate pulls if not needed
# Extract provincial rate for adult ALC days per population (adult population)
alc.rate.ad <- ALC_prov_rate %>% 
  filter(agecluster == 'ad') %>% 
  select(prov.alc.rate.ages) %>% 
  pull()

# Extract provincial rate for senior ALC days per population (senior population)
alc.rate.sr <- ALC_prov_rate %>% 
  filter(agecluster == 'sr') %>% 
  select(prov.alc.rate.ages) %>% 
  pull()
...
# TODO: Remove/rename references to OP_DAYS if not needed
