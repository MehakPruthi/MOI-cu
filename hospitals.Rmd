...
[] Calculate projections for Origins based on historical CSD caserates
[] Scale origins to match projected HBAM destinations
...
readRDS('cache/hospitals/am_proj_destination_ad.rds')
...
```{r}
#Create OD Full matrices for balancing and prop matrix
#Adult
...
for (ct in caretypes) {
    for (a in age_clusters) {
      ...
      saveRDS(ct_origin, file = paste0("cache/hospitals/", ct, "_proj_origin_", a,".rds"))
      ...
      saveRDS(proj_origin_full_ad, file = paste0("cache/hospitals/", ct, "_proj_origin_full_", a, ".rds"))
     ...
     saveRDS(proj_destination_trips, file = paste0("cache/hospitals/", ct, "_proj_destination_", a,".rds"))
     ...
     saveRDS(proj_destination_full_ad, file = paste0("cache/hospitals/", ct, "_proj_destination_full_", a, ".rds"))

    }
}

am_proj_prop_matrix_ad <- matrix_balancing_1d(prop_matrix, am_proj_destination_full_ad, am_proj_origin_full_ad, axis = 2, constrained = TRUE)

#FUNCTION TO RETURN TRESO ORIGIN TO TRESO DESTINATION, RETURN BALANCED MATRIX IN FUNCTION
#SUM MATRICES THAT ARE THE SAME SIZE FOR SINGLE CARETYPE TO COMBINE TOTAL AGE
...
proj_matrix <- function(ct) {
  ...
  for (a in unique(agegroup_ref$age_clusters)) {
     ...
     # proj_summary_table <- proj_prop_matrix %>% 
     #   melt() %>% 
     #   rename('origin' = 'Var1',
     #          'destination' = 'Var2',
     #          'trips' = 'value')
     
     # assign(paste0('proj_summary_table_', a), proj_prop_matrix %>% 
     #   melt() %>% 
     #   rename('origin' = 'Var1',
     #          'destination' = 'Var2',
     #          'trips' = 'value'))

     # saveRDS(proj_summary_table, file = paste0("cache/hospitals/", ct, "_proj_summary_table_", a, ".rds"))
...
# sen <- readRDS('cache/hospitals/am_proj_prop_matrix_sr.rds')
# adu <- readRDS('cache/hospitals/am_proj_prop_matrix_ad.rds')
# kid <- readRDS('cache/hospitals/am_proj_prop_matrix_ped.rds')
