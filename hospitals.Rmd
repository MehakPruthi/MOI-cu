...
crm_treso_tbl <- readRDS('cache/moh/treso_origin_cases.rds') %>% 
  ...
  select(-siteid) %>% 
  distinct() %>% 
  left_join(treso_population_moh, by = c("orig.treso.zone" = "treso_zone", "age_cluster")) %>% 
...
HBAMprojected_filtered <- HBAMprojected_master_tz %>% 
...
crm_hosp_tbl_join <- left_join(crm_hosp_tbl, HBAMprojected_filtered, by = c("id", "caretype", "age_cluster")) %>% 
...
if (USER_SELECT == "HBAM") {
  crm_treso_tbl <- left_join(crm_treso_tbl, crm_hosp_tbl_join, by = c("id", "caretype", "age_cluster")) %>% 
    mutate(cases = hosp.treso.cases * scale.factor)
} else if (USER_SELECT == "CRM") {
  crm_treso_tbl <- crm_treso_tbl %>% 
