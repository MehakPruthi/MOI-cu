...
if (DECISION_MAKING) {
    UTILIZATION_TARGETS <- tibble(caretype = c('AT', 'MH', 'CR', 'GR', 'AM'), target = c(1.00, 1.00, 1.00, 1.00, 1.00))
...
USER_LTC_RATE_FACTOR <- 0.95 # CALIBRATION- this factor at 0.95 matches closest wtih observed 2018 data (daily bed census and reported alc rate)

# Number of LTC beds added by CSD
USER_NEW_LTC_CAP_CSD_TOTAL <- 
tibble(csduid = c(3501012, 3519049), added.ltc.cap = c(15000, 0))
...
USER_INPUT_CURRENT_YEAR = 2020 # Current year used to calculate construction timespan
...
if (nrow(filter(hospital_wide_final, id > 999)) > 0) {
  ...
  cost_hospitals <- bind_rows(cost_existing_hospital, cost_new_hospital)
}
...
write.csv(hospital_wide_final,"RESULT.csv")
```
