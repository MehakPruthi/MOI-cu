...
HBAMhistorical_origcsd_cases <- HBAMhistorical_master_tz %>%
  group_by(year, id, agecluster, caretype, orig_csd) %>%
...
treso_origin_cases <- HBAMhistorical_origcsd_cases %>% 
  ...
  left_join(treso_population_long, by = c('year', 'orig_csd' = 'csduid', 'agecluster')) %>% 
...
crm_alc <- select(hospital_lookup_master, year, id, hosp.csd = csduid) %>%
  ...
  left_join(select(crm_treso, id, hospitalname, agecluster, caretype, orig_csd, orig.treso.zone, hosp.treso.cases, hosp.treso.beddays, hosp.treso.bedsneeded), by = c('id')) %>% 
  mutate(hosp.cd = substr(hosp.csd, 1, 4)) %>%
  group_by(id, agecluster) %>%
  ungroup() %>%
  ...
  ungroup() %>% 
  select(id, hosp.csd, hosp.cd, hospitalname, agecluster, caretype, orig.csd = orig_csd, orig.treso.zone, hosp.treso.cases, 
...
if (decision_making) {  
  ...
  # # Determine travel times by TRESO/CSD if maintaining existing travel patterns
  # crm_travel_time <- crm_alc %>%
  #   rename(orig.treso = orig.treso.zone) %>% 
  #   # Join in treso.zone info for hospitals
  ...
  #   group_by(orig.csd, caretype) %>% 
  #   mutate(orig.csd.caretype.travel.time = sum(cases.travel.time))
...
if (decision_making) {  
  new_hospital_dm <- overburden_csd_hosp_new %>% 
    ...
    left_join(select(treso_population_moh_agecluster, treso_zone, agecluster, scen.proj.pop.treso), by = c('treso_id' = 'treso_zone')) %>% 
...
crm_demand <- crm_hosp_agecluster_all %>% 
  ...
  left_join(select(treso_zone_system, csduid, treso_id), by = c('orig.csd' = 'csduid')) %>% 
  left_join(select(treso_population_moh_agecluster, treso_zone, agecluster, scen.proj.pop.treso), by = c('treso_id' = 'treso_zone', 'agecluster')) %>% 
  ...
  group_by(id, caretype) %>% 
  mutate(beds.needed = ceiling(sum(demand.days.total) / hosp.op.days)) %>% 
  ungroup()

existing_hospital_wide <- crm_demand %>% 
  # Remove new hospitals from 
  # filter(id <= 999) %>% 
  ...
  ungroup() %>% 
  rename(beds = beds.needed) %>% 
  select(id, caretype, agecluster, beds, cases, demand.days.total, demand.days.nonALC, demand.days.ALC) %>%
  ...
  pivot_wider(names_from = c(caretype), values_from = c(beds, cases_ad, cases_ped, cases_sr, demand.days.total_ad, demand.days.total_ped, 
                                                        demand.days.total_sr, demand.days.nonALC_ad, demand.days.nonALC_ped, 
                                                        demand.days.nonALC_sr, demand.days.ALC_ad, demand.days.ALC_ped, 
                                                        demand.days.ALC_sr)) %>% 
  # Make sure there are exactly 0 beds for AM cases (Emergency)
  mutate(beds_AM = 0) %>% 
  # Replace NA values
  replace_na(list(beds_AM = 0, beds_AT = 0, beds_CR = 0, beds_GR = 0, beds_MH = 0,  cases_ad_AM = 0, cases_ad_AT = 0, 
                  cases_ad_CR = 0, cases_ad_GR = 0, cases_ad_MH = 0, cases_ped_AM = 0, cases_ped_AT = 0, cases_ped_CR = 0, 
                  cases_ped_GR = 0, cases_ped_MH = 0, cases_sr_AM = 0, cases_sr_AT = 0, cases_sr_CR = 0, cases_sr_GR = 0, 
                  cases_sr_MH = 0, demand.days.total_ad_AM = 0, demand.days.total_ad_AT = 0, demand.days.total_ad_CR = 0, 
                  demand.days.total_ad_GR = 0, demand.days.total_ad_MH = 0, demand.days.total_ped_AM = 0, demand.days.total_ped_AT = 0, 
                  demand.days.total_ped_CR = 0, demand.days.total_ped_GR = 0, demand.days.total_ped_MH = 0, demand.days.total_sr_AM = 0, 
                  demand.days.total_sr_AT = 0, demand.days.total_sr_CR = 0, demand.days.total_sr_GR = 0, demand.days.total_sr_MH = 0, 
                  demand.days.nonALC_ad_AM = 0, demand.days.nonALC_ad_AT = 0, demand.days.nonALC_ad_CR = 0, demand.days.nonALC_ad_GR = 0, 
                  demand.days.nonALC_ad_MH = 0, demand.days.nonALC_ped_AM = 0, demand.days.nonALC_ped_AT = 0, 
                  demand.days.nonALC_ped_CR = 0, demand.days.nonALC_ped_GR = 0, demand.days.nonALC_ped_MH = 0, 
                  demand.days.nonALC_sr_AM = 0, demand.days.nonALC_sr_AT = 0, demand.days.nonALC_sr_CR = 0, demand.days.nonALC_sr_GR = 0, 
                  demand.days.nonALC_sr_MH = 0, demand.days.ALC_ad_AM = 0, demand.days.ALC_ad_AT = 0, demand.days.ALC_ad_CR = 0, 
                  demand.days.ALC_ad_GR = 0, demand.days.ALC_ad_MH = 0, demand.days.ALC_ped_AM = 0, demand.days.ALC_ped_AT = 0, 
                  demand.days.ALC_ped_CR = 0, demand.days.ALC_ped_GR = 0, demand.days.ALC_ped_MH = 0, demand.days.ALC_sr_AM = 0, 
                  demand.days.ALC_sr_AT = 0, demand.days.ALC_sr_CR = 0, demand.days.ALC_sr_GR = 0, demand.days.ALC_sr_MH = 0)) %>% 
  mutate(total.hosp.beds = beds_AM + beds_AT + beds_CR + beds_GR + beds_MH)

########### JOIN IN EXISTING NUMBER OF BEDS AND TAKE THE MAX OF PROJECTED VS. EXISTING TO DETERMINE NUMBER OF FORECAST BEDS
########### USE USER-INPUT / DM LAYER BEDS AS 'EXISTING' FOR NEW HOSPITALS

# Create wide version of new_hospital table in order to bind into a list of assets
new_hospital_wide <- new_hospital %>% 
  spread(key = caretype, value = beds) %>% 
  rename(am.beds.forecast = AM, at.beds.forecast = AT, cr.beds.forecast = CR, gr.beds.forecast = GR, mh.beds.forecast = MH) %>% 
  mutate(am.beds.forecast = 0) %>% 
  select(-agecluster) %>% 
  distinct()

# Building Hospital Asset Dataframe
hospital_assets_hist <- hospital_lookup_master %>% 
  select(year, id, name, lhin, csduid, csdname, treso_zone, mof_region, pcode, hospital.lat = lat, hospital.long = long, address, city, 
         total.hosp.beds:mh.beds.2017, cases.at.ped:cases.am.sr) %>% 
  bind_rows(., new_hospital_wide) %>% 
  # replace_na(list(am.beds.2017 = 0, at.beds.2017 = 0, cr.beds.2017 = 0, gr.beds.2017 = 0, mh.beds.2017 = 0))
  
  
  # Add empty columns for demand days
  mutate(demand.days.total.at.ped = 0, demand.days.total.at.ad = 0, demand.days.total.at.sr = 0, 
         demand.days.total.cr.ped = 0, demand.days.total.cr.ad = 0, demand.days.total.cr.sr = 0, 
         demand.days.total.gr.ped = 0, demand.days.total.gr.ad = 0, demand.days.total.gr.sr = 0,
         demand.days.total.mh.ped = 0, demand.days.total.mh.ad = 0, demand.days.total.mh.sr = 0,
         demand.days.total.am.ped = 0, demand.days.total.am.ad = 0, demand.days.total.am.sr = 0,
         demand.days.nonALC.at.ped = 0, demand.days.nonALC.at.ad = 0, demand.days.nonALC.at.sr = 0, 
         demand.days.nonALC.cr.ped = 0, demand.days.nonALC.cr.ad = 0, demand.days.nonALC.cr.sr = 0, 
         demand.days.nonALC.gr.ped = 0, demand.days.nonALC.gr.ad = 0, demand.days.nonALC.gr.sr = 0,
         demand.days.nonALC.mh.ped = 0, demand.days.nonALC.mh.ad = 0, demand.days.nonALC.mh.sr = 0,
         demand.days.nonALC.am.ped = 0, demand.days.nonALC.am.ad = 0, demand.days.nonALC.am.sr = 0,
         demand.days.ALC.at.ped = 0, demand.days.ALC.at.ad = 0, demand.days.ALC.at.sr = 0, 
         demand.days.ALC.cr.ped = 0, demand.days.ALC.cr.ad = 0, demand.days.ALC.cr.sr = 0, 
         demand.days.ALC.gr.ped = 0, demand.days.ALC.gr.ad = 0, demand.days.ALC.gr.sr = 0,
         demand.days.ALC.mh.ped = 0, demand.days.ALC.mh.ad = 0, demand.days.ALC.mh.sr = 0,
         demand.days.ALC.am.ped = 0, demand.days.ALC.am.ad = 0, demand.days.ALC.am.sr = 0) %>% 
  rename(at.beds = at.beds.2017, cr.beds = cr.beds.2017, gr.beds = gr.beds.2017, mh.beds = mh.beds.2017) %>% 
  replace_na(list(total.hosp.beds = 0, at.beds = 0, cr.beds = 0, gr.beds = 0, mh.beds = 0)) %>% 
  mutate(am.beds = 0) %>% 
  # Initialize travel time/distance fields
  mutate(avg.travel.time = 0, avg.travel.distance = 0)
  # Initialize demand fields
  # mutate(am.cases, am.demand.days.total.forecast, demand.days.nonALC.forecast = 0, demand.days.ALC.forecast = 0)
