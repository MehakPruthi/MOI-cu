...
rm(hospital_lookup, dbc_beds_2018)
```
...
# # Establish Orig. CSD to Dest. CSD break down by hospital share
# crm_treso_csd_level <- crm_treso %>%
#   group_by(id, hospitalname, hosp_csduid, orig_csd, caretype, agecluster) %>% 
#   summarise(hosp.share.of.orig.csd.cases = sum(hosp.treso.cases), hosp.share.of.orig.csd.beddays = sum(hosp.treso.beddays), hosp.share.of.orig.csd.bedsneeded = sum(hosp.treso.bedsneeded)) %>% 
#   group_by(orig_csd, caretype, agecluster) %>% 
#   mutate(csd.total.cases = sum(hosp.share.of.orig.csd.cases), csd.total.beddays = sum(hosp.share.of.orig.csd.beddays), csd.total.bedsneeded = sum(hosp.share.of.orig.csd.bedsneeded)) %>% 
#   mutate(hosp_ratio_of_csd_case = hosp.share.of.orig.csd.cases/csd.total.cases, hosp_ratio_of_csd_beddays = hosp.share.of.orig.csd.beddays/csd.total.beddays, hosp_ratio_of_csd_bedsneeded = hosp.share.of.orig.csd.bedsneeded/csd.total.bedsneeded)
...
crm_alc <- select(hospital_lookup_master, year, id, hosp.csd = csduid) %>%
  ...
  left_join(crm_treso_caretypes, by = c('id', 'agecluster')) %>% 
  mutate(demand.days.total = hosp.treso.days.total * hosp.treso.caretype.beddays.percent,
         cases = hosp.treso.cases * hosp.treso.caretype.cases.percent,
         demand.days.nonALC = hosp.treso.beddays * hosp.treso.caretype.beddays.percent,
         demand.days.ALC = hosp.treso.beddays.alc * hosp.treso.caretype.beddays.percent,
         beds.needed = demand.days.total/hosp.op.days) %>% 
  select(id, hosp.csd, hosp.cd, hospitalname, caretype, agecluster, orig.csd, orig.treso.zone, cases, demand.days.total, demand.days.nonALC, demand.days.ALC, beds.needed)
...
crm_csd_agecluster <- crm_alc %>%
  group_by(hosp.csd, agecluster, caretype) %>%
  summarise_at(vars(cases:beds.needed), sum) %>%
  ungroup()
...
crm_orig <- crm_alc %>% 
  group_by(orig.csd, id, hosp.csd, caretype) %>%
  summarise_at(vars(cases:beds.needed), sum) %>%
  ungroup()
```
...
# Incorporate new hospitals added by redistributing demand from other hospitals and csds
  ...
  new_hospital <- tibble(
    id = c(6666, 6666, 6666, 6666, 6666, 7777, 7777, 7777, 7777, 7777), 
    ...
    beds = c(200, 100, 125, 50, 0, 250, 150, 75, 75, 0),
...
new_hospital_rates <- new_hospital_overlay %>% 
  select(-agecluster, -hospitalname, -treso_zone, -hosp.cd) %>% 
  distinct() %>% 
  left_join(distinct(crm_staying, orig.csd, caretype, cases.per.bed.in, demand.days.total.per.bed.in, demand.days.nonALC.per.bed.in, 
                     demand.days.ALC.per.bed.in, cases.per.bed.out, demand.days.total.per.bed.out, demand.days.nonALC.per.bed.out, 
                     demand.days.ALC.per.bed.out, orig.cases, orig.demand.days.total, orig.demand.days.nonALC, orig.demand.days.ALC), by = c('hosp.csd' = 'orig.csd', 'caretype')) %>% 
  ...
  mutate(cases.per.bed.in = ifelse(is.na(cases.per.bed.in), STAY_IN_CSD_FACTOR * cases.per.bed.out, cases.per.bed.in),
         demand.days.total.per.bed.in = ifelse(is.na(demand.days.total.per.bed.in), STAY_IN_CSD_FACTOR * demand.days.total.per.bed.out, demand.days.total.per.bed.in),
         demand.days.nonALC.per.bed.in = ifelse(is.na(demand.days.nonALC.per.bed.in), STAY_IN_CSD_FACTOR * demand.days.nonALC.per.bed.out, demand.days.nonALC.per.bed.in),
         demand.days.ALC.per.bed.in = ifelse(is.na(demand.days.ALC.per.bed.in), STAY_IN_CSD_FACTOR * demand.days.ALC.per.bed.out, demand.days.ALC.per.bed.in)) %>%
  select(-cases.per.bed.out, -demand.days.total.per.bed.out, -demand.days.nonALC.per.bed.out, -demand.days.ALC.per.bed.out) %>% 
  # Assign a representative number of beds to AM (Emergency) caretype to weight distribution of AM cases in later steps
  group_by(id) %>% 
  mutate(beds = ifelse(beds == 0, sum(beds), beds)) %>% 
  ungroup() %>% 
...
crm_staying_new <- crm_staying %>% 
  ...
  group_by(orig.csd, caretype) %>% 
  # Determine new sum of cases / demand-days in order to establish market share per hospital
  # mutate(orig.cases = sum(cases),
  #        orig.demand.days.total = sum(demand.days.total),
  #        orig.demand.days.nonALC = sum(demand.days.nonALC),
  #        orig.demand.days.ALC = sum(demand.days.ALC)) %>% 
...
crm_orig_revised <- crm_staying_new %>% 
  select(orig.csd, id, hosp.csd, caretype, specialityfactor, new.hospital, beds, cases.revised, demand.days.total.revised,          
         demand.days.nonALC.revised, demand.days.ALC.revised)



crm_hosp_agecluster_leaving <- crm_hosp_agecluster %>% 
  # Identify existing hospitals which are speciality hospitals, and join in TRESO/CSD info
  left_join(select(speciality_hospitals, id, specialityfactor), by = c('id')) %>% 
  left_join(select(hospitallocations_tz, id, hosp.csd = csduid), by = c('id')) %>% 
  mutate(new.hospital = 0) %>% 
...
# For CSDs without existing hospitals:
  # Triply weight beds in new hospital in CSD relative to hospitals in other CSDs
  # Check to make sure capacity not overwhelmed?
```
...
crm_hosp_agecluster_all <- crm_hosp_agecluster %>%
  # Identify existing hospitals which are speciality hospitals, and join in TRESO/CSD info
  left_join(select(speciality_hospitals, id, specialityfactor), by = c('id')) %>% 
  left_join(select(hospitallocations_tz, id, hosp.csd = csduid), by = c('id')) %>% 
  mutate(new.hospital = 0) %>% 
...
```{r}  
  ...
  # Insert new hospital to list demand distributed list
  hospital_new_list <- bind_rows(new_hospital_overlay, crm_hosp_agecluster) 

  # Create an updated hospital asset list
  updated_hospital_list <- hospital_new_list %>% 
    select(id, hosp.csd, hosp.cd, hospitalname, new.hospital) %>% 
    distinct()

  # Isolate and copy csd and cd number of new hospital to a new column
  updated_hospital_list_stripped <- updated_hospital_list %>% 
    filter(id == "99999")

  # Filter for hospitals in the same CSD and CD and plant flags
  updated_hospital_list_short <- updated_hospital_list %>% 
    mutate(ref.new.hosp.csd = updated_hospital_list_stripped$hosp.csd, ref.new.hosp.cd = updated_hospital_list_stripped$hosp.cd)
  
  temp_1 <- updated_hospital_list_short %>% 
    filter_(~hosp.csd==ref.new.hosp.csd) %>% 
    group_by(id) %>% 
    mutate(x= (n())) %>%
    group_by(x) %>% 
    mutate(num.hosp.csd = sum(x)) %>% 
    select(-x)

  temp_2 <- updated_hospital_list_short %>% 
    filter_(~hosp.cd==ref.new.hosp.cd) %>% 
    group_by(id) %>% 
    mutate(x= (n())) %>%
    group_by(x) %>% 
    mutate(num.hosp.cd = sum(x)) %>% 
    select(-x)

  updated_hospital_list_short <- updated_hospital_list_short %>% 
    left_join(select(temp_1, id, num.hosp.csd), by = "id") %>% 
    left_join(select(temp_2, id, num.hosp.cd), by = "id") %>% 
    select(-x.x, -x.y)
  
# Transfer filter to full list of hospitals and group by agecluster (ped or other)
## If the new hospital is in a csd with existing hospitals, then it will take a part of their share. the amount of share taken is determined by whether the share-giving hospital is a speciality hospital.
hospital_demand_dist <- hospital_new_list %>% 
  right_join(select(updated_hospital_list_short, id, ref.new.hosp.csd, ref.new.hosp.cd, num.hosp.csd, num.hosp.cd), by = 'id') 

hospital_demand_dist_csd <- hospital_demand_dist %>% 
  drop_na(num.hosp.csd) %>% 
  group_by(id, caretype, specialityfactor, new.hospital, beds) %>% 
  mutate (cases.csd = sum(cases), demand.days.total.csd =sum(demand.days.total)) %>% 
  mutate(cases.csd = cases.csd, demand.beds = demand.days.total.csd/hosp.op.days) %>%
  ungroup() %>% 
  replace(is.na(.), 0) %>% 
  group_by(caretype) %>% 
  mutate(total.cases = sum(cases.csd), total.demand.beds.csd = sum(demand.beds))
  

  
# Create a list of hospitals available in the new hospital CSD
...
# Read in minimum and maximum size of hospital (in number of beds) for potential new hospitals
#min_hosp_size_beds <- 100 # TODO: Replace this hard-coded value with user-input value from visualization
#max_hosp_size_beds <- 1500 # TODO: Replace this hard-coded value with user-input value from visualization

# Read in list of new hospitals
# TODO: Replace this hard-coded tibble with user-input value from visualization
new_hosp_list <- tibble(user_input_AT_beds = c(100, 150), 
                        user_input_CR_beds = c(25, 50), 
                        user_input_GR_beds = c(0, 50), 
                        user_input_MH_beds = c(50, 25), 
                        user_input_AM_beds = c(0, 0), 
                        user_input_beds_total = c(175, 275))

# Calculate costs
new_hosp_list_complete <- new_hosp_list %>% 
  mutate(cost_2018 = round(user_input_beds_total * user_input_cost_per_bed + user_input_base_hosp_cost, 0))
