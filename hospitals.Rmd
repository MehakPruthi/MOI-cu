...
######### Set up target utilizations as tibble
######### Join target tibble based on caretype, called 'target'
######### THen proceed with query below

# Join demand to capacity data
demand_capacity_difference <- crm_csd_agecluster %>% 
  left_join(csd_beds_existing, by = c('orig.csd' = 'hosp.csd', 'caretype')) %>% 
  replace_na(list(csd.beds.existing = 0)) %>% 
  # Calculate total beds needed across all ageclusters
  group_by(orig.csd, caretype) %>% 
  mutate(csd.beds.needed = sum(beds.needed),
         adjusted.csd.beds.needed = ceiling(csd.beds.needed / target)) %>% 
  ungroup() %>% 
