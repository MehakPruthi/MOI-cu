...
num_beds <- readRDS('cache/hospitals/num_beds.rds')
...
cd_caserates <- readRDS('cache/hospitals/cd_caserates.rds') %>% 
  select(year,
         ...
         caretype,
         agegroup,
         cd_caserate = caserate) %>% 
  mutate(cd = as.numeric(cd))

csd_caserates <- readRDS('cache/hospitals/csd_caserates.rds') %>% 
  filter(csd_caserates != Inf) #TRESO zones with 0 population resulted in Inf caserates, causing a mismatch of total cases between Origin and Destination vectors when calcluated at the CSD-level (see below)

caserates_comp <- readRDS('cache/hospitals/caserates_comp.rds')
...
saveRDS(prop_matrix, file = 'cache/hospitals/prop_matrix.rds')
...
saveRDS(hospital_overlay, file = 'cache/hospitals/hospital_overlay.rds')
...
```{r}
#Update Master HBAM liste to Include TRESO Zones
hospital_overlay <- readRDS('cache/hospitals/hospital_overlay.rds')
...
prop_matrix <- readRDS('cache/hospitals/prop_matrix.rds')
...
hospital_lookup <- HBAMhistorical_master_tz %>% 
  ...
  mutate(cases_at_ad = ifelse(caretype %in% 'AT' & age_cluster %in% 'ad', totalcases, 0),
         cases_at_ped = ifelse(caretype %in% 'AT' & age_cluster %in% 'ped', totalcases, 0),
         cases_at_sr = ifelse(caretype %in% 'AT' & age_cluster %in% 'sr', totalcases, 0),
         cases_cr_ad = ifelse(caretype %in% 'CR' & age_cluster %in% 'ad', totalcases, 0),
         cases_cr_ped = ifelse(caretype %in% 'CR' & age_cluster %in% 'ped', totalcases, 0),
         cases_cr_sr = ifelse(caretype %in% 'CR' & age_cluster %in% 'sr', totalcases, 0),
         cases_gr_ad = ifelse(caretype %in% 'GR' & age_cluster %in% 'ad', totalcases, 0),
         cases_gr_ped = ifelse(caretype %in% 'GR' & age_cluster %in% 'ped', totalcases, 0),
         cases_gr_sr = ifelse(caretype %in% 'GR' & age_cluster %in% 'sr', totalcases, 0),
         cases_mh_ad = ifelse(caretype %in% 'MH' & age_cluster %in% 'ad', totalcases, 0),
         cases_mh_ped = ifelse(caretype %in% 'MH' & age_cluster %in% 'ped', totalcases, 0),
         cases_mh_sr = ifelse(caretype %in% 'MH' & age_cluster %in% 'sr', totalcases, 0),
         cases_am_ad = ifelse(caretype %in% 'AM' & age_cluster %in% 'ad', totalcases, 0),
         cases_am_ped = ifelse(caretype %in% 'AM' & age_cluster %in% 'ped', totalcases, 0),
         cases_am_sr = ifelse(caretype %in% 'AM' & age_cluster %in% 'sr', totalcases, 0)) %>% 
...
dbc_beds <- full_join(dbc_beds_2017, dbc_beds_2018, by = c('name', 'pcode', 'siteid')) %>%
  ...
  right_join(select(hospital_lookup, pcode, name), by = 'pcode')
...
saveRDS(hospital_lookup_master, file = 'output/hospital_lookup_master.rds')
```
