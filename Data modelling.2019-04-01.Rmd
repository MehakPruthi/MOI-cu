...
schoolPost <- read.csv('C:/Users/azk/Documents/Working away files/EDU_enrol_SCH_PCode_WSP_Temp.csv', stringsAsFactors = FALSE)
pccf <- read.csv('P:/Y362/2019/191-00826-00 MOI Capacity and Demand Model/3. Modelling/PCCF_ONT_2018.csv', stringsAsFactors = FALSE) #postal code conversion file --> postal codes to lat/long
...
names(schoolForecast)
schoolBSID <- schoolForecast %>%
  filter(SCH_YR == 2017) %>%
  select(BSID, schoolName = `School Name`, SFIS, schoolLat = latitude, schoolLong = longitude, DSBINDEX) %>%
...
schoolCatchment <- schoolBSID %>%
  select(BSID, SFIS, schoolName, schoolLat, schoolLong, DSBINDEX) %>%
  left_join(select(schoolPostSlim, BSID, Student.Postal.Code, Enrolment), by = c('BSID' = 'BSID')) %>%
...
#Step 5: Determine 0-90th percentile student travel distances
studentTravel <- schoolCatchmentDist %>%
  left_join(select(schoolCatchmentDistPercent, BSID, schoolName, schoolLat, percDist, DSBINDEX), by = c('BSID' = 'BSID')) %>%
  filter(dist <= percDist) %>%
  write.csv('C:/Users/azk/Documents/Working away files/student_travel.csv')
  #group_by(BSID) %>%
  #summarise(count = n()) %>%
  #arrange(BSID)
